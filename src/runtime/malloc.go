...
const (
	...
	arenaBaseOffset = 0xffff800000000000*sys.GoarchAmd64 + 0x0a00000000000000*sys.GoosAix	//所有arena的基址  0xffff800000000000
...
var (
	...
	physHugePageShift uint	//2的physHugePageShift次方等于physHugePageSize
)
...
mapped:
	...
	for ri := arenaIndex(uintptr(v)); ri <= arenaIndex(uintptr(v)+size-1); ri++ {
		...
		if r == nil {	//表示heapArena未初始化
			r = (*heapArena)(persistentalloc(unsafe.Sizeof(*r), sys.PtrSize, &memstats.gc_sys))	//申请空间
			if r == nil {
		...
		if len(h.allArenas) == cap(h.allArenas) {	//到达上限，重申申请一个更大的数组并将源数据拷贝
...
func mallocgc(size uintptr, typ *_type, needzero bool) unsafe.Pointer {
	...
	noscan := typ == nil || typ.ptrdata == 0	//noscan为true表示没有指针
	if size <= maxSmallSize {	//maxSmallSize=32768
		if noscan && size < maxTinySize {
			...
			spc := makeSpanClass(sizeclass, noscan)	//sizeclass << 1 | noscan
			span = c.alloc[spc]
			v := nextFreeFast(span)
			if v == 0 {
...
type persistentAlloc struct {
	base *notInHeap	//mmap的起始地址
	off  uintptr	//该成员记录了已用的空间
}
