...
type timer struct {
	...
	arg    interface{} // 某个调用了time.Sleep的g
	seq    uintptr
...
func timeSleep(ns int64) {
	...
	gp := getg()
	t := gp.timer
	if t == nil { // 懒申请gp.timer的内存空间
		t = new(timer)
		gp.timer = t
	}
	t.f = goroutineReady
	t.arg = gp
	t.nextwhen = nanotime() + ns // 计算出多少纳秒时停止睡眠
...
func doaddtimer(pp *p, t *timer) {
	...
	t.pp.set(pp)
	i := len(pp.timers)              // 新添加的timer在pp.timers中的索引
	pp.timers = append(pp.timers, t) // 将timer添加到pp.timers
...
func modtimer(t *timer, when, period int64, f func(interface{}, uintptr), arg interface{}, seq uintptr) bool {
	...
	wasRemoved := false
	var pending bool // 返回结果,报告定时器在运行前是否被修改
	var mp *m
loop:
	for {
		...
		case timerNoStatus, timerRemoved:
			...
			// Act like addtimer.
			if atomic.Cas(&t.status, status, timerModifying) { // 将timer的状态切换为timerModifying
				wasRemoved = true
	...
	if wasRemoved {
		t.when = when // 设置定时器的唤醒时间
		pp := getg().m.p.ptr()
		lock(&pp.timersLock) // 加锁以将timer添加到p的堆上
		doaddtimer(pp, t)
		unlock(&pp.timersLock)
...
// Reports whether the timer was modified before it was run.
func resettimer(t *timer, when int64) bool { // when代表了何时解除Sleep状态,单位是纳秒
...
func siftupTimer(t []*timer, i int) {
	...
	when := t[i].when // timer唤醒时间
	tmp := t[i]       // 获取timer
	for i > 0 {       // 找到t中某个when小于等于tmp的
		p := (i - 1) / 4 // parent
		if when >= t[p].when {
			break
		}
