...
func getproccount() int32 {
	...
	r := sched_getaffinity(0, unsafe.Sizeof(buf), &buf[0])	//获取线程cpu亲和力保存到buf[0]
	if r < 0 {
		return 1
	}
	n := int32(0)
...
//go:nowritebarrier
func newosproc(mp *m) {
	stk := unsafe.Pointer(mp.g0.stack.hi)	//g0栈的栈底
	/*
	...
	ret := clone(cloneFlags, stk, unsafe.Pointer(mp), unsafe.Pointer(mp.g0), unsafe.Pointer(funcPC(mstart)))	// 从mstart函数开始运行的系统线程
...
func getHugePageSize() uintptr {
	...
	ptr := noescape(unsafe.Pointer(&numbuf[0]))	//类似于 ptr:=&numbuf[0]
	n := read(fd, ptr, int32(len(numbuf)))	//读取文件内容到ptr中
	closefd(fd)
	if n <= 0 {
		return 0
	}
...
func osinit() {
	ncpu = getproccount()	//cpu核数
	physHugePageSize = getHugePageSize()	//获取操作系统默认物理大页面的字节大小
	osArchInit()
}
...
func minit() {
	...
	// useful in debuggers.
	getg().m.procid = uint64(gettid())	//获取线程tid
}
