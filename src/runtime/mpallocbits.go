...
func (b *pallocBits) summarize() pallocSum {
	...
	for i := 0; i < len(b); i++ {
		a := b[i]	//取出数组中元素
		for j := 0; j < 64; j += 8 {	//j用于向右移位，这也表示b中的元素是按照8bit进行拆分的
			k := uint8(a >> j)

			// Compute start.
			si := uint(sys.TrailingZeros8(k))	//k的低位包含多少个0
			...
			if mi := consec8tab[k]; mi > max {	//mi为k的低bit位连续为0的个数
				max = mi
			}
