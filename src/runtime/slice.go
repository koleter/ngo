...
func cutslice(et *_type, old slice, i, j, k int) slice {
	...
	case et.size == 1:
		lenmem = uintptr(old.len)
		newlenmem = uintptr(len)
		capmem = roundupsize(uintptr(len))
		overflow = uintptr(len) > maxAlloc
	case et.size == sys.PtrSize:
		lenmem = uintptr(old.len) * sys.PtrSize
		newlenmem = uintptr(len) * sys.PtrSize
		capmem = roundupsize(uintptr(len) * sys.PtrSize)
		overflow = uintptr(len) > maxAlloc/sys.PtrSize
	case isPowerOfTwo(et.size):
		var shift uintptr
		if sys.PtrSize == 8 {
			// Mask shift for better code generation.
		...
		lenmem = uintptr(old.len) << shift
		newlenmem = uintptr(len) << shift
		capmem = roundupsize(uintptr(len) << shift)
		overflow = uintptr(len) > (maxAlloc >> shift)
	default:
		lenmem = uintptr(old.len) * et.size
		newlenmem = uintptr(len) * et.size
		capmem, overflow = math.MulUintptr(et.size, uintptr(len))
		capmem = roundupsize(capmem)
	}
	...
	destPtr := uintptr(p)
	srcPtr := uintptr(old.array)
	stepSize := uintptr(int(et.size) * k)

	for i := 0; i < len; i++ {
		memmove(unsafe.Pointer(destPtr), unsafe.Pointer(srcPtr), et.size)
		destPtr += et.size
		srcPtr += stepSize
	}
