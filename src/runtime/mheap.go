...
type mheap struct {
	...
	sweepdone uint32    // if set to 0,all spans are swept
...
type arenaHint struct {
	addr uintptr	//arena的基址
	down bool	//表示arena的生长方向，为true时向下生长
	next *arenaHint
}
...
type mspan struct {
	...
	// helps performance.
	nelems uintptr // number of object in the span.		// 块个数，也即有多少个块可供分配
	...
	allocCount  uint16        // number of allocated objects		// 已分配块的个数
	spanclass   spanClass     // size class and noscan (uint8)		// class表中的class ID
	state       mSpanStateBox // mSpanInUse etc; accessed atomically (get/set methods)
	needzero    uint8         // needs to be zeroed before allocation
	divShift    uint8         // for divide by elemsize - divMagic.shift
	divShift2   uint8         // for divide by elemsize - divMagic.shift2
	elemsize    uintptr       // computed from sizeclass or from npages		// class表中的对象大小，也即块大小
...
func arenaIndex(p uintptr) arenaIdx {
	return arenaIdx((p - arenaBaseOffset) / heapArenaBytes)	//p的地址减去起初申请arena的基地址再除以每个arena的字节数
}
...
func (h *mheap) reclaim(npage uintptr) {
	...
	for npage > 0 {
		...
		idx := uintptr(atomic.Xadd64(&h.reclaimIndex, pagesPerReclaimerChunk) - pagesPerReclaimerChunk)	//相当于idx = h.reclaimIndex
...
func (h *mheap) grow(npage uintptr) bool {
	...
	ask := alignUp(npage, pallocChunkPages) * pageSize	//将堆区提升的字节数大小

	totalGrowth := uintptr(0)
	// This may overflow because ask could be very large
	// and is otherwise unrelated to h.curArena.base.
	end := h.curArena.base + ask
	nBase := alignUp(end, physPageSize)	//physPageSize=4096
...
func (list *mSpanList) remove(span *mspan) {
	...
	if list.first == span {	//span是链表的第一项？
		list.first = span.next
	} else {
		span.prev.next = span.next
	}
	if list.last == span {	//span是链表的最后一项？
		list.last = span.prev
	} else {
