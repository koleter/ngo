...
func main() {
	...
	exit(0)
	//保护性代码，如果exit意外返回，下面的代码也会让该进程死掉
	for {
		var x *int32
		*x = 0
	}
...
func gopark(unlockf func(*g, unsafe.Pointer) bool, lock unsafe.Pointer, reason waitReason, traceEv byte, traceskip int) {
	if reason != waitReasonSleep {	// 不是因为time.Sleep函数将这个g挂起的
		checkTimeouts() // timeouts may expire while two goroutines keep the scheduler busy
	}
	mp := acquirem()
	gp := mp.curg
	status := readgstatus(gp) // 获取协程状态
	if status != _Grunning && status != _Gscanrunning {
		throw("gopark: bad g status")
	}
	mp.waitlock = lock // 这里的lock如果由Sleep进入的话是一个g.timer
...
func cpuinit() {
	...
	case "aix", "darwin", "dragonfly", "freebsd", "netbsd", "openbsd", "illumos", "solaris", "linux":
		...
		n := int32(0)
		for argv_index(argv, argc+1+n) != nil { //提取环境变量的数量
			n++
		}

		for i := int32(0); i < n; i++ {
			p := argv_index(argv, argc+1+i) //取出环境变量
...
func schedinit() {
	...
	sched.maxmcount = 10000 //最多启动10000个工作线程（M）
	...
	procs := int32(1)                                         //初始化P的个数，为cpu的核数		原本这里的是int32(ncpu)
	if n, ok := atoi32(gogetenv("GOMAXPROCS")); ok && n > 0 { //此处go在linux中的编译有些问题
		procs = n //如果环境变量中设置了GOMAXPROCS，则创建指定数量的P
	}
...
func mReserveID() int64 {
	if sched.mnext+1 < sched.mnext {	// 判断溢出
		throw("runtime: thread ID overflow")
	}
	id := sched.mnext
	sched.mnext++
	checkmcount() //检测已创建的线程是否超过了数量限制（10000）
	return id
}
...
func mcommoninit(mp *m, id int64) {
	...
	lock(&sched.lock) //因为sched是一个全局变量，多个线程同时操作sched会有并发问题，因此要先加锁
	...
	mpreinit(mp) //创建用于信号处理的gsignal，只是简单的从堆上分配一个g结构体对象，然后把栈设置好就返回了
	if mp.gsignal != nil {
...
func stopTheWorldWithSema() {
	...
	if wait {
		for {
			...
			if notetsleep(&sched.stopnote, 100*1000) { // 等待这些goroutine暂停
...
func mstart1() {
	...
	save(getcallerpc(), getcallersp()) //getcallerpc获取mstart1执行完的返回地址，getcallersp获取调用mstart1时的栈顶地址
...
func stopm() {
	...
	noteclear(&_g_.m.park) //被其它工作线程唤醒
	acquirep(_g_.m.nextp.ptr())
	_g_.m.nextp = 0
}
...
func startm(_p_ *p, spinning bool) {
	...
	if _p_ == nil { //没有指定p的话需要从p的空闲队列中获取一个p
		...
		if _p_ == nil {  // 没有空闲的p
			unlock(&sched.lock)
			if spinning { // 为true表示进入这个函数之前已经对sched.nmspinning加了1，需要还原
	...
	if mp == nil {
		...
		// queued G's.
		id := mReserveID()	// 获取newM的id
		unlock(&sched.lock)

		var fn func()
		if spinning {
			// The caller incremented nmspinning, so set m.spinning in the new M.
			fn = mspinning	// getg().m.spinning = true
		}
	...
	mp.nextp.set(_p_)	// 设置mp的下一个p
	notewakeup(&mp.park) //唤醒处于休眠状态的工作线程
}
...
func handoffp(_p_ *p) {
	...
	if !runqempty(_p_) || sched.runqsize != 0 {		// _p_的局部g队列不为空
		startm(_p_, false)
		return
	}
...
func stoplockedm() {
	...
	if _g_.m.p != 0 {	// 存在绑定的p
		...
		_p_ := releasep() //断开p与m的关系并将p设置为空闲状态
		handoffp(_p_)
	}
	incidlelocked(1)
...
//
//go:yeswritebarrierrec
func execute(gp *g, inheritTime bool) {	// 运行gp
	_g_ := getg()	// 获取当前的g,应该是g0

	// Assign gp.m before entering _Grunning so running Gs have an
	// M.
	_g_.m.curg = gp //把gp和m关联起来,表示要运行gp上的user code了
	gp.m = _g_.m	// 绑定gp的m
	casgstatus(gp, _Grunnable, _Grunning) //设置待运行g的状态为_Grunning
	gp.waitsince = 0	// 设置gp被阻塞的时间为0
	gp.preempt = false	// 设置抢占信号为false
	gp.stackguard0 = gp.stack.lo + _StackGuard
	if !inheritTime {	// inheritTime为false就开启一个新的时间片
		_g_.m.p.ptr().schedtick++	// p的调度次数加一
	}
...
top:
	...
	// local runq
	if gp, inheritTime := runqget(_p_); gp != nil { //再次看一下本地运行队列是否有需要运行的goroutine
		return gp, inheritTime
	}

	// global runq
	if sched.runqsize != 0 { //再看看全局运行队列是否有需要运行的goroutine
		lock(&sched.lock)
	...
	if !_g_.m.spinning {
		_g_.m.spinning = true             //设置m的状态为spinning
	...
	for i := 0; i < 4; i++ {
		for enum := stealOrder.start(fastrand()); !enum.done(); enum.next() {
			...
			p2 := allp[enum.position()] // 随机获取一个p
			if _p_ == p2 {
				continue
			}
			if gp := runqsteal(_p_, p2, stealRunNextG); gp != nil { // 从p2的运行队列中偷取g放到_p_的运行队列中
				return gp, false
			}
...
top:
	...
	if gp == nil {
		...
		if _g_.m.p.ptr().schedtick%61 == 0 && sched.runqsize > 0 { //每进行61次调度就需要优先从全局运行队列中获取goroutine,因为如果只调度本地队列中的g，那么全局运行队列中的goroutine将得不到运行
	...
	if gp == nil {
		gp, inheritTime = runqget(_g_.m.p.ptr()) //从与m关联的p的本地运行队列中获取goroutine
		// We can see gp != nil here even if the M is spinning,
		// if checkTimers added a local goroutine via goready.
	}
	if gp == nil { //如果从本地运行队列和全局运行队列都没有找到需要运行的goroutine，则从其他工作线程的运行队列中偷取，偷取不到，当前工作线程睡眠,直到获取到需要运行的goroutine之后findrunnable函数才会返回
	...
	//当前函数调用栈使用的是g0的栈空间，调用execute切换到gp的代码和栈空间去运行
...
func dropg() {
	...
	setMNoWB(&_g_.m.curg.m, nil) //设置*_g_.m.curg.m == nil
	setGNoWB(&_g_.m.curg, nil)   //设置*_g_.m.curg == nil
}
...
func park_m(gp *g) { // gp是切换为g0之前的g
	...
	casgstatus(gp, _Grunning, _Gwaiting) // 将协程从运行状态转为等待状态
	dropg()                              //解除g和m之间的关系

	if fn := _g_.m.waitunlockf; fn != nil {
		ok := fn(gp, _g_.m.waitlock)	// 尝试进行解锁
		_g_.m.waitunlockf = nil
		_g_.m.waitlock = nil
		if !ok {	// 解锁函数返回false就将这个gp运行
			if trace.enabled {
				traceGoUnpark(gp, 2)
			}
			casgstatus(gp, _Gwaiting, _Grunnable)	// 从等待状态切换为可运行状态
...
func goschedImpl(gp *g) {
	...
	casgstatus(gp, _Grunning, _Grunnable) //将gp的状态从running变成runnable
	dropg()
	lock(&sched.lock)
	globrunqput(gp) //将gp放到runq队列中
	unlock(&sched.lock)
...
func goexit0(gp *g) {
	_g_ := getg() // 获取当前绑定的g,这里是g0

	casgstatus(gp, _Grunning, _Gdead) // 将gp的状态从运行转为dead
...
func malg(stacksize int32) *g {
	...
	if stacksize >= 0 {
		...
		*(*uintptr)(unsafe.Pointer(newg.stack.lo)) = 0 //*(newg.stack.lo)=0
	}
	return newg
}
...
func newproc(siz int32, fn *funcval) {
	argp := add(unsafe.Pointer(&fn), sys.PtrSize) //获取fn函数的第一个参数的指针
	gp := getg()                                  //获取正在运行的g，初始化时是m0.g0
	pc := getcallerpc()                           //getcallerpc()返回一个地址，也就是调用newproc时由call指令压栈的函数返回地址，
	systemstack(func() {                          //systemstack的作用是切换到g0栈执行作为参数的函数
...
func newproc1(fn *funcval, argp unsafe.Pointer, narg int32, callergp *g, callerpc uintptr) *g {
	...
	siz := narg
	siz = (siz + 7) &^ 7 //将fn的参数size按8字节对齐后向上取整
	...
	_p_ := _g_.m.p.ptr() //初始化时_p_ = g0.m.p,其实是allp[0]
	newg := gfget(_p_)   //从p的本地缓冲里获取一个没有使用的g，初始化时没有，返回nil
	if newg == nil {     // 没有缓存的g
		newg = malg(_StackMin)           //new一个g结构体对象，然后从堆上为其分配栈，并设置g的stack成员和两个stackgard
	...
	if narg > 0 { // 存在参数
		memmove(unsafe.Pointer(spArg), argp, uintptr(narg)) // 从argp处拷贝narg字节到spArg处,这里是将参数从系统栈拷贝到新的协程的栈上
	...
	casgstatus(newg, _Gdead, _Grunnable) //设置newg的状态为_Grunnable，可以运行了
...
func gfget(_p_ *p) *g {
retry:
	// 本地gFree链表为空但是全局不为空,从全局链表中获取空闲g放到本地队列知道本地队列中的空闲g到达32个或者全局的空闲g队列为空
...
func (pp *p) destroy() {
	...
	for pp.runqhead != pp.runqtail { // 局部g队列不为空
		// Pop from tail of local queue
		pp.runqtail--
		gp := pp.runq[pp.runqtail%uint32(len(pp.runq))].ptr() // 获取队列最后的g
		// Push onto head of global queue
		globrunqputhead(gp)
	}
	if pp.runnext != 0 { // runnext中的g也放到全局g队列
...
func procresize(nprocs int32) *p {
	...
	for i := nprocs - 1; i >= 0; i-- {
		p := allp[i]
		if _g_.m.p.ptr() == p { // p已经与m关联了就不放到空闲链表中了
			continue
		}
		p.status = _Pidle
...
func acquirep(_p_ *p) {
	...
	wirep(_p_) //_p_与g.m建立关系并将_p_的状态改为running
...
func wirep(_p_ *p) {
	...
	if _p_.m != 0 || _p_.status != _Pidle { // 此时p.m应该是没有绑定且p的状态是空闲的
		id := int64(0)
		if _p_.m != 0 {
			id = _p_.m.ptr().id
		}
...
func preemptall() bool {
	res := false
	for _, _p_ := range allp { // 将所有的_Prunning状态的p中运行的g的抢占标志设置为true
...
func preemptone(_p_ *p) bool {
	mp := _p_.m.ptr()                // 获取m
	if mp == nil || mp == getg().m { // 不抢占不活跃的p(没有绑定m),也不抢占自己的资源
		return false
	}
...
//go:nowritebarrierrec
func mput(mp *m) {
	mp.schedlink = sched.midle // 下一个m指向空闲的m链表
	sched.midle.set(mp)        // 相当于将mp头插到空闲m链表中
	sched.nmidle++             // 空闲m数量加一
	checkdead()
}
...
func globrunqget(_p_ *p, max int32) *g {
	...
	if n > sched.runqsize { // 最多获取所有全局队列中的所有元素
		n = sched.runqsize
	}
	if max > 0 && n > max { // 最多取max个goroutine
		n = max
	}
	if n > int32(len(_p_.runq))/2 { // 最多只能取本地队列容量的一半
		n = int32(len(_p_.runq)) / 2
	}

	sched.runqsize -= n

	gp := sched.runq.pop() //pop从全局运行队列的队列头取,这第一个g返回给调用函数
	n--
	for ; n > 0; n-- { //从全局运行队列中取出n个goroutine放入本地运行队列
		gp1 := sched.runq.pop()
...
func pidleget() *p {
	_p_ := sched.pidle.ptr()
	if _p_ != nil {
		sched.pidle = _p_.link         //sched.pidle指向下一个空闲的p
		atomic.Xadd(&sched.npidle, -1) // TODO: fast atomic
	}
	return _p_
}
...
func runqempty(_p_ *p) bool {
	...
	for {
		...
		if tail == atomic.Load(&_p_.runqtail) {
			return head == tail && runnext == 0	// 头尾相同且没有下一个可运行的g
		}
	}
}
...
func runqput(_p_ *p, gp *g, next bool) {
	...
	retryNext:
		oldnext := _p_.runnext
		if !_p_.runnext.cas(oldnext, guintptr(unsafe.Pointer(gp))) { //把gp放在_p_.runnext成员里
			goto retryNext
		}
...
retry:
	...
	t := _p_.runqtail
	if t-h < uint32(len(_p_.runq)) { // 如果队列未满
		_p_.runq[t%uint32(len(_p_.runq))].set(gp) // 将gp放到队列尾部
...
// Executed only by the owner P.
func runqputslow(_p_ *p, gp *g, h, t uint32) bool { // 将_p_中本地队列的一半的g放到全局运行队列中
	...
	for i := uint32(0); i < n; i++ { // n个g从本地运行队列放到batch中
	...
	batch[n] = gp // batch的最后一个放入gp

	if randomizeScheduler { // 将batch中的g进行乱序
...
func runqget(_p_ *p) (gp *g, inheritTime bool) {
	...
	for {
		next := _p_.runnext //查看runnext成员是否为空，不为空则返回该goroutine
		if next == 0 {
			break
		}
		if _p_.runnext.cas(next, 0) { //比较_p_.runnext和next,如果相等就把0存放到_p_.runnext中并返回true，否则返回false
	...
	for {
		...
		gp := _p_.runq[h%uint32(len(_p_.runq))].ptr() // 获取队列头部的g
...
func runqgrab(_p_ *p, batch *[256]guintptr, batchHead uint32, stealRunNextG bool) uint32 {
	for {
		...
		// 这个判断是为了防止读取了h和t后_p_.runq中的元素有变动的情况
		if n > uint32(len(_p_.runq)/2) { // read inconsistent h and t
			continue
		}
		// 将n个g从_p_.runq中放到batch中,其实就是放到了偷取当前的_p_的p的队列尾部
		for i := uint32(0); i < n; i++ {
...
func runqsteal(_p_, p2 *p, stealRunNextG bool) *g {
	t := _p_.runqtail
	n := runqgrab(p2, &_p_.runq, t, stealRunNextG) // 这个是实际的偷取函数
	if n == 0 {                                    // 没有偷取到g
		return nil
	}
	n--                                               // 因为我们要在这n个g中选一个进行调度
	gp := _p_.runq[(t+n)%uint32(len(_p_.runq))].ptr() // 获取_p_的队列中最后一个g
	if n == 0 {                                       // 此时队列空了无需再处理了
		return gp
	}
	h := atomic.LoadAcq(&_p_.runqhead) // load-acquire, synchronize with consumers
...
func (q *gQueue) pop() *g {
	gp := q.head.ptr() // 获取链表头
	if gp != nil {
		q.head = gp.schedlink // 将链表头指向下一个g
		if q.head == 0 {      // 下一个g不存在表示这个链表已经空了
			q.tail = 0
		}
	}
	return gp
...
func sync_runtime_canSpin(i int) bool {
	...
	if p := getg().m.p.ptr(); !runqempty(p) { // 运行队列中的g不为空
		return false
	}
	return true
}
...
type randomOrder struct {
	count    uint32   //p的数量
	coprimes []uint32 //存储小于count且互质的数
}
...
func doInit(t *initTask) {
	...
	default: // not initialized yet
		...
		for i := uintptr(0); i < t.ndeps; i++ {
			p := add(unsafe.Pointer(t), (3+i)*sys.PtrSize) //p指向线性表中t的下一个元素
			t2 := *(**initTask)(p)
			doInit(t2)
		}
