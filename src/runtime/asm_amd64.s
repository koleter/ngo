...
TEXT runtime·rt0_go(SB),NOSPLIT,$0
	...
	MOVQ	$runtime·g0(SB), DI		//g0的地址赋给DI
	LEAQ	(-64*1024+104)(SP), BX	//栈空间开辟约64k
	...
	MOVL	$0, AX	
	CPUID
	MOVL	AX, SI	//eax为0时CPUID后eax变成了CPUID最大可输入的eax寄存器的value
	CMPL	AX, $0
	JE	nocpuinfo
...
nocpuinfo:
	...
	MOVQ	DI, CX // arg 1
#endif
	CALL	AX	//设置线程栈空间大小为3kb，并设置g0的第一个成员所在的地址
...
needtls:
	...
	LEAQ	runtime·m0+m_tls(SB), DI	//m0的tls的地址赋给DI
	CALL	runtime·settls(SB)	//调用settls设置线程本地存储，设置fs段寄存器的基址为DI+8的位置

	// store through it, to make sure it works		m0与主线程关联在一起
	get_tls(BX)	
	MOVQ	$0x123, g(BX)	//把整数常量0x123拷贝到fs段基地址偏移-8的内存位置，也就是m0.tls[0]=0x123
...
ok:
	...
	get_tls(BX)		//无	
	LEAQ	runtime·g0(SB), CX	//将g0的地址存储到CX， CX=&g0
	MOVQ	CX, g(BX)		//将g0的地址保存在线程本地存储里面，也就是m0.tls[0]=&g0  fs:[0xfffffffffffffff8]
	LEAQ	runtime·m0(SB), AX	//将m0的地址存储到AX，AX = &m0
	...
	MOVQ	AX, 8(SP)
	CALL	runtime·args(SB)	//处理操作系统传递的参数和env
	CALL	runtime·osinit(SB)	//初始化系统核心数，将全局变量ncpu初始化为核心数
	CALL	runtime·schedinit(SB)	//调度器初始化,也就是p的初始化
...
TEXT runtime·gogo(SB), NOSPLIT, $16-8
	MOVQ	buf+0(FP), BX		// gp.sched赋值给BX
	MOVQ	gobuf_g(BX), DX	//DX=gp.sched.g
	MOVQ	0(DX), CX		// make sure g != nil
	get_tls(CX)				//无意义
	MOVQ	DX, g(CX)		// 将gp.sched.g放到fs:[0xfffffffffffffff8]中
...
TEXT runtime·mcall(SB), NOSPLIT, $0-8
	MOVQ	fn+0(FP), DI	// 获取fn参数保存到DI

	get_tls(CX)
	MOVQ	g(CX), AX	// save state in g->sched
	MOVQ	0(SP), BX	// caller's PC      mcall的返回地址放入BX
	...
	MOVQ	g(CX), BX   // 再次获取g赋给BX,此时AX与BX中都保存了g
	...
	JMP	AX
	MOVQ	SI, g(CX)	// 把g0的地址设置到线程本地存储之中
	MOVQ	(g_sched+gobuf_sp)(SI), SP	// sp = m->g0->sched.sp     这里将栈替换为g0的栈
	PUSHQ	AX	// g入栈
	MOVQ	DI, DX	// DI是结构体funcval实例对象的指针,它的第一个成员才是穿入函数的地址
	MOVQ	0(DI), DI	// 读取传入函数的地址到DI寄存器
	CALL	DI  // 调用函数
	POPQ	AX
...
TEXT runtime·systemstack(SB), NOSPLIT, $0-8
	...
	CMPQ	AX, m_gsignal(BX)	//m->gsignal与当前的g是否相同
	JEQ	noswitch
...
TEXT runtime·morestack(SB),NOSPLIT,$0-0
	...
	get_tls(CX)
	MOVQ	g(CX), BX	//将当前的g存储到BX中
	MOVQ	g_m(BX), BX	//g->m存储到BX中
	MOVQ	m_g0(BX), SI	//m->g存储到SI中
	CMPQ	g(CX), SI	//比较两个g
	JNE	3(PC)
	...
	MOVQ	m_gsignal(BX), SI	//m->gsignal存储到SI中
	CMPQ	g(CX), SI	//比较两个g
	JNE	3(PC)
	...
	MOVQ	AX, (m_morebuf+gobuf_pc)(BX)	//f's caller's PC存储到m->morebuf->pc
	LEAQ	16(SP), AX	// f's caller's SP
	MOVQ	AX, (m_morebuf+gobuf_sp)(BX)	//f's caller's SP存储到m->morebuf->sp
	get_tls(CX)
	MOVQ	g(CX), SI						//获取当前的g存储到SI
	MOVQ	SI, (m_morebuf+gobuf_g)(BX)		//当前的g存储到m->morebuf->g

	// Set g->sched to context in f.
	MOVQ	0(SP), AX // f's PC	
	MOVQ	AX, (g_sched+gobuf_pc)(SI)	//f's PC存储到g->sched->pc
	MOVQ	SI, (g_sched+gobuf_g)(SI)	//当前的g存储到g->sched->g
	LEAQ	8(SP), AX // f's SP
	MOVQ	AX, (g_sched+gobuf_sp)(SI)	//f's SP存储到当前的g->sched->pc
	MOVQ	BP, (g_sched+gobuf_bp)(SI)	//存储rbp
	MOVQ	DX, (g_sched+gobuf_ctxt)(SI)	//DX前面已经清0了

	// Call newstack on m->g0's stack.
	MOVQ	m_g0(BX), BX	//获取m->g0保存到rbx
	MOVQ	BX, g(CX)	//切换goroutine为m->g0
	MOVQ	(g_sched+gobuf_sp)(BX), SP	//切换栈为m->g0->sched->sp
...
TEXT ·asmcgocall(SB),NOSPLIT,$0-20
	MOVQ	fn+0(FP), AX	//_cgo_thread_start
	MOVQ	arg+8(FP), BX	//&ts
	...
	get_tls(CX)
	MOVQ	g(CX), R8	//获取fs:[0xfffffffffffffff8]中的g保存到r8
	CMPQ	R8, $0
	JEQ	nosave
	MOVQ	g_m(R8), R8	//m
	MOVQ	m_g0(R8), SI
	MOVQ	g(CX), DI
	CMPQ	SI, DI	//fs:[0xfffffffffffffff8]->m->g0 == fs:[0xfffffffffffffff8] ?
	JEQ	nosave
...
TEXT aeshashbody<>(SB),NOSPLIT,$0-0
	...
	MOVQ	h+8(FP), X0			// 64 bits of per-table hash seed	时间戳
