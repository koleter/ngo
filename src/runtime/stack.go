...
const (
	...
	_StackSystem = sys.GoosWindows*512*sys.PtrSize + sys.GoosPlan9*512 + sys.GoosDarwin*sys.GoarchArm*1024 + sys.GoosDarwin*sys.GoarchArm64*1024	//0
	...
	_FixedStack  = _FixedStack6 + 1		//0x800
	...
	// bottom of the stack.
	_StackGuard = 928*sys.StackGuardMultiplier + _StackSystem	//0x740
...
func stackalloc(n uint32) stack {
	...
	if n < _FixedStack<<_NumStackOrders && n < _StackCacheSize {
		...
		if stackNoCache != 0 || thisg.m.p == 0 || thisg.m.preemptoff != "" {
			...
			x = c.stackcache[order].list	//获取对应size的stackfreelist
			if x.ptr() == nil {
				stackcacherefill(c, order)
				x = c.stackcache[order].list
			}
			c.stackcache[order].list = x.ptr().next	//断链，指向下一个
			c.stackcache[order].size -= uintptr(n)	//调整size
		}
		v = unsafe.Pointer(x)
	} else {
		var s *mspan
...
func copystack(gp *g, newsize uintptr) {
	...
	used := old.hi - gp.sched.sp	//计算出已用栈空间大小
...
func newstack() {
	...
	morebuf := thisg.m.morebuf	//获取f's caller的信息
