...
TEXT runtime·nanotime1(SB),NOSPLIT,$16-8
	...
	get_tls(CX)
	MOVQ	g(CX), AX   // 获取当前的g
	MOVQ	g_m(AX), BX // BX unchanged by C code.  获取g.m
...
ret:
	...
	// return nsec in AX
	IMULQ	$1000000000, AX     // 秒乘以1000000000变成纳秒单位
	ADDQ	DX, AX  // 加上原来获得的纳秒
	MOVQ	AX, ret+0(FP)
	RET
fallback:
	LEAQ	0(SP), DI
...
ok:
	...
	MOVQ	$0, err+40(FP)
	RET
// 参数及类型分别是：addr unsafe.Pointer, n(内存大小) uintptr, prot(读写等mask), flags(private等mask), fd int32, off uint32
// Call the function stored in _cgo_mmap using the GCC calling convention.
// This must be called on the system stack.
TEXT runtime·callCgoMmap(SB),NOSPLIT,$16
	MOVQ	addr+0(FP), DI	//第一个参数
	MOVQ	n+8(FP), SI
	MOVL	prot+16(FP), DX
	MOVL	flags+20(FP), CX
	MOVL	fd+24(FP), R8
...
TEXT runtime·clone(SB),NOSPLIT,$0
	MOVL	flags+0(FP), DI	//flags保存到DI
	MOVQ	stk+8(FP), SI	//skt保存到SI
	MOVQ	$0, DX
	MOVQ	$0, R10

	// Copy mp, gp, fn off parent stack for use by child.
	// Careful: Linux system call clobbers CX and R11.
	MOVQ	mp+16(FP), R8	//mp保存到R8
	MOVQ	gp+24(FP), R9	//gp保存到R9
	MOVQ	fn+32(FP), R12	//fn保存到R12
	...
	MOVQ	SI, SP  // 上方的JEQ 3(PC)指令会跳转到这里,这里是设置栈顶为g0的栈底
	...
	LEAQ	m_tls(R8), DI
	CALL	runtime·settls(SB)  // 设置新线程本地存储

	// In child, set up new stack
	get_tls(CX)
	MOVQ	R8, g_m(R9) // g.m = 新创建的m
...
#endif
	MOVQ	DI, SI	//设置m0->tls+8地址处为fs段寄存器的地址
...
TEXT runtime·sched_getaffinity(SB),NOSPLIT,$0
	MOVQ	pid+0(FP), DI	//第一个参数保存到rdi
	MOVQ	len+8(FP), SI	//第二个参数保存到rsi
	MOVQ	buf+16(FP), DX	//第三个参数保存到rdx
...
TEXT ·uname(SB),NOSPLIT,$0-16
	MOVQ    utsname+0(FP), DI	//获取第一个参数保存到DI寄存器中
	MOVL    $SYS_uname, AX
	SYSCALL
	MOVQ	AX, ret+8(FP)
	RET
