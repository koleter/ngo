...
func gcWaitOnMark(n uint32) {
	for {
		...
		nMarks := atomic.Load(&work.cycles)	// 再次获取循环计数
		if gcphase != _GCmark {
		...
		work.sweepWaiters.list.push(getg())	// 将当前的g放到work.sweepWaiters.list的头部
		goparkunlock(&work.sweepWaiters.lock, waitReasonWaitForGCCycle, traceEvGoBlock, 1)		// 当前的g停止运行并调度其他的g,也就是这行应该不会返回
	}
}
...
func gcStart(trigger gcTrigger) {
	...
	mp := acquirem()
	if gp := getg(); gp == mp.g0 || mp.locks > 1 || mp.preemptoff != "" {	// 当前运行在g0上或者工作线程被使用多次或者当前的g不可被抢占
		releasem(mp)
		return
	}
	releasem(mp)
