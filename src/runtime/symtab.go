...
type moduledata struct {
	...
	filetab      []uint32
	findfunctab  uintptr
	minpc, maxpc uintptr	//指向ftab中的第一项与最后一项
...
func findfunc(pc uintptr) funcInfo {
	...
	x := pc - datap.minpc	//距离pc相差的字节数
	b := x / pcbucketsize	//find index of findfunctab
	i := x % pcbucketsize / (pcbucketsize / nsub)	//确定到某个subbucket

	ffb := (*findfuncbucket)(add(unsafe.Pointer(datap.findfunctab), b*unsafe.Sizeof(findfuncbucket{})))	// 函数所在的findfuncbucket
