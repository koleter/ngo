...
type common struct {
	mu          sync.RWMutex        // guards this group of fields		读写锁，仅用于控制本数据内的成员访问
	output      []byte              // Output generated by test or benchmark.	存储当前测试产生的日志，每产生一条日志则追加到该切片中，待测试结束后再一并输出
	w           io.Writer           // For flushToParent.	子测试执行结束需要把产生的日志输送到父测试中的output切片中，传递时需要考虑缩进等格式调整，通过w把日志传递到父测试
	ran         bool                // Test or benchmark (or one of its subtests) was executed.	仅表示是否已执行过。比如，跟据某个规范筛选测试，如果没有测试被匹配到的话，则common.ran为false，表示没有测试运行过
	failed      bool                // Test or benchmark has failed.	如果当前测试执行失败，则置为true
	skipped     bool                // Test of benchmark has been skipped.	标记当前测试是否已跳过
	done        bool                // Test is finished and all subtests have completed.	表示当前测试及其子测试已结束，此状态下再执行Fail()之类的方法标记测试状态会产生panic
	helpers     map[string]struct{} // functions to be skipped when writing file/line info	标记当前为函数为help函数，其中打印的日志，在记录日志时不会显示其文件名及行号
	cleanup     func()              // optional function to be called at the end of the test
	cleanupName string              // Name of the cleanup function.
	cleanupPc   []uintptr           // The stack trace at the point where Cleanup was called.

	chatty     *chattyPrinter // A copy of chattyPrinter, if the chatty flag is set.	对应命令行中的-v参数，默认为false，true则打印更多详细日志
	bench      bool           // Whether the current test is a benchmark.
	finished   bool           // Test function has completed.	如果当前测试结束，则置为true
	hasSub     int32          // Written atomically.	标记当前测试是否包含子测试，当测试使用t.Run()方法启动子测试时，t.hasSub则置为1
	raceErrors int            // Number of races detected during test.	竞态检测错误数
	runner     string         // Function name of tRunner running the test.		执行当前测试的函数名

	parent   *common	// 如果当前测试为子测试，则置为父测试的指针
	level    int       // Nesting depth of test or benchmark.	测试嵌套层数，比如创建子测试时，子测试嵌套层数就会加1
	creator  []uintptr // If level > 0, the stack trace at the point where the parent called t.Run.		测试函数调用栈
	name     string    // Name of test or benchmark.	记录每个测试函数名，比如测试函数 TestAdd(t *testing.T) , 其中t.name即“TestAdd”。测试结束，打印测试结果会用到该成员
	start    time.Time // Time test or benchmark started	记录测试开始的时间
	duration time.Duration		// 记录测试所花费的时间
	barrier  chan bool // To signal parallel subtests they may start.	用于控制父测试和子测试执行的channel，如果测试为Parallel，则会阻塞等待父测试结束后再继续
	signal   chan bool // To signal a test is done.		通知当前测试结束
	sub      []*T      // Queue of subtests to be run in parallel.	子测试列表
...
type testContext struct {
	...
	maxParallel int		// 最大并发数，默认为系统CPU数，可以通过参数 -parallel n 指定
}
