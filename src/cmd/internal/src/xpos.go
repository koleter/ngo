...
type XPos struct {
	index int32	// PosTable.baseList中的index,该值越小表示先编译的该文件,相等表示同一个文件
	lico
}
...
type PosTable struct {
	baseList []*PosBase		// 根据PosBase中的index可以找到对应的PosBase
	indexMap map[*PosBase]int	//根据PosBase可以找到位于baseList的index
