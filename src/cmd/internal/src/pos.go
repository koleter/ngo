...
type PosBase struct {
	...
	filename    string // file name used to open source file, for error messages	relative filename
...
type lico uint32	// 低2位记录是否为statement,忽略之后的低2位,之后的低8位记录column number,右移12位记录的是line number
...
func (x lico) withStmt(stmt uint) lico {
	...
	// 先将最低两位表示statement或者非statement的bit位清零,再或对应的标志
