...
func Flushplist(ctxt *Link, plist *Plist, newprog ProgAlloc, myimportpath string) {
	...
	var curtext *LSym
	var etext *Prog	// 通过Link成员再次串起所有的Prog
	var text []*LSym
	...
	for _, s := range text {
		...
		ctxt.Arch.Assemble(ctxt, s, newprog)	// 真正的编译函数的函数
		if ctxt.Errors > 0 {
			continue
		}
		linkpcln(ctxt, s)
...
// 设置s的size以及数据的类型并将s添加到ctxt的Data中
func (ctxt *Link) Globl(s *LSym, size int64, flag int) {
	...
	s.Set(AttrOnList, true)
	ctxt.Data = append(ctxt.Data, s)	// 添加s到ctxt的Data字段中
	s.Size = size
	if s.Type == 0 {
		s.Type = objabi.SBSS
	}
