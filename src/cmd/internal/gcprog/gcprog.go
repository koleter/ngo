...
type Writer struct {
	writeByte func(byte)
	index     int64	// the number of bits written to the bit stream so far
...
func (w *Writer) Repeat(n, c int64) {
	...
	if n < 128 {
		w.byte(0x80 | byte(n))	// n小于128就加上128
	} else {
		w.byte(0x80)	// 0x80开头表示一个大于128的值
		w.varint(n)		// 以特定算法将n写入,最后一个字节小于0x80
	}
	w.varint(c)
	w.index += n * c
}
...
func (w *Writer) varint(x int64) {
	...
	for x >= 0x80 {	// 当x大于0x80时,对x的每7个bit分别加上0x80
		w.byte(byte(0x80 | x))
		x >>= 7
	}
	w.byte(byte(x))
...
func (w *Writer) flushlit() {
	...
	w.byte(byte(w.nb))
	var bits uint8
	for i := 0; i < w.nb; i++ {	// w.b中的数据bit写到w中
		bits |= w.b[i] << uint(i%8)
		if (i+1)%8 == 0 {
			if w.debug != nil {
