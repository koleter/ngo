...
type Sym struct {
	...
	Pkg  *Pkg	// 该Sym所在的package
	Name string // object name

	// saved and restored by dcopy
	Def        *Node    // definition: ONAME OTYPE OPACK or OLITERAL	Node that declares symbol
	Block      int32    // blocknumber to catch redeclaration
	Lastlineno src.XPos // last declaration for diagnostic

	flags   bitset8
	Label   *Node // corresponding label (ephemeral)	Label所代表的那个Node节点，当前的Node必须是一个Label标签该字段才会有效
	Origpkg *Pkg  // original package for . import   当通过. 导入了某个包后，这个包中的所有Sym的该字段都会指向被导入的Pkg
}
