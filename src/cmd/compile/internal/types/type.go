...
const (
	...
	TINT
	TUINT
	TUINTPTR // unsafe uintptr  能够存储指针值的无符号整数
	...
	TCHAN
	TMAP
	TINTER //implemented as 2 pointers
	TFORW  // 未计算的类型
	TANY
	TSTRING
	TUNSAFEPTR // unsafePointer
...
const (
	...
	Crecv ChanDir = 1 << 0 // 表示可以从Chan接收数据
	Csend ChanDir = 1 << 1 // 表示可以向Chan输入数据
...
var Types [NTYPE]*Type // Etype与Type结构体的对应关系
...
type Type struct {
	...
	// TARRAY: *Array
	// TSLICE: Slice
	Extra interface{} // 某些类型可能需要记录一些额外的信息

	// Width is the width of this Type in bytes.
	Width int64 // valid if Align > 0

	methods    Fields // 代表method的fields，是通过自己的类型声明定义的所有函数
	allMethods Fields // 当前类型所有可以调用的函数

	Nod  *Node // canonical OTYPE node	记录代表此类型的Node节点,这个Node记录了类型的符号,代表这个类型的OTYPE节点
...
type Forward struct {
	Copyto      []*Type  // where to copy the eventual value to		存储其他的通过当前类型进行type声明的类型
...
type Func struct {
	Receiver *Type // function receiver		Receiver,Results,Params都是TSTRUCT类型
	Results  *Type // function results
	Params   *Type // function params

	Nname *Node // 函数名包装后的Node节点
	pkg   *Pkg
	...
	Outnamed bool // 函数声明时明确定义了返回的results的变量名的数量大于0
}
...
type Struct struct {
	...
	Funarg Funarg // type of function arguments for arg struct	该结构体记录了一个函数recv或参数或返回值参数,该成员记录了该参数的类型
}
...
type Field struct {
	...
	Embedded uint8 // embedded field	1代表是嵌入的类型,0代表不是，函数字段都属于嵌入的类型，2表示类型中的某个函数调用时需要用到指针，如a.b.test(),b是一个指针类型

	Pos  src.XPos
	Sym  *Sym   // 保存成员变量名，如果是嵌入的结构体类型，那么就是嵌入的结构体字面量
	Type *Type  // field type
	Note string // literal string annotation	tag

	// For fields that represent function parameters, Nname points
	// to the associated ONAME Node.	// 如果这个成员不为nil,表示存在定义了名字的返回参数变量
	Nname *Node
...
const (
	...
	fieldNointerface             // method上有Nointerface这个行指令时为true
)
...
func NewArray(elem *Type, bound int64) *Type {
	...
	t.SetNotInHeap(elem.NotInHeap()) // 如果内部元素不能在堆上申请内存,那么该数组也不在堆上申请内存
	return t
}
...
func (t *Type) HasPointers() bool {
	...
	case TPTR, TSLICE:
		return !t.Elem().NotInHeap() // 没有标记为NotInHeap就返回true，就可以分配在堆上
...
var (
	...
	TypeMem     = newSSA("mem")
	TypeFlags   = newSSA("flags") // 表示会修改flag寄存器
