...
func (p *parser) advance(followlist ...token) {
	...
	if len(followlist) > 0 {
		if p.fnest > 0 { // 函数嵌套了多层,单独全局函数的fnest为1
			followset |= stopset // 函数中遇到的关键字就会返回的情况
		}
		for _, tok := range followlist {
			followset |= 1 << tok // followlist中的关键词也添加到followset中
		}
	}

	for !contains(followset, p.tok) { // 找到了followset中的token
		if trace {
...
func (p *parser) varDecl(group *Group) Decl {
	...
	d.NameList = p.nameList(p.name())
	if p.gotAssign() { // var a, b = expr, expr
		d.Values = p.exprList()
	} else { // var a, b Type = expr, expr
		d.Type = p.type_()
		if p.gotAssign() {
			d.Values = p.exprList()
		}
...
func (p *parser) funcDeclOrNil() *FuncDecl {
	...
	if p.tok == _Lparen { // func (receiver) methodName(){}
		rcvr := p.paramList()
		switch len(rcvr) {
		case 0:
...
func (p *parser) funcBody() *BlockStmt {
	p.fnest++               // 函数嵌套级别加一
	errcnt := p.errcnt      // 记录当前发现的编译错误的数量
	body := p.blockStmt("") // 解析函数体
	p.fnest--
	...
	if p.mode&CheckBranches != 0 && errcnt == p.errcnt { // 当前函数体无编译错误
		checkBranches(body, p.errh) // 检查分支
	}
...
func (p *parser) binaryExpr(prec int) Expr {
	...
	x := p.unaryExpr()
	for (p.tok == _Operator || p.tok == _Star) && p.prec > prec { // 组合表达式,当前符号的优先级高于上一个符号
		t := new(Operation)
		t.pos = p.pos()
		t.Op = p.op
		t.X = x
...
func (p *parser) unaryExpr() Expr {
	...
	switch p.tok { //这个switch处理表达式最左边可能出现的运算符
	case _Operator, _Star: //此处应是一元表达式
		switch p.op {
		...
		if _, ok := x.(*ChanType); ok {
			...
			for dir == SendOnly {
				...
				if dir == RecvOnly {
					...
					// (report same error as for "type _ <-<-chan E")   <-chan <-chan也是不允许的
...
func (p *parser) callStmt() *CallStmt {
	...
	if t := unparen(x); t != x {   // 为true表示go或者defer之后跟的函数表达式被包含在一个()中
...
func (p *parser) operand(keep_parens bool) Expr { //keep_parens表示是否处于()或者{}中
	if trace {
...
func (p *parser) pexpr(keep_parens bool) Expr {
	...
	x := p.operand(keep_parens) // 获取除运算符之外的可以在expr文法最前面的操作符
...
loop:
	for {
		...
		case _Lbrack:
			...
			t.pos = pos
			t.X = x
			t.Index[0] = i //如果i只是一个最初的Expr，也表示x[:n]
			p.want(_Colon)
			if p.tok != _Colon && p.tok != _Rbrack {
				// x[i:j...
				t.Index[1] = p.expr()
			}
			if p.tok == _Colon || p.tok == _Semi {
				t.Full = true
				// x[i:j:...]
				/*if t.Index[1] == nil {
				...
				if p.tok == _Semi {
					t.Extra = true // 以分号隔开表示第三个值代表切片的步进
				}
				p.next()
				if p.tok != _Rbrack {
					// x[i:j:k...
...
func (p *parser) funcResult() []*Field {
	...
	if p.tok == _Lparen { //函数用()包裹的返回值的定义,是一个返回列表
		return p.paramList()
	}
...
func (p *parser) addField(styp *StructType, pos Pos, name *Name, typ Expr, tag *BasicLit) {
	if tag != nil {
		for i := len(styp.FieldList) - len(styp.TagList); i > 0; i-- { // 为之前的未应用tag的field添加空的tag
			styp.TagList = append(styp.TagList, nil)
		}
		styp.TagList = append(styp.TagList, tag) // 添加当前的tag
	}
...
func (p *parser) fieldDecl(styp *StructType) {
	...
	case _Name:
		name := p.name()
		if p.tok == _Dot || p.tok == _Literal || p.tok == _Semi || p.tok == _Rbrace { //这里判断的是struct字面量定义的是匿名成员或者内嵌结构体的情况
			// embed oliteral
			typ := p.qualifiedName(name) // 同时处理了p.tok == _Dot时的情况
			tag := p.oliteral()
		...
		names := p.nameList(name) // 在结构体中，可以存在a,b int这样声明成员的情况，此时需要将a,b整理成names
		typ := p.type_()          // 成员的类型
		tag := p.oliteral()       //定义成员以及类型后可以再跟一个字面量
...
func (p *parser) methodDecl() *Field {
	...
	case _Name:
		...
		f := new(Field)
		f.pos = name.Pos()
		if p.tok != _Lparen { // interface中嵌入了另一个interface类型，embedded interface
			// packname
...
func (p *parser) paramList() (list []*Field) { //这里可能有多个变量用同一个类型进行声明的情况,如a,b,c int64
	if trace {
	...
	if named == 0 { // named为0表示可能存在没有定义变量名的匿名类型，未必是都没有
		...
		ok := true
		var typ Expr                          // 临时记录类型
		for i := len(list) - 1; i >= 0; i-- { // 从后向前遍历list
			if par := list[i]; par.Type != nil {
				...
				par.Type = typ // 为par更新类型
			} else {
				// par.Type == nil && typ == nil => we only have a par.Name   这里表示最后的param没有声明类型
				ok = false
				t := p.badExpr()
...
func (p *parser) simpleStmt(lhs Expr, keyword token) SimpleStmt {
	...
	// 不是多个表达式，也不是赋值以及:=声明走下面的语句块,这也就是说，只有=以及:=才可以同时操作多个expr
...
func (p *parser) header(keyword token) (init SimpleStmt, cond Expr, post SimpleStmt) {
	...
	if p.tok == _Lbrace { // switch{}是允许的,但是if{}是不行的
		if keyword == _If {
	...
	if p.tok != _Semi { // 不为；表示存在head
		// accept potential varDecl but complain
		if p.got(_Var) { //switch var
	...
	if p.tok != _Lbrace {
		if p.tok == _Semi { // init之后为分号，表示存在cond
			semi.pos = p.pos()
			semi.lit = p.lit
			p.next()
		} else {
			...
			p.want(_Lbrace)
			if p.tok != _Lbrace { // 这里表示不是两个{{同时存在的情况
				p.advance(_Lbrace, _Rbrace) // for better synchronization (e.g., issue #22581)	消耗token直到找到_Lbrace或者_Rbrace
			}
		}
		if keyword == _For {
			if p.tok != _Semi {
				if p.tok == _Lbrace { // for init;cond{
			...
			p.want(_Semi)
			if p.tok != _Lbrace { // 表示存在post
				post = p.simpleStmt(nil, 0 /* range not permitted */)
				if a, _ := post.(*AssignStmt); a != nil && a.Op == Def { // post中出现了lhs:=rhs的情况就报错
					p.syntaxErrorAt(a.Pos(), "cannot declare in post statement of for loop")
				}
			}
		} else if p.tok != _Lbrace { // switch init;cond
			condStmt = p.simpleStmt(nil, keyword)
		}
	} else { // for cond{}  or  switch cond{}
		condStmt = init
		init = nil
	}
	...
	case *ExprStmt: // 如果是ExprStmt，cond取原来的表达式，判断都属于ExprStmt
		cond = s.X
	default:
...
func (p *parser) stmtOrNil() Stmt {
	...
	if p.tok == _Name {
		p.clearPragma()
		lhs := p.exprList()
		if label, ok := lhs.(*Name); ok && p.tok == _Colon { //label: 用于goto时的跳转
	...
	case _Break, _Continue:
		...
		s.Tok = p.tok
		p.next()
		if p.tok == _Name { // 表示出现了break label或者continue label
			s.Label = p.name()
		}
		return s
	...
	case _Goto:
		...
		s.Tok = _Goto
		p.next()
		s.Label = p.name() // 这里可以看出goto后面必须跟一个label
		return s
	...
	case _Return:
		...
		s.pos = p.pos()
		p.next()
		if p.tok != _Semi && p.tok != _Rbrace { // 这里只有一个return表示返回参数列表已经定义
			s.Results = p.exprList()
		}
		return s

	case _Semi: // 在词法分析过程中会将换行跳过，出现EOF的话直接结束了decl循环，所以这里貌似不会进入
		s := new(EmptyStmt)
		s.pos = p.pos()
		return s
	}
...
func (p *parser) stmtList() (l []Stmt) {
	...
	// case与default出现之后表示为另一个Statement
...
// The first name must be provided.    处理用一个类型声明多个成员的情况
