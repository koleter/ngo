...
type source struct {
	...
	// r是token右边的下标,b是token的起始下标,e是buf的有效数据的右边界
...
redo:
	...
	if s.ch = rune(s.buf[s.r]); s.ch < sentinel {
		s.r++
		s.chw = 1      // 宽度为一的ascii字符
		if s.ch == 0 { // 读取到了作为字符串结尾的0
...
func (s *source) fill() {
	...
	b := s.r // 记录读取的当前的单词的index
	if s.b >= 0 {
		b = s.b
		s.b = 0 // after buffer has grown or content has been moved down
	}
	content := s.buf[b:s.e] //获取s.r到s.e之间的buffer
	...
	if len(content)*2 > len(s.buf) {
		...
		copy(s.buf, content) // 将content拷贝到s.buf的起始位置
	}
	s.r -= b // 重新设置已读取内容的右边界
	s.e -= b
