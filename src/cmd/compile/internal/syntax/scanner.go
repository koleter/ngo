...
type scanner struct {
	source
	mode   uint
	nlsemi bool // if set '\n' and EOF translate to ';'		为false时在读取token时会将\n视为空格跳过    读取到不是break, continue, fallthrough, return这些关键词时会设置为false

	// current token, valid after calling next()
	line, col uint
	blank     bool // line is blank up to col	表示之后是一个新行
	tok       token
...
redo:
	...
	// token start
	s.line, s.col = s.pos()
	s.blank = s.line > startLine || startCol == colbase // 是否读取到了换行
	s.start()
...
func (s *scanner) ident() {
	...
	// general case
	if s.ch >= utf8.RuneSelf { //处理indent中存在多字节utf-8字符的情况
	...
	// possibly a keyword
	lit := s.segment() // 获取读取到的一个单词
	if len(lit) >= 2 {
		if tok := keywordMap[hash(lit)]; tok != 0 && tokStrFast(tok) == string(lit) { // 判断是否为一个关键词
			s.nlsemi = contains(1<<_Break|1<<_Continue|1<<_Fallthrough|1<<_Return, tok) // 判断是否为break, continue, fallthrough或者return这些关键词中的一个
			s.tok = tok
			return
		}
	}
...
func (s *scanner) escape(quote rune) bool {
	var n int            // n代表escape后面跟着的符号数量，例如\x后面跟着2个符号
	var base, max uint32 // 用于数字，base代表进制，max代表最大值
