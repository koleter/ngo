...
func ssaMarkMoves(s *gc.SSAGenState, b *ssa.Block) {
	flive := b.FlagsLiveAtEnd // b块结尾时flag是否活跃或者控制语句中是否有flag类型的value
	...
	for i := len(b.Values) - 1; i >= 0; i-- { // 倒序遍历values
		...
		if v.Type.IsFlags() { // 之前出现的代码flag不活跃了
			flive = false
		}
...
func ssaGenValue(s *gc.SSAGenState, v *ssa.Value) {
	...
	case ssa.OpAMD64ADDQ, ssa.OpAMD64ADDL:
		...
		case r == r2:
			...
			p.To.Type = obj.TYPE_REG
			p.To.Reg = r
		default:	// 编译为lea r, dword ptr ds:[r1 + r2]
			var asm obj.As
	...
	case ssa.OpAMD64LEAQ1, ssa.OpAMD64LEAQ2, ssa.OpAMD64LEAQ4, ssa.OpAMD64LEAQ8,
		...
		gc.AddAux(&p.From, v)
	case ssa.OpAMD64LEAQ, ssa.OpAMD64LEAL, ssa.OpAMD64LEAW: // 取内存地址保存到寄存器中的操作
		p := s.Prog(v.Op.Asm())
		p.From.Type = obj.TYPE_MEM
		p.From.Reg = v.Args[0].Reg() // 为v.Args[0]分配的寄存器的objNum
		gc.AddAux(&p.From, v)        // 整理v中Aux与AuxInt的偏移值到p.From.Offset上
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg()
...
func ssaGenBlock(s *gc.SSAGenState, b, next *ssa.Block) {
	switch b.Kind {
	case ssa.BlockPlain:
		if b.Succs[0].Block() != next { // 后继块不是next块,生成jmp指令
			p := s.Prog(obj.AJMP)
	...
	case ssa.BlockDefer:
		...
		p := s.Prog(x86.ATESTL) // test rax, rax    ???????
		p.From.Type = obj.TYPE_REG
		p.From.Reg = x86.REG_AX
		p.To.Type = obj.TYPE_REG
		p.To.Reg = x86.REG_AX
		p = s.Prog(x86.AJNE) // rax不为0跳转
		p.To.Type = obj.TYPE_BRANCH
