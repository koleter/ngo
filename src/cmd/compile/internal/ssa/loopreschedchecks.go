...
type rewriteTarget struct {
	v *Value	// v的第i个参数是rewrite.before
	i int
}

type rewrite struct {
	before, after *Value          // before is the expected value before rewrite, after is the new value installed.
	rewrites      []rewriteTarget // all the targets for this rewrite.		rewriteTarget中v的第i个参数是rewrite.before
}
...
func insertLoopReschedChecks(f *Func) {
	...
	backedges := backedges(f)	// 深度优先的顺序获取函数从entry块开始的所有的后继边
	if len(backedges) == 0 { // no backedges means no rescheduling checks.
		return
	}

	lastMems := findLastMems(f)	// 块ID对应该块最后的memory类型的value,一般是一个store,但是如果该块中没有store且有memory类型的phi时,就对应这个phi

	idom := f.Idom()
	po := f.postorder()	// 获取倒序遍历的块
	...
	for i := len(po) - 1; i >= 0; i-- {
		b := po[i]
		mem := lastMems[b.ID]	// 获取块的最后的内存状态value
		for j := 0; mem == nil; j++ { // if there's no def, then there's no phi, so the visible mem is identical in all predecessors.
			...
			mem = memDefsAtBlockEnds[b.Preds[j].b.ID]	// 令mem为前驱的块的memory状态
		}
		memDefsAtBlockEnds[b.ID] = mem	// 记录块最后的memory状态
		if f.pass.debug > 2 {
	...
	for i, emc := range tofixBackedges {
		...
		for _, v := range h.Values {	// 尝试找出phi memory value保存到headerMemPhi中
		...
		if headerMemPhi == nil {
			...
			mem0 := memDefsAtBlockEnds[idom[h.ID].ID]	// 获取最近支配节点的memory状态
			headerMemPhi = newPhiFor(h, mem0)	// 在h块中创建一个phi value,并添加相同的h块前驱数量的mem0参数
			newmemphis[h] = rewrite{before: mem0, after: headerMemPhi}
			addDFphis(mem0, h, h, f, memDefsAtBlockEnds, newmemphis, sdom)	// addDFphis函数中memDefsAtBlockEnds[h.ID]不可能等于mem0,所以这个函数调用貌似没有必要

		}
		tofixBackedges[i].m = headerMemPhi	// 记录Memphi
	...
	rewriteNewPhis(f.Entry, f.Entry, f, memDefsAtBlockEnds, newmemphis, dfPhiTargets, sdom)	// 整理每个块的被重写的phi到newmemphis[block]中的rewrites数组中
	...
	for _, emc := range tofixBackedges {
		e := emc.e	// 获取边
		headerMemPhi := emc.m	// 获取边的抵达块中的phi mem value
		h := e.b	// 块
		i := e.i	// e的反向边索引
		p := h.Preds[i]	// 对应的反向前驱边
		bb := p.b	// 前驱块
		mem0 := headerMemPhi.Args[i]	// 从前驱块中传递下来的mem参数
		// bb e->p h,
...
// requirement.
/*
defsForUses: For each block, the mem def seen at its bottom. Could be from earlier block.
newphis:	block对应的rewrite结构体
dfPhiTargets:	dfPhiTargets notes inputs to phis in dominance frontiers that should not be rewritten as part of the dominated children of some outer rewrite.
sdom: 支配树
*/
func rewriteNewPhis(h, b *Block, f *Func, defsForUses []*Value, newphis map[*Block]rewrite, dfPhiTargets map[rewriteTarget]bool, sdom SparseTree) {
	...
	change := newphis[h]	// 获取h中的newphi
	x := change.before	// h之前的phi
	y := change.after	// 现在插入到h块中的phi
	...
	if x != nil { // don't waste time on the common case of no definition.
		...
		for _, v := range b.Values {
			...
			for i, w := range v.Args {
				...
				*p = append(*p, tgt)	// 将tgt添加到change.rewrites中
				if f.pass.debug > 1 {
		...
		if dfu := defsForUses[b.ID]; dfu != nil && dfu.Block != b {	// b块中的最后的mem变量不是在b块中定义的,也就是说从前驱块传递下来的,那么b的后继块也有可能有旧的被重写的mem value
			for _, e := range b.Succs {
				s := e.b	// b的后继块

				for _, v := range s.Values {
					if v.Op == OpPhi && v.Args[e.i] == x {	// x是s块传进来的value
	...
	for c := sdom[b.ID].child; c != nil; c = sdom[c.ID].sibling {	// 继续深度遍历处理b块的孩子节点
...
func addDFphis(x *Value, h, b *Block, f *Func, defForUses []*Value, newphis map[*Block]rewrite, sdom SparseTree) {
	oldv := defForUses[b.ID]	// 获取b块的最后的memory状态
...
func findLastMems(f *Func) []*Value {
	...
	var stores []*Value	// 存储一个块中非phi的memory类型的value
	lastMems := make([]*Value, f.NumBlocks())	// 记录一个块最后的memory类型的value
	storeUse := f.newSparseSet(f.NumValues())	// 记录当前块中的哪些memory类型的value ID是被store使用的
	...
	for _, b := range f.Blocks {
		...
		storeUse.clear()
		stores = stores[:0]
		var memPhi *Value	// 记录当前块中的memory类型的phi
		for _, v := range b.Values {
			if v.Op == OpPhi {
				if v.Type.IsMemory() {
					memPhi = v
			...
			if v.Type.IsMemory() {	// 非phi的memory类型的value
				stores = append(stores, v)	// 保存到stores中
				for _, a := range v.Args {
					if a.Block == b && a.Type.IsMemory() {	// 使用了相同块中的memory类型的value作为参数
						storeUse.add(a.ID)
					}
				}
			}
		}
		if len(stores) == 0 {	// 没有store操作
			lastMems[b.ID] = memPhi		// 当前块的最后的memory类型就是这个memory类型的phi
			continue
		}
		...
		lastMems[b.ID] = last	// 记录b块的最后一个store是last
	}
	return lastMems
}
...
func backedges(f *Func) []Edge {
	...
	for len(stack) > 0 {
		l := len(stack)
		x := stack[l-1]	// 获取栈的最后一项
		if x.i < len(x.b.Succs) {
			// 表示存在未遍历的后继边
			e := x.b.Succs[x.i]	// 获取后继边
			stack[l-1].i++
			s := e.b	// 获取后继块
			if mark[s.ID] == notFound {
				mark[s.ID] = notExplored
				stack = append(stack, backedgesState{s, 0})	// 加入栈以待遍历
			} else if mark[s.ID] == notExplored {
				edges = append(edges, e)	// 将后继边添加到edges中
			}
		} else {
			// 表示x.b的后继边已经遍历完毕
			mark[x.b.ID] = done
			stack = stack[0 : l-1]	// 将当前块从栈中弹出
		}
	}
	return edges
}
