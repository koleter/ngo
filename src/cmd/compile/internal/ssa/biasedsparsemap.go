...
type biasedSparseMap struct {
	s     *sparseMap	// 该数组的长度是一个函数中所有的块最后一行减去第一行的差再加一
	first int	// 起始行号
}
...
func (s *biasedSparseMap) getEntry(i int) (x uint, v int32) {
	sparseEntry := s.s.contents()[i]
	x = uint(int(sparseEntry.key) + s.first)
	v = sparseEntry.val
	return
}
...
// add inserts x->v into s, provided that x is in the range of keys stored in s.	x是行号,v表示函数的第几个块
