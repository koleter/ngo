...
type debugState struct {
	// See FuncDebug.
	slots    []LocalSlot // 存储当前函数中的所有栈槽
	vars     []GCNode    // 下面的varParts成员的key,存储了所有的slots中的变量节点
	varSlots [][]SlotID  // vars中的索引(varID)对应varParts[vars[i]]
	lists    [][]byte

	// The user variable that each slot rolls up to, indexed by SlotID.
	slotVars []VarID // slotID对应varID
	...
	valueNames [][]SlotID // vid对应存储这个value的所有SlotID
	...
	varParts           map[GCNode][]SlotID // SlotID是func.Names的索引
...
func (state *debugState) initializeCache(f *Func, numVars, numSlots int) {
	...
	numPieces := 0 // 记录state.varSlots中的所有元素数量
...
func BuildFuncDebug(ctxt *obj.Link, f *Func, loggingEnabled bool, stackOffset func(LocalSlot) int32) *FuncDebug {
	...
	if state.varParts == nil { // 初始化state.varParts
	...
	for i, slot := range f.Names { // 遍历所有的栈槽
		...
		topSlot := &slot
		for topSlot.SplitOf != nil { // 找到最初的slot
	...
	for varID, n := range state.vars { // 遍历slots中的节点
		parts := state.varParts[n] // 获取变量存储的SlotID
		state.varSlots[varID] = parts
		for _, slotID := range parts { // 遍历slotID
	...
	for i, slot := range f.Names { // 遍历所有的栈槽
		...
		for _, value := range f.NamedValues[slot] { // 遍历栈槽存储的所有values
