...
func zcse(f *Func) {
	...
	for _, b := range f.Blocks {	// 对于不需要参数的value
		for i := 0; i < len(b.Values); i++ {
			v := b.Values[i]
			if opcodeTable[v.Op].argLen == 0 {	// value的操作Op不需要参数
				key := vkey{v.Op, keyFor(v), v.Aux, v.Type}
				if vals[key] == nil {	// 第一次发现的不需要参数的value,将其放到Entry块中
					vals[key] = v
					if b != f.Entry {
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			for i, a := range v.Args {
				if opcodeTable[a.Op].argLen == 0 {	// 某个value中的参数的Op不需要参数
					key := vkey{a.Op, keyFor(a), a.Aux, a.Type}
					if rv, ok := vals[key]; ok {	// ok为true时.说明在entry块中有一个相同的公共子表达式
						v.SetArg(i, rv)	// 设置第i个参数为entry块中的公共子表达式
					}
				}
			}
		}
