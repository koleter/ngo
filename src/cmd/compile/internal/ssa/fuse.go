...
func fuseBlockIf(b *Block) bool {
	...
	var ss0, ss1 *Block	// 表示从不同分支分别到达的ss块
	s0 := b.Succs[0].b	// 第一个后继块
	i0 := b.Succs[0].i	// s0的第i0个前驱是b块
	if s0.Kind != BlockPlain || len(s0.Preds) != 1 || !isEmpty(s0) {	// s0不能被移除,这个判断应该尽量确保这里找到的s0实际上是ss
		/*
		s0.Kind != BlockPlain 表示s0中还有if代码或者s0就是ss而ss可能是个ret块
		len(s0.Preds) != 1 时这里的s0应该是上面的ss
		*/
		s0, ss0 = b, s0	// 块向前推移
	} else {	// 分支s0是基本块且s0只有一个前驱块且s0没有live value
		ss0 = s0.Succs[0].b
		i0 = s0.Succs[0].i
	}
	s1 := b.Succs[1].b	// 第二个分支
	i1 := b.Succs[1].i
	if s1.Kind != BlockPlain || len(s1.Preds) != 1 || !isEmpty(s1) {	// s1不能被移除
		s1, ss1 = b, s1
	} else {	// 分支s1是基本块且s0只有一个前驱块且s0没有live value
		ss1 = s1.Succs[0].b
		i1 = s1.Succs[0].i
	}

	if ss0 != ss1 {	// 两个分支最终应该都到ss
		return false
	}
	ss := ss0
	...
	for _, v := range ss.Values {
		if v.Op == OpPhi && v.Uses > 0 && v.Args[i0] != v.Args[i1] {	// v.Args[i0] != v.Args[i1]说明不同的分支对于该变量有不同的操作,这里要确保phi中的参数是相等的
			return false
		}
	}
	...
	if s0 != b && s1 != b {	// 应该将s0与s1全部干掉
		// Replace edge b->s0->ss with b->ss.
		// We need to keep a slot for Phis corresponding to b.
		b.Succs[0] = Edge{ss, i0}	// 直接连接b到ss
		ss.Preds[i0] = Edge{b, 0}
		b.removeEdge(1)	// 删掉b的第2条后继边,这里应该是删掉了s1
		s1.removeEdge(0)	// 删掉s1的第1条后继边,这里是删掉了s1到ss的边
	} else if s0 != b {	// 清除s0,第三种情况
		b.removeEdge(0)
		s0.removeEdge(0)
	} else if s1 != b {	// 清除s1,第一种情况
		b.removeEdge(1)
		s1.removeEdge(0)
	} else {	// 第四种情况
		b.removeEdge(1)
...
// 如果一个块c只有一个前驱块b且b是基本块,那么将b合并到c
func fuseBlockPlain(b *Block) bool {
	if b.Kind != BlockPlain {
		return false
	}

	c := b.Succs[0].b	// 找到b块的唯一后继块
	if len(c.Preds) != 1 {	// 同时保证b也是c的唯一前驱块
		return false
	}
	...
	if b.Pos.IsStmt() == src.PosIsStmt {
		...
		if l != 0 && c.Pos.Line() == l {	// 后继块没有衔接前驱块的同行statement
	...
	for i, e := range c.Preds {	// 整理边
		p := e.b
		p.Succs[e.i] = Edge{c, i}
	}
	f := b.Func
	if f.Entry == b {	// 如果入口块被合并到c,那么c为新的入口块
		f.Entry = c
	}
