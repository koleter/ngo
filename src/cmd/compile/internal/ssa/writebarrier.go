...
type ZeroRegion struct {
	base *Value	// load操作的value指针
	mask uint64	// if mask & (1<<i) != 0, then [base+i*ptrSize, base+(i+1)*ptrSize)是0
}
...
func needwb(v *Value, zeroes map[ID]ZeroRegion) bool {
	...
	if IsStackAddr(v.Args[0]) {	// 对栈上的内存写入不需要写屏障
	...
	if v.Op == OpStore && IsGlobalAddr(v.Args[1]) {	// 将一个全局变量的地址存到内存
		// Storing pointers to non-heap locations into zeroed memory doesn't need a write barrier.
		ptr := v.Args[0]	// 获取指针,表示将值保存到什么地址
		var off int64	// 对原始指针的偏移
		...
		if off < 0 || off+size > 64*ptrSize {	// 超出track的范围
...
func writebarrier(f *Func) {
	...
	zeroes := f.computeZeroMap()	// 已知的哪些指针空间区域必定为0
	...
	FindSeq:
		...
		flag := b.NewValue2(pos, OpLoad, cfgtypes.UInt32, wbaddr, mem)	// 从writeBarrier的地址中加载4个字节
		flag = b.NewValue2(pos, OpNeq32, cfgtypes.Bool, flag, const0)	// 判断flag是否为常量0
		b.Kind = BlockIf
		b.SetControl(flag)
...
func (f *Func) computeZeroMap() map[ID]ZeroRegion {
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			mem := v.MemoryArg()	// 获取load的内存参数
			if IsNewObject(v, mem) {	// 判断v是将runtime.newobject的结果保存到栈上的操作
				nptr := v.Type.Elem().Size() / ptrSize	// 类型宽度除以指针大小
				if nptr > 64 {	// 限制nptr最大为64
					nptr = 64
				}
	...
	for {
		...
		for _, b := range f.Blocks {
			...
			for _, v := range b.Values {
				...
				z, ok := zeroes[v.MemoryArg().ID]	// 找到存储上面对象的values
				if !ok {
					continue
				}
				ptr := v.Args[0]	// 获取指针
				var off int64	// 实际指针的偏移
				size := v.Aux.(*types.Type).Size()	// 获取类型宽度,即存储内存多少字节
				for ptr.Op == OpOffPtr {	// 指针是一个其他指针的偏移
					off += ptr.AuxInt	// 添加原始指针到当前位置的偏移
					ptr = ptr.Args[0]	// 继续处理上一个指针
				}
				if ptr != z.base {
				...
				for i := min; i < max; i += ptrSize {	// 将min到max之间的bit位清0
					bit := i / ptrSize
					z.mask &^= 1 << uint(bit)
				}
				if z.mask == 0 {
...
func IsReadOnlyGlobalAddr(v *Value) bool {
	...
	if v.Op == OpAddr && v.Aux.(*obj.LSym).Type == objabi.SRODATA {		// 是对一个只读的数据取地址
		return true
	}
	return false
}
...
func IsNewObject(v *Value, mem *Value) bool {
	...
	if !isSameSym(mem.Aux, "runtime.newobject") {		// 上一个影响了内存状态的value是runtime.newobject的函数调用,这里的v应该是load函数调用的结果
		return false
	}
	if v.Args[0].Op != OpOffPtr {	// v.Args[0]的类型应该是一个指针
		return false
	}
	if v.Args[0].Args[0].Op != OpSP {	// runtime.newobject的结果保存到栈上的某处偏移
		return false
	}
