...
type lineRange struct {
	first, last uint32	// 分别记录某个文件的起始行号与结束行号
}
...
type xposmap struct {
	...
	maps map[int32]*biasedSparseMap	// 文件号对应着一个biasedSparseMap指针
...
func (m *xposmap) set(p src.XPos, v int32) {
	s := m.mapFor(p.FileIndex())	// 通过文件index找到对应的biasedSparseMap
	if s == nil {
...
func (m *xposmap) foreachEntry(f func(j int32, l uint, v int32)) {
	for j, mm := range m.maps {
		s := mm.size()
		for i := 0; i < s; i++ {	// 遍历dense数组
			l, v := mm.getEntry(i)	// 获取dense数组第i项对应的行号和块编号
			f(j, l, v)
		}
	}
}
