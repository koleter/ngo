...
func applyRewrite(f *Func, rb blockRewriter, rv valueRewriter) {
	...
	for {
		...
		for _, b := range f.Blocks {
			...
			for i, c := range b.ControlValues() {
				for c.Op == OpCopy {	// 去掉控制Value的所有的外层OpCopy包装
					c = c.Args[0]
			...
			for j, v := range b.Values {
				...
				for i, a := range v.Args {
					...
					if a.Pos.IsStmt() == src.PosIsStmt {
						if aa.Block == a.Block && aa.Pos.Line() == a.Pos.Line() && aa.Pos.IsStmt() != src.PosNotStmt {	// a与aa在相同行
	...
	for _, b := range f.Blocks {
		...
		for i, v := range b.Values {
			vl := v.Pos	// 记录value的pos
			if v.Op == OpInvalid {
				if v.Pos.IsStmt() == src.PosIsStmt {	// 失效的statement
