...
func branchelim(f *Func) {
	...
	po := f.postorder()	// 获取块的后序遍历数组
	for {	// 遍历块中的所有values以及其args,将所有的int或者指针或者UnsafePtr类型的value的ID添加到稀疏集中
		n := loadAddr.size()
		for _, b := range po {
			for i := len(b.Values) - 1; i >= 0; i-- {	// 倒序遍历块的value
				v := b.Values[i]
		...
		if loadAddr.size() == n {	// 遍历所有的values后没有新的成员被添加就跳出循环
			break
		}
	}
...
func elimIf(f *Func, loadAddr *sparseSet, dom *Block) bool {
	...
	// successor of dom.
	if dom.Kind != BlockIf || dom.Likely != BranchUnknown {	// dom为if块且分支方向不明确时向下走
		return false
	}
	var simple, post *Block
	for i := range dom.Succs {	// 遍历后继
		bb, other := dom.Succs[i].Block(), dom.Succs[i^1].Block()	// if块的两个后继块
		// dom
		//  | \
		//  | bb(simple)
		//  | /
	...
	if len(simple.Values) > maxfuseinsts || !canSpeculativelyExecute(simple) {	// 2个以上的values或者存在phi或者simple块中存在有副作用的values就返回false
		return false
	}
	...
	findBlockPos := func(b *Block) bool {	// 用于判断是否有与b块相同行号且为语句标记的value
		pos := b.Pos
		for _, v := range b.Values {
	...
	if simpleStmt {
		...
		if !simpleStmt && simplePos.SameFileAndLine(postPos) {	// simple块中存在与块相同行号且为语句标记的value且simple块与post块的行号相同
			postStmt = false
		}
	...
	if postStmt {
		if dom.Pos.IsStmt() != src.PosIsStmt {
			dom.Pos = postPos	// 设置为post块的位置
		} else {
			// Try the successor block
			if len(dom.Succs) == 1 && len(dom.Succs[0].Block().Preds) == 1 {	// 合并块之后dom的后继只有一个且后继的前驱也只有dom
				succ := dom.Succs[0].Block()	// 获取dom的唯一后继块
...
func shouldElimIfElse(no, yes, post *Block, arch string) bool {
	...
	case "amd64":
		const maxcost = 2
		phi := 0	// post块中的phi的数量
		other := 0	// post的values的参数是在yes块或者no块定义的数量
		for _, v := range post.Values {
			if v.Op == OpPhi {
