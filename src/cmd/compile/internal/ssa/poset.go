...
type posetUndo struct {
	typ  undoType
	idx  uint32	// node的index
	ID   ID		// 此ID如果为0表示为虚拟节点
	edge posetEdge
}
...
//
type poset struct {
	lastidx   uint32            // last generated dense index	用于计数
	flags     uint8             // internal flags		如果为1,将数字视为无符号数
	values    map[ID]uint32     // map SSA values to dense indexes				value ID  ->  nodes的index
	constants map[int64]uint32  // record SSA constants together with their value		int常量值 -> 第几个创建的节点,也就是nodes的index
	nodes     []posetNode       // nodes (in all DAGs)		节点列表
	roots     []uint32          // list of root nodes (forest)		存储的值为节点下标(第几个创建的节点)
...
func (po *poset) upushnew(id ID, idx uint32) {
	po.undo = append(po.undo, posetUndo{typ: undoNewNode, ID: id, idx: idx})	// 取消新建第idx个节点,id指value ID
}
...
func (po *poset) addchild(i1, i2 uint32, strict bool) {
	...
	if i1l == 0 {	// 左边无节点
		po.setchl(i1, e2)	// po.nodes[i1].l = e2
		po.upush(undoSetChl, i1, 0)
	} else if i1r == 0 {	// 右边无节点
		po.setchr(i1, e2)	// po.nodes[i1].r = e2
		po.upush(undoSetChr, i1, 0)
	} else {
		// If i1 already has two children, add an intermediate dummy
		// node to record the relation correctly (without relating
		// i2 to other existing nodes). Use a non-deterministic value
		...
		//      i1r   i2
		//
		dummy := po.newnode(nil)	// 创建虚拟节点
		if (i1^i2)&1 != 0 { // non-deterministic
			po.setchl(dummy, i1r)	// 设置dummy的左节点为i1r
			po.setchr(dummy, e2)	// 设置dummy的右节点为e2
			po.setchr(i1, newedge(dummy, false))	// 设置i1的右子节点为dummy
			po.upush(undoSetChr, i1, i1r)	// 回退,设置po.nodes[i1].r = i1r
		} else {
			po.setchl(dummy, i1l)
			po.setchr(dummy, e2)
...
func (po *poset) newnode(n *Value) uint32 {
	i := po.lastidx + 1
	po.lastidx++
	po.nodes = append(po.nodes, posetNode{})	// 添加一个空节点
	if n != nil {
		if po.values[n.ID] != 0 {
...
func (po *poset) lookup(n *Value) (uint32, bool) {	// 第一个出参表示po.nodes的索引,第二个出参表示之前是否处理过n中的常量
	i, ok := po.values[n.ID]
	if !ok && n.IsGenericIntConst() { // po.values中没有记录且n是一个整数常量
		po.newconst(n)	// 返回之前创建的拥有相同值的节点或者创建一个常量节点并放到森林中
		i, ok = po.values[n.ID]
	}
	return i, ok
}
...
func (po *poset) newconst(n *Value) {
	...
	val := n.AuxInt	// 获取value中的int常量的值
	if po.flags&posetFlagUnsigned != 0 {
		val = int64(n.AuxUnsigned())	// 将value中的int常量的值视为无符号数
	}
	if c, found := po.constants[val]; found {	// 如果常量在之前出现过
		po.values[n.ID] = c	// 将n直接指向之前创建的常量节点索引
		po.upushalias(n.ID, 0)
		return
	}

	// Create the new node for this constant
	i := po.newnode(n)	// i表示创建的第几个节点,是po.nodes中的索引
	...
	if len(po.constants) == 0 {
		idx := len(po.roots)	// 新建常量节点在root中的index
		po.roots = append(po.roots, i)	// 将i存储到root中,表示第i个节点是一个根节点
		po.roots[0], po.roots[idx] = po.roots[idx], po.roots[0]	// 与root中的第一项进行交换,保证roots[0]存储的是常量
		po.upush(undoNewRoot, i, 0)	// 回退对于po.roots = append(po.roots, i)这一步的操作
		po.constants[val] = i
		po.upushconst(i, 0)
		return
	}
	...
	var lowerptr, higherptr uint32	// lowerptr是小边界值节点所在的index,higherptr是大边界值节点所在的index

	if po.flags&posetFlagUnsigned != 0 {
		var lower, higher uint64	// lower是小边界,higher是大边界
		val1 := n.AuxUnsigned()	// 获取n的无符号值
		for val2, ptr := range po.constants {
			val2 := uint64(val2)
			if val1 == val2 {	// 这个判断应该不会为true
				panic("unreachable")
			}
	...
	case higherptr != 0:
		...
		//
		i2 := higherptr
		r2 := po.findroot(i2)	// 找到i2所在的root节点
		if r2 != po.roots[0] { // all constants should be in root #0
			panic("constant not in root #0")
		}
		dummy := po.newnode(nil)
		po.changeroot(r2, dummy)	// 这里其实相当于po.roots[0] = dummy
		po.upush(undoChangeRoot, dummy, newedge(r2, false))
		po.addchild(dummy, r2, false)	// 记录原root为dummy的子节点
		po.addchild(dummy, i, false)	// 记录n为dummy的子节点
		po.addchild(i, i2, true)	// 记录higher为n的子节点
	}
...
// aliasnodes takes care of rearranging the DAG, changing references of parentID/children
...
func (po *poset) aliasnodes(n1 *Value, i2s bitset) {
	i1 := po.values[n1.ID]	// 获取n1在nodes中的下标
	if i1 == 0 {
	...
	// Go through all the nodes to adjust parentID/chidlren of nodes in i2s
	for idx, n := range po.nodes {
		...
		if i2s.Test(l.Target()) {
			po.setchl(uint32(idx), newedge(i1, l.Strict()))	// 将n的左节点替换为i1
			po.upush(undoSetChl, uint32(idx), l)
		}
		if i2s.Test(r.Target()) {
			po.setchr(uint32(idx), newedge(i1, r.Strict()))	// 将n的右节点替换为i1
		...
		if i2s.Test(uint32(idx)) {	// n节点也在替换列表中
			if l != 0 && !i2s.Test(l.Target()) {	// n的左节点不在替换列表中
				po.addchild(i1, l.Target(), l.Strict())	// 左节点作为i1的子节点
			}
			if r != 0 && !i2s.Test(r.Target()) {	// n的右节点不在替换列表中
				po.addchild(i1, r.Target(), r.Strict())	// 右节点作为i1的子节点
			}
			po.setchl(uint32(idx), 0)	// n节点的左右子节点设置为空
...
func (po *poset) dfs(r uint32, strict bool, f func(i uint32) bool) bool {
	...
	if strict {	// strict为true,排除前面包含等于的边(A<=B<=C)并找到所有的非strict节点保存在open中
		...
		next := make([]uint32, 0, 64)	// 保存所有的非strict节点

		for len(open) > 0 {
			i := open[len(open)-1]	// 从open中弹出一项
			...
			// same node.
			if !closed.Test(i) {	// i没有遍历过
				closed.Set(i)	// 标记i已被遍历
...
func (po *poset) reaches(i1, i2 uint32, strict bool) bool {
	return po.dfs(i1, strict, func(n uint32) bool {	// 深搜确定是否某个node index等于i2
		return n == i2
	})
}
...
func (po *poset) findroot(i uint32) uint32 {
	...
	for _, r := range po.roots {
		if po.reaches(r, i, false) {	// r可以找到i,说明i在r这个根节点下
			return r
		}
	}
...
func (po *poset) mergeroot(r1, r2 uint32) uint32 {
	...
	if r2 == po.roots[0] {	// 保证r2不是po.root[0],因为r2是要被移除的
		r1, r2 = r2, r1
	}
	r := po.newnode(nil)	// 创建一个虚拟节点
	po.setchl(r, newedge(r1, false))	// 将两个根节点作为子节点挂载到虚拟节点下
	po.setchr(r, newedge(r2, false))
	po.changeroot(r1, r)	// 将roots中的r1替换为r
	po.removeroot(r2)	// 移除r2节点
...
func (po *poset) collapsepath(n1, n2 *Value) bool {
	...
	// (excluding n1 itself)
	paths.Clear(i1)
	po.aliasnodes(n1, paths)	// 将paths中的所有节点替换为n1
	return true
}
...
func (po *poset) findpaths1(cur, dst uint32, seen bitset, path bitset) {
	...
	if path.Test(l) || path.Test(r) {	// 从当前节点的左边或者右边找到了dst,说明当前节点也在路径上
		path.Set(cur)
	}
}
...
// strict为true表示n1与n2之间没有相等关系
func (po *poset) setOrder(n1, n2 *Value, strict bool) bool {
	i1, f1 := po.lookup(n1)
	i2, f2 := po.lookup(n2)

	switch {
	case !f1 && !f2:	// 之前没有处理过两个value中的常量
...
func (po *poset) Undo() {
	...
	for len(po.undo) > 0 {
		pass := po.undo[len(po.undo)-1]	// 从栈中弹出一项
		...
		case undoNewConstant:
			...
			var val int64
			var i uint32
			for val, i = range po.constants {	// 找到po.constants中对应的常量值与index
				if i == pass.idx {
					break
				}
			}
		...
		case undoAliasNode:
			ID, prev := pass.ID, pass.idx	// 获取value所在的index与之前的index
			cur := po.values[ID]	// 现在记录的index
			if prev == 0 {
		...
		case undoNewRoot:
			i := pass.idx	// 获取待回退的根节点所在nodes中的index
			l, r := po.children(i)
			if l|r != 0 {
				panic("non-empty root in undo newroot")
			}
			po.removeroot(i)	// 将i从roots中移除

		case undoChangeRoot:
			i := pass.idx	// 获取新的根节点索引
			l, r := po.children(i)	// 获取左右子节点索引
			if l|r != 0 {
				panic("non-empty root in undo changeroot")
			}
			po.changeroot(i, pass.edge.Target())	// 将根节点还原

		case undoMergeRoot:
			i := pass.idx	// 获取merge之后的根节点索引
			l, r := po.children(i)	// 获取两个子节点
			po.changeroot(i, l.Target())	// 将根节点还原
			po.roots = append(po.roots, r.Target())	// 添加原来被删除的根节点
