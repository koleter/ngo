...
type Func struct {
	Config *Config     // architecture information		ssaConfig
	Cache  *Cache      // re-usable cache				&ssaCaches[worker]
	fe     Frontend    // frontend state associated with this Func, callbacks into compiler frontend
	pass   *pass       // current pass information (name, options, etc.)
	Name   string      // e.g. NewFunc or (*Func).NumBlocks (no package prefix)    函数名
	Type   *types.Type // type signature of the function.
	Blocks []*Block    // unordered set of all basic blocks (note: not indexable by ID)
	Entry  *Block      // the entry basic block			starting block
	...
	NoSplit   bool // true if function is marked as nosplit.  Used by schedule check pass.		fn.Func.Pragma&Nosplit != 0

	// when register allocation is done, maps value ids to locations
	RegAlloc []Location	// 这里的Location其实就是Register, 如果是tuple类型的话就是LocPair([]Register),如果是LocalSlot表示对应的value是一个参数(OpArg)

	// map from LocalSlot to set of Values that we want to store in that slot.
	NamedValues map[LocalSlot][]*Value	// 这个value切片可以认为是通过相同的符号进行多次赋值之后生成的多个values
	// Names is a copy of NamedValues.Keys. We keep a separate list
	// of keys to make iteration order deterministic.
	Names []LocalSlot // 所有的栈槽可以通过该字段进行遍历
	...
	cachedPostorder  []*Block   // cached postorder traversal	缓存倒序遍历
	cachedIdom       []*Block   // cached immediate dominators	直接支配树数组
...
func (f *Func) retSparseSet(ss *sparseSet) {
	...
	f.Cache.scrSparseSet = append(f.Cache.scrSparseSet, ss) // f.Cache.scrSparseSet已满就进行扩展
}
...
func (f *Func) newValueNoBlock(op Op, t *types.Type, pos src.XPos) *Value {
	...
	if notStmtBoundary(op) {	// 这个操作永远不会代表语句边界
		pos = pos.WithNotStmt()
	}
	v.Pos = pos
	return v
...
func (f *Func) freeValue(v *Value) {
	...
	if nArgs == 0 || v.Op == OpOffPtr {
		vv := f.constants[v.AuxInt] // 从常量缓存中移除该value
		for i, cv := range vv {
			if v == cv {
				vv[i] = vv[len(vv)-1]
				vv[len(vv)-1] = nil
...
func (f *Func) NewBlock(kind BlockKind) *Block {
	...
	if f.freeBlocks != nil {
		...
		ID := f.bid.get()                  // 每创建一个新块都会将ID自增一
		if int(ID) < len(f.Cache.blocks) { // ID小于缓存的blocks的长度
			b = &f.Cache.blocks[ID] // 依据ID从缓存块中获取block
			b.ID = ID
		} else {
			b = &Block{ID: ID} // 新建一个Block结构体
		}
	}
	b.Kind = kind
	b.Func = f
...
func (f *Func) constVal(op Op, t *types.Type, c int64, setAuxInt bool) *Value {
	...
	vv := f.constants[c]   // 从静态缓存中获取Value数组
	for _, v := range vv { // 遍历数组
		if v.Op == op && v.Type.Compare(t) == types.CMPeq { // 操作码与类型都相同
			if setAuxInt && v.AuxInt != c { // 设置AuxInt但是获取到的AuxInt与传入的c不相同
	...
	var v *Value
	if setAuxInt { // 为true表示要设置AuxInt属性的值为c
...
// postorder returns the reachable blocks in f in a postorder traversal.	倒序遍历块保存到缓存中并返回
