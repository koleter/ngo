...
func nextGoodStatementIndex(v *Value, i int, b *Block) int {
	...
	// first built.
	if i >= len(b.Values)-1 {	// 超出块中所包含的代码数量了
		return i
	}
	...
	for j := i + 1; j < len(b.Values); j++ {
		...
		if u.Pos.SameFileAndLine(v.Pos) {	// u与v在相同文件的相同行
...
func numberLines(f *Func) {
	po := f.Postorder()		// 倒序遍历的所有块
	endlines := make(map[ID]src.XPos)	// 记录块id与块最后一行位置的映射
	ranges := make(map[int]lineRange)
	note := func(p src.XPos) {	// 该函数用于更新一个文件的第一行与最后一行
		line := uint32(p.Line())	// 获取行号
		i := int(p.FileIndex())	// 文件index
		lp, found := ranges[i]
		change := false
		if line < lp.first || !found {
			lp.first = line	// 记录函数的第一行代码的行号
			change = true
		}
		if line > lp.last {
			lp.last = line
	...
	for j := len(po) - 1; j >= 0; j-- {	// po是缓存倒序排序的所有的块,这里循环也倒序遍历,实际上是正序遍历所有的块
		b := po[j]
		// Find the first interesting position and check to see if it differs from any predecessor
		firstPos := src.NoXPos  // 当前块第一行有效的Value所在位置
		firstPosIndex := -1	// first interesting Value index position
		if b.Pos.IsStmt() != src.PosNotStmt {	// 不是空块
			note(b.Pos)	// 记录块的行号信息,块的Pos记录的是这个块的最后一行有效代码的位置
		}
		for i := 0; i < len(b.Values); i++ {	// 遍历当前块的Value
			v := b.Values[i]
			if v.Pos.IsStmt() != src.PosNotStmt {	// 找到不是非statement的value
				note(v.Pos)	// 记录Value的起始行号,因为下面直接跳出循环了,所以不能算是记录最后一行的行号
				// skip ahead to better instruction for this line if possible
				i = nextGoodStatementIndex(v, i, b)	// 找到下一个有效的statememt语句所在的index
				v = b.Values[i]
				firstPosIndex = i
				firstPos = v.Pos
		...
		if firstPosIndex == -1 { // Effectively empty block, check block's own Pos, consider preds.
			line := src.NoXPos	// 通过所有前驱块的最后一行计算
			for _, p := range b.Preds {
				pbi := p.Block().ID // 获取前驱块的ID
				if !endlines[pbi].SameFileAndLine(line) {	// 前驱块最后一行的文件以及行号与line不同
					if line == src.NoXPos {
						line = endlines[pbi]	// 获取前驱块的最后一行赋值给line
						continue
					} else {	// 多个前驱块最后一行所在文件的位置不同
						line = src.NoXPos
						break
					}
			...
			if b.Pos.IsStmt() == src.PosNotStmt {	// 当前块没有statement
				b.Pos = line	// 设置当前块的位置为前驱块的最后一行
				endlines[b.ID] = line	// 当前块的最后一行也是开头的第一行
				continue
			}
			// If the block differs from its predecessors, mark it as a statement
			if line == src.NoXPos || !line.SameFileAndLine(b.Pos) {	// 多个前驱块的最后一行不相同或者前驱块最后一行与当前块的位置不在同一个文件的同一行
				b.Pos = b.Pos.WithIsStmt()  // mark b as a statement
				if f.pass.debug > 0 {
					fmt.Printf("Mark stmt effectively-empty-block %s %s %s\n", f.Name, b, flc(b.Pos))
				}
			}
			endlines[b.ID] = b.Pos  // 设置块的最后一行
			continue
		}
		...
		if len(b.Preds) == 0 { // Don't forget the entry block
			...
			for _, p := range b.Preds {
				pbi := p.Block().ID // 前驱块ID
				if !endlines[pbi].SameFileAndLine(firstPos) {   // 前驱块结尾与当前块第一行不同
					b.Values[firstPosIndex].Pos = firstPos.WithIsStmt()
					if f.pass.debug > 0 {
		...
		for i := firstPosIndex + 1; i < len(b.Values); i++ {
			v := b.Values[i]
			if v.Pos.IsStmt() == src.PosNotStmt {	// 跳过不是statement的语句
				continue
			}
			note(v.Pos)
			...
			v = b.Values[i]
			if !v.Pos.SameFileAndLine(firstPos) {   // 当前的Value位置与上一行有效Value不相同
				if f.pass.debug > 0 {
					fmt.Printf("Mark stmt new line %s %s %s %s prev pos = %s\n", f.Name, b, v, flc(v.Pos), flc(firstPos))
				}
				firstPos = v.Pos	// 更新Position
				v.Pos = v.Pos.WithIsStmt()
			} else {
				v.Pos = v.Pos.WithDefaultStmt()	// 与上一行有效Value相同就设为Default
			}
		}
		if b.Pos.IsStmt() != src.PosNotStmt && !b.Pos.SameFileAndLine(firstPos) {	// 当前块的最后一行statement与Value记录的不同
			if f.pass.debug > 0 {
				fmt.Printf("Mark stmt end of block differs %s %s %s prev pos = %s\n", f.Name, b, flc(b.Pos), flc(firstPos))
			}
			b.Pos = b.Pos.WithIsStmt()	// 必然是statement
			firstPos = b.Pos	// 设置为块的最后一行position
		}
	...
	if f.pass.stats&1 != 0 {
		...
		f.LogStat("SUM_LINE_RANGE", total, "MAXMIN_LNE_RANGE", maxline-minline, "MAXFILE", maxfile, "NFILES", len(entries))
	}
