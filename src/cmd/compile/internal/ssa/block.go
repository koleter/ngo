...
type Block struct {
	...
	Likely BranchPrediction	// 用于分支预测时的判断

	// After flagalloc, records whether flags are live at the end of the block.
	FlagsLiveAtEnd bool

	// Subsequent blocks, if any. The number and order depend on the block kind.
	Succs []Edge	// Succs[0]对应判断为true时的分支,Succs[1]对应判断为false时的分支
	...
	// The containing function	编译中的函数
	Func *Func

	// Storage for Succs, Preds and Values.
	succstorage [2]Edge
	predstorage [4]Edge	// 前驱块合并到该块时该成员会保存前驱块的preds
	valstorage  [9]*Value
}
...
type Edge struct {	// 如果通过某一个块的后继找到该边,那么b.preds[i]就是那个块;如果通过某一个块的前驱找到该边,那么b.succs[i]就是那个块
	...
	//   e := x.Succs[idx]					获取x的第idx条后继边
	//   e.b.Preds[e.i] = Edge{x,idx}		x的第idx条后继边所指向的块的第i条前驱边对应自己这条边
...
func (b *Block) AddEdgeTo(c *Block) {
	i := len(b.Succs)
	j := len(c.Preds)
	b.Succs = append(b.Succs, Edge{c, j})	// b的下一个块是c,c的第j个preds块是b
	c.Preds = append(c.Preds, Edge{b, i}) // c的上一个块是b,b的第i个preds块是b
...
func (b *Block) removeSucc(i int) {
	n := len(b.Succs) - 1
	if i != n {	// 移除的不是最后一条输出边
		e := b.Succs[n]	// 获取最后一条输出边
		b.Succs[i] = e	// 第i条输出边设置为最后一条边
		// Update the other end of the edge we moved.
		e.b.Preds[e.i].i = i	// 调整前驱块与当前块的边索引
	}
	b.Succs[n] = Edge{}
	b.Succs = b.Succs[:n]
	b.Func.invalidateCFG()
...
func (b *Block) LackingPos() bool {
	...
	if b.Pos != src.NoXPos {	// 记录了源码的位置返回false
		return false
	}
	for _, v := range b.Values {	// 遍历所有的ssa.Value
		if v.LackingPos() {
			continue
		}
		return false
