...
func decomposeBuiltIn(f *Func) {
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			decomposeBuiltInPhi(v)	// 将复杂类型的phi拆分重写为简单类型的phi
		}
	}

	// Decompose other values
	applyRewrite(f, rewriteBlockdec, rewriteValuedec)	// rewriteBlockdec什么都不做直接返回false, rewriteValuedec将复杂类型重写为简单的类型
	if f.Config.RegSize == 4 {
		applyRewrite(f, rewriteBlockdec64, rewriteValuedec64)		// rewriteBlockdec什么都不做直接返回false,rewriteValuedec64将64位的操作拆解为32位的等价操作
	}

	// Split up named values into their components.
	var newNames []LocalSlot
	for _, name := range f.Names {	// 以下关于Splitxxx函数定义在ssa.go文件中
		t := name.Type	// 获取类型
		switch {
		case t.IsInteger() && t.Size() > f.Config.RegSize:	// 32位程序,但是却是int64类型
...
func decomposeBuiltInPhi(v *Value) {
	switch {
	case v.Type.IsInteger() && v.Type.Size() > v.Block.Func.Config.RegSize:	// phi的类型为int64但是程序是32位的
		decomposeInt64Phi(v)	// 将int64类型的phi拆成用两个32位的数组成的value
	case v.Type.IsComplex():
		decomposeComplexPhi(v)	// 将复数类型的phi拆成用两个浮点数组成的value
	case v.Type.IsString():
		decomposeStringPhi(v)	// 将phi(string1,string2...)改写为OpStringMake(phi(string1ptr,string2ptr...), phi(string1len,string2len...))
	case v.Type.IsSlice():
		decomposeSlicePhi(v)
...
// 将int64类型的phi拆成用两个32位的数组成的value
...
func decomposeStructPhi(v *Value) {
	t := v.Type
	n := t.NumFields()
	var fields [MaxStruct]*Value	// 结构体中的每个字段都是一个phi函数
	for i := 0; i < n; i++ {	// 为结构体中的每个field创建一个phi
