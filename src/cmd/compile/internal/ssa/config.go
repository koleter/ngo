...
type Config struct {
	...
	gpRegMask      regMask       // general purpose integer register mask	除SP以外的通用寄存器掩码,包括r8到r15
	fpRegMask      regMask       // floating point register mask			xmm0到xmm15共16个寄存器的掩码
	fp32RegMask    regMask       // floating point register mask
	fp64RegMask    regMask       // floating point register mask
	specialRegMask regMask       // special register mask
	GCRegMap       []*Register   // garbage collector register map, by GC register index
	FPReg          int8          // register number of frame pointer, -1 if not used			这个fp寄存器应该就是栈底寄存器rbp
	LinkReg        int8          // register number of link register if it is a general purpose register, -1 if not used	在ARM体系结构中LR的特殊用途有两种：一是用来保存子程序返回地址；二是当异常发生时，LR中保存的值等于异常发生时PC的值减4（或者减2），因此在各种异常模式下可以根据LR的值返回到异常发生前的相应位置继续执行
	hasGReg        bool          // has hardware g register
	ctxt           *obj.Link     // Generic arch information
	optimize       bool          // Do optimization		没有-N参数时为true
	noDuffDevice   bool          // Don't use Duff's device
	useSSE         bool          // Use SSE for non-float operations
	useAvg         bool          // Use optimizations that need Avg* operations
	useHmul        bool          // Use optimizations that need Hmul* operations
	use387         bool          // GO386=387
	SoftFloat      bool          // 用软件模拟出来的float
