...
func cse(f *Func) {
	...
	a := make([]*Value, 0, f.NumValues())	// 存储所有的类型不为memory的value
	if f.auxmap == nil {
		f.auxmap = auxmap{}
	}
	...
	partition := partitionValues(a, f.auxmap)	// 一定方式将粗略的等价Value进行分组

	// map from value id back to eqclass id
	valueEqClass := make([]ID, f.NumValues())	// 对于分为同一组的value,他们的eqclass id是相同的
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			// Use negative equivalence class #s for unique values.
			valueEqClass[v.ID] = -v.ID	// 初始化为v.ID到-v.ID的映射
		}
	}
	var pNum ID = 1
	for _, e := range partition {
		...
		for _, v := range e {	// 对于同一个组中的Value,其具有相同的EqClass ID
	...
	for {
		...
		for i := 0; i < len(partition); i++ {
			...
			if opcodeTable[e[0].Op].commutative {	// 每个组中的value的操作数可交换,因为通过op进行的排序,所以每一组value具有相同的Op
				// Order the first two args before comparison.
				for _, v := range e {
					if valueEqClass[v.Args[0].ID] > valueEqClass[v.Args[1].ID] {	// 保证参数中ID小的Value在前面
			...
			sort.Sort(byArgClass)	// 每个value根据参数的eqClass ID进行从小到大的排序

			// Find split points.
			splitPoints = append(splitPoints[:0], 0)
			for j := 1; j < len(e); j++ {	// 遍历一组等价value
				v, w := e[j-1], e[j]
				// Note: commutative args already correctly ordered by byArgClass.
				eqArgs := true	// 所有的参数是否相等
				for k, a := range v.Args {
					b := w.Args[k]	// w的第k个参数
					if valueEqClass[a.ID] != valueEqClass[b.ID] {	// 判断v.Args[k]与w.Args[k]是否等价
						eqArgs = false
						break
					}
				}
				if !eqArgs {
					splitPoints = append(splitPoints, j)	// 添加切割边界,找到参数都相同的value组
				}
			}
			if len(splitPoints) == 1 {	// 只有最初添加的0
				continue // no splits, leave equivalence class alone.
			}

			// Move another equivalence class down in place of e.
			partition[i] = partition[len(partition)-1]	// 将当前的eqclass从partition中移除
			...
			for j := 0; j < len(splitPoints)-1; j++ {
				f := e[splitPoints[j]:splitPoints[j+1]]	// 分割等价values
				if len(f) == 1 {
					// Don't add singletons.
					valueEqClass[f[0].ID] = -f[0].ID
					continue
				}
				for _, v := range f {	// 新的等价分组
					valueEqClass[v.ID] = pNum
				}
				pNum++
				partition = append(partition, f)	// 新分组添加到partition中
			}
			changed = true
		}
	...
	// 至此,partition中的每部分都是公共表达式
	sdom := f.Sdom()	// 构建稀疏树节点

	// Compute substitutions we would like to do. We substitute v for w
	// if v and w are in the same equivalence class and v dominates w.
	rewrite := make([]*Value, f.NumValues())	// index为待重写的value的ID,值为用哪个value进行重写
	byDom := new(partitionByDom) // reusable partitionByDom to reduce allocs
	for _, e := range partition {	// 遍历每一组等价value
		byDom.a = e
		byDom.sdom = sdom
		sort.Sort(byDom)	// 根据block的访问顺序进行排序,先访问的放在前面
		for i := 0; i < len(e)-1; i++ {
			...
			for j := i + 1; j < len(e); j++ {	// 遍历后面出现的所有value判断是否为公共表达式并将其替换
				w := e[j]
				if w == nil {
					continue
				}
				if sdom.IsAncestorEq(v.Block, w.Block) {	// 判断v.Block是否支配w.Block
					rewrite[w.ID] = v	// 记录以便之后用v重写w
					e[j] = nil	// 被重写的部分设置为nil
				} else {
	...
	rewrites := int64(0)	// 记录重写的次数

	// Apply substitutions
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			for i, w := range v.Args {
				if x := rewrite[w.ID]; x != nil {	// w需要用x重写
					if w.Pos.IsStmt() == src.PosIsStmt {	// 被重写的Value是语句边界
		...
		for i, v := range b.ControlValues() {
			if x := rewrite[v.ID]; x != nil {	// v需要用x重写
				if v.Op == OpNilCheck {
...
func cmpVal(v, w *Value, auxIDs auxmap) types.Cmp {
	...
	if v.Op == OpPhi && v.Block != w.Block {	// phi函数所在的块也要相同
...
func (sv partitionByArgClass) Less(i, j int) bool {
	v := sv.a[i]	// 取出第i项与第j项value
	w := sv.a[j]
	for i, a := range v.Args {
		b := w.Args[i]
		if sv.eqClass[a.ID] < sv.eqClass[b.ID] {	// 第i项参数的eqClass ID小于第j项参数的eqClass ID
			return true
		}
