...
type SparseTreeNode struct {
	child   *Block	// 被当前节点支配的节点
	sibling *Block
	parent  *Block	// 支配当前节点的支配节点
...
// newSparseTree creates a SparseTree from a block-to-parentID map (array indexed by Block.ID)
// parentOf的index表示一个blockID,value为这个blockID代表的块的最近支配节点
func newSparseTree(f *Func, parentOf []*Block) SparseTree {
	t := make(SparseTree, f.NumBlocks())
	for _, b := range f.Blocks {
		n := &t[b.ID]	// 获取b块的最近支配节点所在的块
		if p := parentOf[b.ID]; p != nil {	// 根据blockID获取支配节点
			n.parent = p	// 设置n被p支配
			n.sibling = t[p.ID].child	// 支配节点支配的上一个节点作为当前的兄弟节点,这应该是一个链表,通过当前节点的支配节点找到该节点,并通过该节点的sibling不断遍历所有的被支配节点
			t[p.ID].child = b	// 设置p支配b,需要注意的是循环是正向遍历所有的块,所以这种链表的构造方式会导致后遍历的块成为链表头
		}
	}
...
// newSparseOrderedTree creates a SparseTree from a block-to-parentID map (array indexed by Block.ID)
...
func (t SparseTree) numberBlock(b *Block, n int32) int32 {
	...
	n += 2
	for c := t[b.ID].child; c != nil; c = t[c.ID].sibling {	// 遍历所有被b支配的节点
		// preserves n = next free number	从这里可以看出是深度遍历,对于支配树中处于同一级别的节点来说,头结点的entry与exit小于兄弟节点的entry
		n = t.numberBlock(c, n)
	}
...
// Sibling returns a siblingID of x in the dominator tree (i.e.,
