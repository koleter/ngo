...
func (m *SparseTreeMap) Insert(b *Block, adjust int32, x interface{}, helper *SparseTreeHelper) {
	...
	// sp will be the sparse parentID in this sparse tree (nearest ancestor in the larger tree that is also in this sparse tree)
	...
	// This newly inserted block may now be the sparse parentID of some existing nodes (the new sparse children of this block)
	// Iterate over nodes bracketed by this new node to correct their parentID, but not over the proper sparse descendants of those nodes.
	...
	for tme := d.(*sparseTreeMapEntry); tme != entry; tme = d.(*sparseTreeMapEntry) {
		...
		// next sparse siblingID (or right-bracketing sparse parentID == entry) is first node after tme.index.exit - tme.adjust
...
func (m *SparseTreeMap) findEntry(b *Block, adjust int32, helper *SparseTreeHelper) *sparseTreeMapEntry {
	...
	// The Glb (not EQ) of this probe is either the entry-indexed end of a sparse parentID
	// or the exit-indexed end of a sparse siblingID
	...
	// otherEntry is a sparse Sibling, and shares the same sparse parentID (nearest ancestor within larger tree)
