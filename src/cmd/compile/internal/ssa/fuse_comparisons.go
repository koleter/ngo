...
func fuseIntegerComparisons(b *Block) bool {
	if len(b.Preds) != 1 {	// 前驱块只有一个
		return false
	}
	p := b.Preds[0].Block()		// 获取b的唯一前驱块
	...
	for i, op := range [2]Op{OpOrB, OpAndB} {
		...
		if !canSpeculativelyExecute(b) {	// b块的所有value中不包含phi以及各种带有副作用的value
			return false
		}
...
// 判断x与y是相同的变量分别操作不同的int常量且int常量处于有符号数的正数范围
func areMergeableInequalities(x, y *Value) bool {
	...
	for _, f := range inequalityChecks {
		if !f(x) || !f(y) {		// 保证x与y都是有符号数或者都是无符号数
			continue
		}

		// Check that both inequalities are comparisons with constants.
		xi := getConstIntArgIndex(x)	// 获取x的第一个int常量,不存在的话返回-1
		if xi < 0 {
			return false
		}
		yi := getConstIntArgIndex(y)	// 获取y的第一个int常量,不存在的话返回-1
		if yi < 0 {
			return false
		}
