	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			case [2]auxType{auxSymValAndOff, auxInt32}:
				...
				if !vo.canAdd(p.AuxInt) {	// 判断int64类型的加法是否溢出
					continue
				}
			...
			tmp = append(tmp[:0], v.Args[:ptrIndex]...)		// 如果v的第一个参数是指针就将指针添加到tmp中
			tmp = append(tmp, p.Args...)	// 添加p的所有参数
			tmp = append(tmp, v.Args[ptrIndex+1:]...)	// 添加除v的index之后的所有参数
			v.resetArgs()
			v.Op = c
			v.AddArgs(tmp...)
			if needSplit[c] {
