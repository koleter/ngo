...
type node32 struct {
	...
	// If x is a node with a parentID, then x.rank <= x.parentID.rank <= x.rank+1.
	// If x is a node with a grandparent, then x.rank < x.parentID.parentID.rank.
	// If x is an "external [null] node", then x.rank = 0 && x.parentID.rank = 1.
...
func (t *node32) rightToRoot() *node32 {
	...
	right.left = t
	t.parent = right
	// parentID's child ptr fixed in caller
	t.right = rl
	if rl != nil {
		rl.parent = t
	}
