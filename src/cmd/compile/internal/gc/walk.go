...
func walk(fn *Node) {
	...
	for i, ln := range fn.Func.Dcl {	// 遍历函数显示声明的所有变量
		if ln.Op == ONAME && (ln.Class() == PAUTO || ln.Class() == PAUTOHEAP) {	// 局部变量或者移动到堆中的参数变量
			ln = typecheck(ln, ctxExpr|ctxAssign)	// 重新进行类型检查
			fn.Func.Dcl[i] = ln
		}
	}
	...
	/*for _, ln := range fn.Func.Dcl {
		if ln.Op == ONAME && (ln.Class() == PAUTO || ln.Class() == PAUTOHEAP) && ln.Name.Defn != nil && ln.Name.Defn.Op == OTYPESW && ln.Name.Used() {	// ln是通过type switch得到的变量
			ln.Name.Defn.Left.Name.SetUsed(true)
		}
	}*/

	/*for _, ln := range fn.Func.Dcl {	//处理局部变量的ONAME, 未被使用且没有&的变量, 会报错
...
func walkstmt(n *Node) *Node {
	...
	case OAS,	// Left = Right or (if Colas=true) Left := Right   20
		OASOP,	// Left Etype= Right (x += y)	26
		OAS2,	// List = Rlist (x, y, z = a, b, c)		21
		OAS2DOTTYPE,	// List = Right (x, ok = I.(int))	22
		OAS2RECV,	// List = Right (x, ok = <-c)	25
		OAS2FUNC,	// List = Right (x, y = f())	23
		OAS2MAPR,	// List = Right (x, ok = m["foo"])	24
		OCLOSE,		// close(Left)		33
		OCOPY,		// copy(Left, Right)	44
		OCALLMETH,	// Left(List/Rlist) (direct method call x.Method(args))		29
		OCALLINTER,	// Left(List/Rlist) (interface method call x.Method(args))	30
		OCALL,		// Left(List) (function call, method call or type conversion)	27
		OCALLFUNC,	// Left(List/Rlist) (function call f(args))		28
		ODELETE,	// delete(Left, Right)	50
		OSEND,		// Left <- Right	93
		OPRINT,		// print(List)		90
		OPRINTN,	// println(List)	91
		OPANIC,		// panic(Left)		89
		OEMPTY,		// no-op (empty statement)		117
		ORECOVER,	// recover()	100
		OGETG:		// runtime.getg() (read g pointer)		151
		if n.Typecheck() == 0 {
...
// 如果make函数中创建的是可以转化为数值类型的slice且其数量*类型的宽度小于64k就返回true
...
func convFuncName(from, to *types.Type) (fnname string, needsaddr bool) {
	...
	case 'T':	// from是一个具体的类型
		...
		switch tkind {	// 具体的类型转换为interface需要具体类型的地址
		case 'E':	// to是一个空的interface
			if !from.HasPointers() {
				return "convT2Enoptr", true
			}
			return "convT2E", true	
		case 'I':	// to是一个非空的interface
			if !from.HasPointers() {
...
func walkexpr(n *Node, init *Nodes) *Node {
	...
	if n.Op == ONAME && n.Class() == PAUTOHEAP {	// 移动到堆中的局部变量
		nn := nod(ODEREF, n.Name.Param.Heapaddr, nil)	// 对于堆地址解引用
...
opswitch:
	...
	case ODOTTYPE, ODOTTYPE2:
		...
		if !n.Type.IsInterface() && !n.Left.Type.IsEmptyInterface() {	// 不是空接口强转为其他非空接口的情况
	...
	case OLEN, OCAP:
		...
		t := n.Left.Type	// len或者cap函数中参数的类型
	...
	case OCALLINTER, OCALLFUNC, OCALLMETH:
		...
		if n.Op == OCALLFUNC && n.Left.Op == OCLOSURE {	// 调用了闭包函数
			...
			n.Left.Func.Enter.Set(nil)	// 闭包函数节点的Enter设置为nil

			// Replace OCLOSURE with ONAME/PFUNC.
			n.Left = n.Left.Func.Closure.Func.Nname	// 获取外部函数的ONAME节点

			// Update type of OCALLFUNC node.
			// Output arguments had not changed, but their offsets could.
			if n.Left.Type.NumResults() == 1 {	// 返回的参数只有一个
				n.Type = n.Left.Type.Results().Field(0).Type	// 设置节点的类型为返回值类型
			} else {
				n.Type = n.Left.Type.Results()	// // 设置节点的类型为返回值结构体类型
			}
		}

		walkCall(n, init)

	case OAS, OASOP:	// Left = Right or (if Colas=true) Left := Right         Left Etype= Right (x += y)
		init.AppendNodes(&n.Ninit)

		// Recognize m[k] = append(m[k], ...) so we can reuse
		// the mapassign call.
		mapAppend := n.Left.Op == OINDEXMAP && n.Right.Op == OAPPEND	// ONAME = OLITERAL
		...
		if mapAppend {
			n.Right.List.SetFirst(n.Left)	// OINDEXMAP节点的复用，避免计算两次
		}
		...
		case ORECV:
			...
			n1 := nod(OADDR, n.Left, nil)	// 取变量的地址
	...
	case OAS2:	// List = Rlist (x, y, z = a, b, c)
		...
		ll := ascompatee(OAS, n.List.Slice(), n.Rlist.Slice(), init)	// 将OAS2整理成多个OAS的操作
		ll = reorder3(ll)	// 为多次变更的左部变量创建临时变量语句并保存在OAS2操作的前面
		n = liststmt(ll)	// 将ll包装为一个OBLOCK代码块节点
	...
	case OAS2RECV:	// List = Right (x, ok = <-c)
		init.AppendNodes(&n.Ninit)
		...
		n = nod(OAS, ok, call)	// 函数调用的结果赋给ok
		n = typecheck(n, ctxStmt)

	// a,b = m[i]
	case OAS2MAPR:	// List = Right (x, ok = m["foo"])
		init.AppendNodes(&n.Ninit)
		...
		if w := t.Elem().Width; w <= zeroValSize {	// w为Map的value类型的宽度
		...
		if !a.isBlank() {
			var_ := temp(types.NewPtr(t.Elem()))	// map保存的value的指针类型
			var_.SetTypecheck(1)
			var_.MarkNonNil() // mapaccess always returns a non-nil pointer
			n.List.SetFirst(var_)
			n = walkexpr(n, init)
			init.Append(n)
			n = nod(OAS, a, nod(ODEREF, var_, nil))	// a = *var
		}
	...
	case OCONVIFACE:	// Type(Left) (type conversion, to interface)
		...
		case fromType.IsBoolean() || (fromType.Size() == 1 && fromType.IsInteger()):
			...
			index := nod(OLSH, byteindex(n.Left), nodintconst(3))	// 计算n.Left * 8
			if thearch.LinkArch.ByteOrder == binary.BigEndian {	// 如果大端序存储再加7
				index = nod(OADD, index, nodintconst(7))
			}
			value = nod(OINDEX, staticuint64s, index)
			value.SetBounded(true)	// 设置不需要进行数组的边界检查
		case n.Left.Class() == PEXTERN && n.Left.Name != nil && n.Left.Name.Readonly():	// 用户声明的全局不可修改的变量
		...
		if toType.IsEmptyInterface() && fromType.IsInterface() && !fromType.IsEmptyInterface() {
			...
			tmp := temp(types.NewPtr(types.Types[TUINT8]))	// *byte类型
			init.Append(nod(OAS, tmp, typecheck(nod(OITAB, c, nil), ctxExpr)))	// tmp = i.itab

			// Get the type out of the itab.
			nif := nod(OIF, typecheck(nod(ONE, tmp, nodnil()), ctxExpr), nil)	// if tmp != nil
			nif.Nbody.Set1(nod(OAS, tmp, itabType(tmp)))	//    tmp = tmp.type
			init.Append(nif)
		...
		fnname, needsaddr := convFuncName(fromType, toType)	// 获取fromType转换到toType的函数名,以及用该函数进行类型转换是否需要使用fromType的指针类型

		if !needsaddr && !fromType.IsInterface() {	// 满足该条件时表示fromType是一个具体的类型
			...
			dowidth(fn.Type)
			call := nod(OCALL, fn, nil)	// 创建函数调用节点用于调用类型转换函数
			call.List.Set1(n.Left)	// 设置函数的参数
	...
	case ODIV, OMOD:	// Left / Right, Left % Right
		n.Left = walkexpr(n.Left, init)
		n.Right = walkexpr(n.Right, init)
	...
	case OINDEX:	// Left[Right] (index of array or slice)
		...
		if Isconst(n.Right, CTINT) {
			if n.Right.Val().U.(*Mpint).Cmp(maxintval[TINT]) > 0 {
				yyerror("index out of bounds")
			}
		}
	...
	case ONEW:	// new(Left); corresponds to calls to new in source code
		...
		if n.Esc == EscNone {
			...
			r := temp(n.Type.Elem())	// 创建一个相同类型的临时变量
			r = nod(OAS, r, nil) // zero temp
			r = typecheck(r, ctxStmt)
			init.Append(r)
			r = nod(OADDR, r.Left, nil)	// 取临时变量的地址
	...
	case OMAKECHAN:	// make(Type, Left) (type is chan)
	...
	case OMAKEMAP:	// make(Type, Left) (type is map)
		t := n.Type
		hmapType := hmap(t)
		hint := n.Left
	...
	case OMAKESLICE:	 // make(Type, Left, Right) (type is slice)
		l := n.Left
		r := n.Right
		if r == nil {
			r = safeexpr(l, init)
		...
		if n.Esc == EscNone {
			...
			//     panicmakeslicecap()
			// }
			nif := nod(OIF, nod(OGT, conv(l, types.Types[TUINT64]), nodintconst(i)), nil)	// if uint64(len) > cap
			niflen := nod(OIF, nod(OLT, l, nodintconst(0)), nil)	//     if len < 0
	...
	case OMAKESLICECOPY:	// makeslicecopy(Type, Left, Right) (type is slice; Left is length and Right is the copied from slice)
		...
		if !t.Elem().HasPointers() && n.Bounded() {	// 切片中不包含指针且无需边界检查
	...
	case ORUNESTR:	// Type(Left) (Type is string, Left is rune)
		a := nodnil()
		if n.Esc == EscNone {
	...
	case OBYTES2STR, ORUNES2STR:	// Type(Left) (Type is string, Left is a []byte), Type(Left) (Type is string, Left is a []rune)
		a := nodnil()
		if n.Esc == EscNone {
	...
	case OSTR2BYTES:	// Type(Left) (Type is []byte, Left is a string)
		s := n.Left
		if Isconst(s, CTSTR) {	// s是字符串常量
			sc := strlit(s)	// 获取字符串
			...
			n.Op = OSLICEARR	// Left[List[0] : List[1]] (Left is array),这里n的List为nil,所以应该是slice = array[:]
			n.Left = p
			n = walkexpr(n, init)
			break
		}
	...
	case OSTR2RUNES:	// Type(Left) (Type is []rune, Left is a string)
		a := nodnil()
		if n.Esc == EscNone {
			// Create temporary buffer for slice on stack.
	...
	case OCLOSURE:	// func Type { Body } (func literal)
		n = walkclosure(n, init)

	case OCALLPART:	// Left.Right (method expression x.Method, not called)
...
func ascompatee(op Op, nl, nr []*Node, init *Nodes) []*Node {
	...
	if i < len(nl) || i < len(nr) {	// 出现这种情况表示左右列表的长度不同
		var nln, nrn Nodes
		nln.Set(nl)
		nrn.Set(nr)
...
func ascompatet(nl Nodes, nr *types.Type) []*Node {
	...
	// nn保存最终将返回结果赋值给变量的OAS列表,mm保存会用到函数调用的的OAS节点列表
	var nn, mm Nodes
	for i, l := range nl.Slice() {
		if l.isBlank() {
			continue
		}
		r := nr.Field(i)	// 获取函数的第i个返回参数
...
// 处理函数声明时存在可变参数但是调用时没有可变参数的情况,将实参构成一个切片
func fixVariadicCall(call *Node) {
	fntype := call.Left.Type	//获取被调用的函数的类型
	if !fntype.IsVariadic() || call.IsDDD() {	// 函数声明中没有可变参数或者调用函数时存在可变参数就退出
		return
	}
	// 到了这里，应该是函数声明时有可变参数但是调用时没有使用可变参数
	vi := fntype.NumParams() - 1	// 可变参数下标
	vt := fntype.Params().Field(vi).Type	// 可变参数的类型

	args := call.List.Slice()	//形参列表
	extra := args[vi:]			// 获取应该组合为切片的所有可变参数列表
...
func walkCall(n *Node, init *Nodes) {
	...
	params := n.Left.Type.Params()	// 获取函数定义的所有的参数
	args := n.List.Slice()	// 函数的实参列表
	...
	if n.Op == OCALLMETH {
		...
		withRecv[0] = n.Left.Left	// add the receiver at the beginning of the args.
		n.Left.Left = nil
...
func walkprint(nn *Node, init *Nodes) *Node {
	...
	walkexprlistcheap(nn.List.Slice(), init)	// 遍历print或者println中的参数列表
	...
	t := make([]*Node, 0, len(s))
	for i := 0; i < len(s); {
		var strs []string	// 存储参数列表中的字符串常量，如果保存了多个常量，表示这些常量是邻接的
		for i < len(s) && Isconst(s[i], CTSTR) {
			strs = append(strs, strlit(s[i]))
			i++
		}
		if len(strs) > 0 {
			t = append(t, nodstr(strings.Join(strs, "")))	// 将多个相邻的字符串折叠为一个
		}
		if i < len(s) {
			t = append(t, s[i])
			i++
	...
	calls := []*Node{mkcall("printlock", nil, init)}	// 存储最终生成的所有print函数调用节点
	for i, n := range nn.List.Slice() {	// 遍历print或者println中的参数列表
		if n.Op == OLITERAL {	// 字面量参数
			switch n.Val().Ctype() {
			case CTRUNE:
		...
		case TUINT, TUINT8, TUINT16, TUINT32, TUINT64, TUINTPTR:
			if isRuntimePkg(n.Type.Sym.Pkg) && n.Type.Sym.Name == "hex" {	// runtime下的hex类型
				on = syslook("printhex")
			} else {
		...
		r := nod(OCALL, on, nil)	// 调用最终的打印函数
		if params := on.Type.Params().FieldSlice(); len(params) > 0 {
			t := params[0].Type	// 获取第一个参数的类型
			if !types.Identical(t, n.Type) {	// 实参与形参类型不相同
				n = nod(OCONV, n, nil)	// 将n强转为t类型
				n.Type = t
			}
			r.List.Append(n)	// 添加实参n
		}
...
func reorder3(all []*Node) []*Node {
	...
	var early []*Node	// 被修改过的值再次被使用会生成临时变量保存到这个切片中
...
func reorder3save(n *Node, all []*Node, i int, early *[]*Node) *Node {
	if !aliased(n, all, i) {	// 赋值n之前没有影响到的变量
		return n
	}

	q := temp(n.Type)	// 创建一个临时变量
	q = nod(OAS, q, n)	// 临时变量保存n的值
...
func zeroResults() {
	for _, f := range Curfn.Type.Results().Fields().Slice() {	// 遍历当前函数的返回结果列表
		v := asNode(f.Nname)
...
func returnsfromheap(params *types.Type) []*Node {
	var nn []*Node
	for _, t := range params.Fields().Slice() {	// 遍历回参列表
		v := asNode(t.Nname)	// 获取关联的ONAME节点
		if v == nil {
			continue
		}
		if stackcopy := v.Name.Param.Stackcopy; stackcopy != nil && stackcopy.Class() == PPARAMOUT {
			nn = append(nn, walkstmt(typecheck(nod(OAS, stackcopy, v), ctxStmt)))	// 将v赋值给stackcopy
		}
	}
...
func vmkcall(fn *Node, t *types.Type, init *Nodes, va []*Node) *Node {
	...
	n := fn.Type.NumParams()	// 获取函数的入参数量
	if n != len(va) {	// 表示参数数量不对
		Fatalf("vmkcall %v needs %v args got %v", fn, n, len(va))
	}

	r := nod(OCALL, fn, nil)	// 新建一个OCALL节点，Left指向fn
	r.List.Set(va)	// 设置实参
	if fn.Type.NumResults() > 0 {	// 有参数
...
func convnop(n *Node, t *types.Type) *Node {
	...
	n.Type = t
	n = typecheck(n, ctxExpr)	// 类型转换是一个表达式
	return n
}
...
func byteindex(n *Node) *Node {
	...
	if !types.Identical(n.Type, types.Types[TUINT8]) {
		n = nod(OCONV, n, nil)	// 将节点n强转为uint8类型
		n.Type = types.Types[TUINT8]
		n.SetTypecheck(1)
	}
	n = nod(OCONV, n, nil)	// 将n强转为int类型
...
func chanfn(name string, n int, t *types.Type) *Node {
	...
	default:
		Fatalf("chanfn %d", n)
	case 1:
		fn = substArgTypes(fn, t.Elem())	// 将fn的类型替换为管道的元素类型
	case 2:
		fn = substArgTypes(fn, t.Elem(), t.Elem())
	}
	return fn
...
func addstr(n *Node, init *Nodes) *Node {
	...
	if n.Esc == EscNone {
		...
		if sz < tmpstringbufsize {
			...
			t := types.NewArray(types.Types[TUINT8], tmpstringbufsize)	// tmpstringbufsize长度的byte数组
			buf = nod(OADDR, temp(t), nil)	// 取数组变量的地址
		}
	}
...
func isAppendOfMake(n *Node) bool {
	...
	y := second.Left	// 获取makeslice的len参数
...
func copyany(n *Node, init *Nodes, runtimecall bool) *Node {
	if n.Left.Type.Elem().HasPointers() {	// 左边指针指向的元素类型中有指针
		Curfn.Func.setWBPos(n.Pos)
		fn := writebarrierfn("typedslicecopy", n.Left.Type.Elem(), n.Right.Type.Elem())
		n.Left = cheapexpr(n.Left, init)
		ptrL, lenL := n.Left.slicePtrLen()	// 获取slice的ptr与len
	...
	if runtimecall {
		if n.Right.Type.IsString() {	// copy操作的右边是字符串
			fn := syslook("slicestringcopy")
			n.Left = cheapexpr(n.Left, init)
			ptr, len := n.Left.slicePtrLen()
			str := nod(OCONVNOP, n.Right, nil)	// string强转为string?这什么操作?
			str.Type = types.Types[TSTRING]
	...
	nfrm := nod(OSPTR, nr, nil)	// src的元素指针
	nto := nod(OSPTR, nl, nil)	// dst的元素指针
	...
	setwid := nod(OAS, nwid, conv(nlen, types.Types[TUINTPTR]))	// len(to)保存到setwid
	ne.Nbody.Append(setwid)
	nwid = nod(OMUL, nwid, nodintconst(nl.Type.Elem().Width))	// 计算出要拷贝的字节数
...
func walkcompare(n *Node, init *Nodes) *Node {
	...
	if n.Left.Type.IsInterface() != n.Right.Type.IsInterface() {	// 一个interface与非interface进行的比较
		...
		if l.Type.IsEmptyInterface() {
			tab.Type = types.NewPtr(types.Types[TUINT8])	// 指向一个字节的指针
			tab.SetTypecheck(1)
			eqtype = nod(eq, tab, rtyp)
		} else {	// l是个非空接口
			nonnil := nod(brcom(eq), nodnil(), tab)	// tab != nil
			match := nod(eq, itabType(tab), rtyp)	// l.tab._type == type(r)
...
func walkcompareInterface(n *Node, init *Nodes) *Node {
	...
	var cmp *Node
	if n.Op == OEQ {	// 这里表示如果两个interface使用==进行比较的话就要同时满足eqtab与eqdata
...
func walkcompareString(n *Node, init *Nodes) *Node {
	...
	case Isconst(n.Left, CTSTR):	// 字符串与其他类型的比较
		cs = n.Left
		ncs = n.Right
	case Isconst(n.Right, CTSTR):	// 其他类型与字符串的比较
		cs = n.Right
		ncs = n.Left
	}
	if cs != nil {	// 比较操作中存在常量字符串
		cmp := n.Op
		// Our comparison below assumes that the non-constant string
		// is on the left hand side, so rewrite "" cmp x to x cmp "".
		// See issue 24817.
		if Isconst(n.Left, CTSTR) {	// 如果比较运算符的左边是字符串就将比较运算符置反
			cmp = brrev(cmp)
		}
		...
		if s := strlit(cs); len(s) <= maxRewriteLen {
			...
			r := nod(cmp, nod(OLEN, ncs, nil), nodintconst(int64(len(s))))	// 建立比较节点
			remains := len(s)	// 字符串的长度
			for i := 0; remains > 0; {
				if remains == 1 || !canCombineLoads {
					cb := nodintconst(int64(s[i]))	// 获取s[i]字符
					ncb := nod(OINDEX, ncs, nodintconst(int64(i)))	// 获取ncs的第i个元素
					r = nod(and, r, nod(cmp, ncb, cb))	// 构造进原来的抽象语法树中
					remains--
					i++
					continue
				}
				...
				ncsubstr = conv(ncsubstr, convType)	// 将ncs的第i项转换为数字类型
				csubstr := int64(s[i])	// s[i]转为数字类型
				// Calculate large constant from bytes as sequence of shifts and ors.
				// Like this:  uint32(s[0]) | uint32(s[1])<<8 | uint32(s[2])<<16 ...
				// ssa will combine this into a single large load.
				for offset := 1; offset < step; offset++ {
					b := nod(OINDEX, ncs, nodintconst(int64(i+offset)))	// 取出ncs的i+offset项
					b = conv(b, convType)	// 转为对应的类型
					b = nod(OLSH, b, nodintconst(int64(8*offset)))	// 左移8*offset位
	...
	// 字符串比较中没有常量字符串
	var r *Node
	if n.Op == OEQ || n.Op == ONE {	// 比较运算符为等于或不等
		...
		r = nod(n.Op, r, nodintconst(0))	// 依据cmpstring的结果与0比较来确定两个字符串谁大谁小
	}
...
func bounded(n *Node, max int64) bool {
	...
	sign := n.Type.IsSigned()	// 是否为有符号数
	bits := int32(8 * n.Type.Width)	// 类型宽度占多少个比特位
	...
	case OAND:
		v := int64(-1)
		if smallintconst(n.Left) {	// 如果&操作符左边是int常量的话,那么右边必定不是int常量,否则之前会直接计算出结果
			v = n.Left.Int64()
...
func usemethod(n *Node) {
	t := n.Left.Type	// 获取参数的类型(所有的参数会被组装为一个结构体)
	...
	if n := t.NumParams(); n != 1 {	// 参数数量不是1个返回
		return
	}
	if n := t.NumResults(); n != 1 && n != 2 {	// 返回值数量不是1个也不是2个返回
		return
	}
	p0 := t.Params().Field(0)	// 获取参数
	res0 := t.Results().Field(0)	// 第一个返回参数
	var res1 *types.Field
	if t.NumResults() == 2 {	// 如果返回参数有2个
		res1 = t.Results().Field(1)		// 获取第二个返回参数
	}

	if res1 == nil {	// 只有一个回参
		if p0.Type.Etype != TINT {	// 参数不是int类型就返回
			return
		}	
	} else {	// 两个回参的情况
		if !p0.Type.IsString() {	// 参数不是string类型就返回	
			return
		}
		if !res1.Type.IsBoolean() {	// 第二个回参不是布尔类型就返回
			return
		}
	}
...
func usefield(n *Node) {
	...
	field := dotField[typeSymKey{t.Orig, n.Sym}]	// 通过类型及sym获取对应的字段
	if field == nil {
	...
	// 字段的tag后面有go:track的情况
	outer := n.Left.Type	// 获取结构体类型
	if outer.IsPtr() {
		outer = outer.Elem()
	}
	if outer.Sym == nil {
...
func substArgTypes(old *Node, types_ ...*types.Type) *Node {
	...
	n.Type = types.SubstAny(n.Type, &types_)	// 将n的类型从any替换为指定类型
	if len(types_) > 0 {
