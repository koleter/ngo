...
func exportsym(n *Node) {
	if n.Sym.OnExportList() {	// 判断符号是否已经被导出了
		return
	}
	n.Sym.SetOnExportList(true)	// 标记已被导出
...
func autoexport(n *Node, ctxt Class) {
	if n.Sym.Pkg != localpkg {	// 导出的变量需要定义在当前包中
		return
	}
	if (ctxt != PEXTERN && ctxt != PFUNC) || dclcontext != PEXTERN {	// 这里表示只有全局的变量或函数才可以被导出
		return
	}
	if n.Type != nil && n.Type.IsKind(TFUNC) && n.IsMethod() {	// 拥有receiver的函数,也就是method不会被导出
		return
	}

	if types.IsExported(n.Sym.Name) || initname(n.Sym.Name) {	// 只有首字母大写的函数或者init函数才会被导出
		exportsym(n)
	}
	if asmhdr != "" && !n.Sym.Asm() {
		n.Sym.SetAsm(true)
