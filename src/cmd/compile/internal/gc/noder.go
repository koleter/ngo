...
func (p *noder) closeScope(pos syntax.Pos) {
	...
	if trackScopes {
		scopeVars := p.scopeVars[len(p.scopeVars)-1] // 作用域的变量数
		...
		if scopeVars == len(Curfn.Func.Dcl) {
			...
			prevScope := ScopeID(0)
			if nmarks >= 2 { // nmarks大于等于2表示
				prevScope = Curfn.Func.Marks[nmarks-2].Scope
			}
			if Curfn.Func.Marks[nmarks-1].Scope == prevScope { // 倒数第一个与倒数第二个作用域是相同的就将最后一个作用域弹出
...
func (p *noder) node() {
	...
	mkpackage(p.file.PkgName.Value) // 设置当前被编译的package名
...
func (p *noder) importDecl(imp *syntax.ImportDecl) {
	...
	val := basicLit(imp.Path) // 将import导入的包进行字符串处理
	ipkg := importfile(&val)  // ipkg为import后的导入的实际包对应的Pkg
	...
	var my *types.Sym
	if imp.LocalPkgName != nil { // 存在对于导入的包的别名
...
func (p *noder) varDecl(decl *syntax.VarDecl) []*Node {
	names := p.declNames(decl.NameList) // 将所有的变量成员包装成Op = ONONAME的Node节点
...
type constState struct {
	group  *syntax.Group // const块的group
	typ    *Node         // 记录上一个有效表达式的type
	values []*Node       // 记录上一个有效表达式的Value
	iota   int64
}

func (p *noder) constDecl(decl *syntax.ConstDecl, cs *constState) []*Node {
	if decl.Group == nil || decl.Group != cs.group { // 这里限制了constState对于全局变量声明中同一个const块有效
		*cs = constState{
			group: decl.Group,
		}
	}
	...
	names := p.declNames(decl.NameList) // 声明的变量组成的Nodes
	typ := p.typeExprOrNil(decl.Type)

	var values []*Node // 存储
	if decl.Values != nil {
		values = p.exprList(decl.Values)
		cs.typ, cs.values = typ, values // 存储当前的类型以及值
	} else {
		if typ != nil { // 值为nil却存在type的情况
			yyerror("const declaration cannot have type without expression")
		}
		typ, values = cs.typ, cs.values // value没有给出,那么从上一次存储的地方获取类型以及值
	}

	nn := make([]*Node, 0, len(names))
	for i, n := range names { // 遍历左部变量
		if i >= len(values) {
			yyerror("missing value in const declaration")
			break
		}
		v := values[i]
		if decl.Values == nil { // 当前的声明没有Values
			v = treecopy(v, n.Pos)
		}
...
func (p *noder) typeDecl(decl *syntax.TypeDecl) *Node {
	...
	if pragma, ok := decl.Pragma.(*Pragma); ok {
		if !decl.Alias { // 没有=
			param.Pragma = pragma.Flag & TypePragmas // 这个类型仅保留不能通过Heap创建的行指令
			pragma.Flag &^= TypePragmas              // Flag中去除TypePragmas
		}
		p.checkUnused(pragma)
	}
...
type funcPair struct {
	reg                     *regexp.Regexp
	beforenodes, afternodes []*Node
}

var AopMap = map[string]*funcPair{}

func recordAop(str string, node *Node) {
	for _, stmt := range strings.Split(str, ";") {
		...
		dir := strings.Trim(stmt[:i], " ")
		funcPattern := strings.Trim(stmt[i+1:], " ")
		p, ok := AopMap[funcPattern]
		if !ok {
			p = &funcPair{
				reg:         regexp.MustCompile(funcPattern),
				beforenodes: nil,
				afternodes:  nil,
			}
			AopMap[funcPattern] = p
		}

		switch dir {
		case "before":
			p.beforenodes = append(p.beforenodes, node)
		case "after":
			p.afternodes = append(p.afternodes, node)
		case "around":
			p.beforenodes = append(p.beforenodes, node)
			p.afternodes = append(p.afternodes, node)
		}
...
func (p *noder) funcDecl(fun *syntax.FuncDecl) *Node {
	...
	if fun.Recv == nil {
		if name.Name == "init" {
			name = renameinit()
			if t.List.Len() > 0 || t.Rlist.Len() > 0 { // init函数的形参或返回值列表的数量大于0
				yyerrorl(f.Pos, "func init must have no arguments and no return values")
			}
		}

		if localpkg.Name == "main" && name.Name == "main" {
			if t.List.Len() > 0 || t.Rlist.Len() > 0 { // main函数的形参或返回值列表的数量大于0
	...
	if pragma, ok := fun.Pragma.(*Pragma); ok {
		...
		if pragma.Flag&Aop != 0 {
			for _, s := range pragma.AopStr {
				recordAop(s, f.Func.Nname)
			}
		}
	...
	if fun.Body != nil {
		if f.Func.Pragma&Noescape != 0 { // //go:noescape行指令不能有函数体
...
func (p *noder) expr(expr syntax.Expr) *Node {
	...
	case nil, *syntax.BadExpr:
		return nil
	case *syntax.Name:
		return p.mkname(expr) // 依据expr的value获取Sym并将Sym代表的变量或者闭包变量包装为Node节点
	case *syntax.BasicLit:
		n := nodlit(basicLit(expr))
	...
	case *syntax.SelectorExpr:
		// parser.new_dotname
		obj := p.expr(expr.X) // 将X包装成Node节点
		if obj.Op == OPACK {  // 表示使用了某个导入的包名
	...
	case *syntax.CallExpr:
		...
		n.List.Set(p.exprs(expr.ArgList)) // 处理函数的实参列表
	...
	case *syntax.StructType: // 结构体的声明
		return p.structType(expr)
	case *syntax.InterfaceType: // interface的声明
...
func (p *noder) sum(x syntax.Expr) *Node {
	...
	adds := make([]*syntax.Operation, 0, 2) // 将多个+连接的Operation结构体保存到adds
	for {
	...
	n := p.expr(x)
	if Isconst(n, CTSTR) && n.Sym == nil { // 字符串常量
		nstr = n
		chunks = append(chunks, strlit(nstr))
	}

	for i := len(adds) - 1; i >= 0; i-- { // 倒序遍历
		add := adds[i]
...
func (p *noder) structType(expr *syntax.StructType) *Node {
	...
	for i, field := range expr.FieldList {
		...
		if i < len(expr.TagList) && expr.TagList[i] != nil {
			n.SetVal(basicLit(expr.TagList[i]))
		}
		l = append(l, n)
	}
...
func (p *noder) interfaceType(expr *syntax.InterfaceType) *Node {
	...
	for _, method := range expr.MethodList {
		...
		if method.Name == nil {
			...
			sig.Left = fakeRecv() // 为interface成员定义一个fakeRecv
...
func (p *noder) stmtsFall(stmts []syntax.Stmt, fallOK bool) []*Node {
	...
	for i, stmt := range stmts {
		s := p.stmtFall(stmt, fallOK && i+1 == len(stmts)) //最后一个stmt才会是true
		if s == nil {
...
func (p *noder) stmtFall(stmt syntax.Stmt, fallOK bool) *Node {
	...
	case *syntax.AssignStmt:
		if stmt.Op != 0 && stmt.Op != syntax.Def { // a = b
		...
		rhs := p.exprList(stmt.Rhs)                             // 处理赋值表达式右部
		lhs := p.assignList(stmt.Lhs, n, stmt.Op == syntax.Def) // 处理赋值表达式左部
	...
	case *syntax.ReturnStmt:
		...
		n.List.Set(results)
		if n.List.Len() == 0 && Curfn != nil { // return后没有跟变量且处于函数中
			for _, ln := range Curfn.Func.Dcl { // 遍历函数中所有声明的变量
				if ln.Class() == PPARAM { // 形参或者recv
					continue
				}
				if ln.Class() != PPARAMOUT { // 不属于返回值列表就跳出循环
					break
				}
				if asNode(ln.Sym.Def) != ln { // 出现这种情况表示在函数中定义了返回值变量名,但是在函数体中又声明了相同的变量
...
// expr是赋值表达式左部,defn是赋值表达式最终转化的抽象语法树节点,colas为true表示通过:=声明的变量,colas为false表示通过=进行的变量赋值
func (p *noder) assignList(expr syntax.Expr, defn *Node, colas bool) []*Node {
	...
	seen := make(map[*types.Sym]bool, len(exprs)) // 记录已经遍历的符号,通过:=进行变量的声明不能出现相同的符号
	...
	for i, expr := range exprs {
		...
		name, ok := expr.(*syntax.Name) // 左部的操作必须是Name
		if !ok {
		...
		sym := p.name(name) // 依据name获取或创建Sym
		if sym.IsBlank() {  // 变量如果为“_”
			continue
		}

		if seen[sym] { // 此处为true表示一行赋值语句中左部出现了除了_之外相同的变量名
		...
		if sym.Block == types.Block { // 符号声明在当前块中
			res[i] = oldname(sym) // 直接获取声明的符号
			continue
		}
		...
		declare(n, dclcontext)
		n.Name.Defn = defn
		defn.Ninit.Append(nod(ODCL, n, nil)) // 声明变量n
		res[i] = n
	}
...
func (p *noder) caseClauses(clauses []*syntax.CaseClause, tswitch *Node, rbrace syntax.Pos) []*Node {
	...
	for i, clause := range clauses {
		...
		if l := n.Nbody.Len(); l > 0 && n.Nbody.Index(l-1).Op == OFALL { // case中的最后一行出现了fallthrough
			if tswitch != nil {
...
func basicLit(lit *syntax.BasicLit) Val {
...
func (p *noder) pragma(pos syntax.Pos, blankLine bool, text string, old syntax.Pragma) syntax.Pragma {
	...
	switch {
	case strings.HasPrefix(text, "go:aop "):
		pragma.Flag |= Aop
		pragma.AopStr = append(pragma.AopStr, text[7:])
