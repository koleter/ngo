...
func restrictlookup(name string, pkg *types.Pkg) *types.Sym {
	if !types.IsExported(name) && pkg != localpkg { // 不能使用非当前包中的不可导出的符号
...
func importdot(opkg *types.Pkg, pack *Node) {
	n := 0
	for _, s := range opkg.Syms { // 遍历该包中所有定义的Sym
		if s.Def == nil {
			continue
		}
		if !types.IsExported(s.Name) || strings.ContainsRune(s.Name, 0xb7) { // 0xb7 = center dot	符号不可导出或者符号名中包含center dot就不会将符号放到当前包中
			continue
		}
		s1 := lookup(s.Name) // 在本地包中添加可以导出且符号中不包含center dot的symbol
		if s1.Def != nil {   // 表示导入了多个拥有相同名字的Sym
...
func newnamel(pos src.XPos, s *types.Sym) *Node {
	...
	// 下面x的写法是为了方便直接申请出Node, Name与Param三个结构体的内存
	var x struct {
		n Node
		m Name
		p Param
...
// If not, return OXXX.
func assignop(src, dst *types.Type, why *string) Op { // 相当于判断要从src类型到dst类型需要做什么转换方式
	if why != nil {
		*why = ""
	}
	...
	if types.Identical(src.Orig, dst.Orig) { // src与dst具有相同的初始类型
		if src.IsEmptyInterface() { // 都是空的interface
...
func convertop(srcConstant bool, src, dst *types.Type, why *string) Op {
	...
	// (b) Disallow string to []T where T is go:notinheap.   这里的T是byte类型或者rune类型
...
func assignconvfn(n *Node, t *types.Type, context func() string) *Node {
	...
	if t.Etype == TBLANK && n.Type.Etype == TNIL { // TBLANK: 31     TNIL: 30
		yyerror("use of untyped nil")
	}

	n = convlit1(n, t, false, context) // 将n设置为t类型
	if n.Type == nil {
		return n
	}
	if t.Etype == TBLANK {
...
func (n *Node) labeledControl() *Node {
	...
	ctl := n.Name.Defn // 找到声明label的那个节点
	if ctl == nil {
		return nil
	}
	switch ctl.Op {
...
// 从runtime包下根据name字符串找到指定的定义Sym的节点(该节点应该是一个函数节点)
...
func brrev(op Op) Op {
	...
	case ONE: // !=
		return ONE // !=
	case OLT: // Left < Right
		return OGT // Left > Right
	case OGT: // Left > Right
		return OLT // Left < Right
	case OLE: // Left <= Right
		return OGE // Left >= Right
	case OGE: // Left >= Right
		return OLE // Left <= Right
	}
...
func safeexpr(n *Node, init *Nodes) *Node {
	...
	if n.Ninit.Len() != 0 {
		...
		init.AppendNodes(&n.Ninit) // 将n.Ninit中的元素添加到init中并清空n.Ninit
	}

	switch n.Op {
	case ONAME, OLITERAL: // 变量与字面量没有副作用
		return n

	case ODOT, OLEN, OCAP:
		l := safeexpr(n.Left, init)
		if l == n.Left { // 表示safeexpr没有改变参数
			return n
		}
		r := n.copy()
		r.Left = l
...
func copyexpr(n *Node, t *types.Type, init *Nodes) *Node {
	l := temp(t)        // 创建一个类型t的临时变量
	a := nod(OAS, l, n) // n赋给临时变量
	a = typecheck(a, ctxStmt)
	a = walkexpr(a, init)
	init.Append(a)
	return l // 将临时变量返回,副作用通过a已经收集完毕了
}
...
func lookdot0(s *types.Sym, t *types.Type, save **types.Field, ignorecase bool) int {
	...
	if u.IsStruct() || u.IsInterface() {
		for _, f := range u.Fields().Slice() { // 遍历u所拥有的fields
			if f.Sym == s || (ignorecase && f.IsMethod() && strings.EqualFold(f.Sym.Name, s.Name)) {
				if save != nil {
					*save = f // save保存最初发现的最短dotpath
				}
				c++ // 发现相同的field就将c加一
			}
		}
	...
	u = methtype(u)
	if u != nil {
		for _, f := range u.Methods().Slice() { // 遍历u所包含的所有方法
			if f.Embedded == 0 && (f.Sym == s || (ignorecase && strings.EqualFold(f.Sym.Name, s.Name))) {
				if save != nil {
					*save = f
				}
				c++ // 不是嵌入的类型且是相同符号就将c加一
			}
		}
	}
...
func adddot1(s *types.Sym, t *types.Type, d int, save **types.Field, ignorecase bool) (c int, more bool) {
	...
	if !u.IsStruct() && !u.IsInterface() { // 不是结构体或者interface类型直接返回
		return c, false
	}

	for _, f := range u.Fields().Slice() {
		if f.Embedded == 0 || f.Sym == nil { // 只处理嵌入类型
			continue
		}
		if d < 0 {
		...
		if a != 0 && c == 0 { // 某个field拥有多个相同名字的函数或者成员变量且当前属于最初找到的相同符号
			dotlist[d].field = f // 记录在深度d中找到的可以到达符号s的字段f
		}
		c += a // 累加所有相同名字的字段或函数
		if more1 {
			more = true
...
func dotpath(s *types.Sym, t *types.Type, save **types.Field, ignorecase bool) (path []Dlist, ambig bool) {
	...
	for d := 0; ; d++ {
		...
		if c, more := adddot1(s, t, d, save, ignorecase); c == 1 { // 表示该类型中仅包含s一个符号，没有与s相同命名的符号
			return dotlist[:d], false // 截取dotlist的前d项
		} else if c > 1 { // there are multiple shortest paths to the same depth
			return nil, true
		} else if !more {
			return nil, false
		}
...
var slist []Symlink // 存储interface或者结构体类型扩展后的所有函数fields构成的Symlink节点构体
...
func expand1(t *types.Type, top bool) {
	...
	if u.IsStruct() || u.IsInterface() { // t定义时是结构体或者接口类型
		for _, f := range u.Fields().Slice() { // 遍历字段
			if f.Embedded == 0 { // 非嵌入式类型跳过
				continue
			}
			if f.Sym == nil {
				continue
			}
			expand1(f.Type, false) // 对嵌入的类型进行递归
		}
	}
...
// 扩展t的函数,将内嵌的类型的函数添加到t中
func expandmeth(t *types.Type) {
	if t == nil || t.AllMethods().Len() != 0 { // allMethods的length不为0表示expand过了
		return
	}
	...
	slist = slist[:0] // 这是用来存储类型t可以调用的所有函数的,如果类型t是一个结构体,有内嵌类型,那么内嵌类型可以调用的函数也在这里
	expand1(t, true)
	...
	for i, sl := range slist {
		...
		var f *types.Field
		path, _ := dotpath(sl.field.Sym, t, &f, false) // 最短的路径相当于(path[length-1].field).(path[length-2].field)...((path[0].field))
		if path == nil {
			continue
		}

		// dotpath may have dug out arbitrary fields, we only want methods.
		if !f.IsMethod() { // 是定义的结构体字段就continue
			continue
		}
		...
		for _, d := range path {
			if d.field.Type.IsPtr() { // dotpath中某一层是一个指针
				f.Embedded = 2
				break
			}
		}
...
func structargs(tl *types.Type, mustname bool) []*Node {
	...
	for _, t := range tl.Fields().Slice() {
		s := t.Sym
		if mustname && (s == nil || s.Name == "_") { // 参数没有名字就为创建一个名字
...
func genwrapper(rcvr *types.Type, method *types.Field, newnam *types.Sym) {
	...
	if rcvr.IsPtr() && rcvr.Elem() == method.Type.Recv().Type &&
		rcvr.Elem().Sym != nil && rcvr.Elem().Sym.Pkg != localpkg { // rcvr是指针类型,与函数的receiver类型相同且rcvr指向的类型不是在当前包中定义的就退出
		return
	}

	// Only generate I.M wrappers for I in I's own package
	// but keep doing it for error.Error (was issue #29304).
	if rcvr.IsInterface() && rcvr.Sym != nil && rcvr.Sym.Pkg != localpkg && rcvr != types.Errortype { // rcvr是接口且不在当前包中声明就退出,只在receiver被声明的包中生成这个包装函数
		return
	}

	lineno = autogeneratedPos
	dclcontext = PEXTERN // global variable

	tfn := nod(OTFUNC, nil, nil)         // 生成一个函数节点
	tfn.Left = namedfield(".this", rcvr) // 函数节点的receiver为类型为rcvr的节点,符号为.this
	...
	fn.Func.SetDupok(true) // 设置可以重复定义

	nthis := asNode(tfn.Type.Recv().Nname) // 代表receiver的ONAME节点

	methodrcvr := method.Type.Recv().Type // method的recv的类型
	...
	if rcvr.IsPtr() && rcvr.Elem() == methodrcvr {
		...
		n.Nbody.Set1(call) // n意为 if(nthis == nil) {panicwrap()}
		fn.Nbody.Append(n)
	}

	dot := adddot(nodSym(OXDOT, nthis, method.Sym)) // nthis.method
	...
	if !instrumenting && rcvr.IsPtr() && methodrcvr.IsPtr() && method.Embedded != 0 && !isifacemethod(method.Type) && !(thearch.LinkArch.Name == "ppc64le" && Ctxt.Flag_dynlink) {
		...
		dot = dot.Left // skip final .M     receiver的ONAME节点
		// TODO(mdempsky): Remove dependency on dotlist.
		if !dotlist[0].field.Type.IsPtr() {
			dot = nod(OADDR, dot, nil)
		}
		as := nod(OAS, nthis, convnop(dot, rcvr)) // nthis = receiver
		fn.Nbody.Append(as)                       // 生成一个跳转到原函数的指令
		fn.Nbody.Append(nodSym(ORETJMP, nil, methodSym(methodrcvr, method.Sym)))
	} else {
		fn.Func.SetWrapper(true)     // ignore frame for panic+recover matching
		call := nod(OCALL, dot, nil) // 生成一个函数调用指令
		call.List.Set(paramNnames(tfn.Type))
		call.SetIsDDD(tfn.Type.IsVariadic())
		if method.Type.NumResults() > 0 { // 存在返回参数
			n := nod(ORETURN, nil, nil) // 这里相当于是返回了call这个函数调用节点的返回值
			n.List.Set1(call)
			call = n
		}
		fn.Nbody.Append(call) // 添加到生成的函数中
	}
	...
	fn = typecheck(fn, ctxStmt) // 对生成的函数进行类型检查
	...
	if rcvr.IsPtr() && rcvr.Elem() == method.Type.Recv().Type && rcvr.Elem().Sym != nil { // 通过指针调用函数但是这个函数声明时receiver不含指针
		inlcalls(fn)
	}
...
func implements(t, iface *types.Type, m, samename **types.Field, ptr *int) bool {
	...
	if t.IsInterface() {
		...
		for _, im := range iface.Fields().Slice() {
			...
			if i == len(tms) { // 表示t未全部实现iface中的所有函数
				*m = im
				*samename = nil
				*ptr = 0
				return false
			...
			tm := tms[i]
			if !types.Identical(tm.Type, im.Type) { // 两个interface拥有相同的字段,但是字段的实际类型并不相同
				*m = im
				*samename = tm
				*ptr = 0
				return false
	...
	// 现在判断t的底层类型是否继承了iface接口
	t = methtype(t) // 获取t的底层类型
	var tms []*types.Field
	if t != nil {
		expandmeth(t)
	...
	i := 0
	for _, im := range iface.Fields().Slice() { // 遍历接口的所有函数fields
		if im.Broke() {
			continue
		}
		for i < len(tms) && tms[i].Sym != im.Sym { //循环比较子类与interface的field直到发现相同的field
			i++
		}
		if i == len(tms) { // 判断如果是子类的方法Sym循环完毕
			*m = im
			*samename, _ = ifacelookdot(im.Sym, t, true)
			*ptr = 0
			return false
		}
		tm := tms[i]                                                // 实际的field，在子类实现
		if tm.Nointerface() || !types.Identical(tm.Type, im.Type) { // 判断两个具有相同符号的两个函数类型是否一致
			*m = im
			*samename = tm
			*ptr = 0
			return false
		...
		rcvr := tm.Type.Recv().Type                                               // 获取函数的receiver类型
		if rcvr.IsPtr() && !t0.IsPtr() && !followptr && !isifacemethod(tm.Type) { // t0类型不是指针类型却有recv函数且该函数调用过程不涉及到指针寻址且函数不是接口所声明的
			if false && Debug['r'] != 0 {
...
func ifaceData(pos src.XPos, n *Node, t *types.Type) *Node {
	...
	ptr.Type = types.NewPtr(t) // 将t转为指针类型
	ptr.SetTypecheck(1)
	ind := nodl(pos, ODEREF, ptr, nil) // 对指针解引用获取实际的data
	ind.Type = t
	ind.SetTypecheck(1)
	ind.SetBounded(true)
	return ind
