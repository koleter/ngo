...
func typecheckinl(fn *Node) {
	lno := setlineno(fn) // 将行号设置为fn的行号并将设置前的行号返回并保存到lno中
...
func caninl(fn *Node) {
	...
	n := fn.Func.Nname
	if n.Func.InlinabilityChecked() { // 已经判断了函数是否可内联了
		return
	}
	defer n.Func.SetInlinabilityChecked(true) // 延迟设置
	...
	n.Func.Inl = &Inline{ // 记录函数可以内联,n.Func.Inl！=nil
		Cost: inlineMaxBudget - visitor.budget, // 内联该函数的代价
...
type hairyVisitor struct {
	budget        int32 // 应该是记录最大的内联消耗上限的
	reason        string
	extraCallCost int32          // 额外的函数调用花费
	usedLocals    map[*Node]bool // 记录局部变量节点是否在内联中被使用
}
...
func (v *hairyVisitor) visit(n *Node) bool {
	...
	case OCALLFUNC: // Left(List/Rlist) (function call f(args))
		...
		if n.Left.Op == ONAME && n.Left.Class() == PFUNC && isRuntimePkg(n.Left.Sym.Pkg) { // 调用的函数是runtime包下的
			fn := n.Left.Sym.Name // 获取函数名
		...
		if isIntrinsicCall(n) { // 编译器生成的函数就跳出switch
			// Treat like any other node.
			break
		}

		if fn := n.Left.Func; fn != nil && fn.Inl != nil { // fn已经被内联处理过了
			v.budget -= fn.Inl.Cost
			break
		}
		if n.Left.isMethodExpression() { // T.M
...
func inlcopy(n *Node) *Node {
	...
	switch n.Op {
	case ONAME, OTYPE, OLITERAL: // 变量,类型,字面量不进行深度拷贝
		return n
	}
...
func inlnode(n *Node, maxCost int32, inlMap map[*Node]bool) *Node { // n为被内联的函数节点,maxCost是指内联该函数的最大花费
	if n == nil {
		return n
	}
	...
	switch n.Op {
	case OCALLFUNC, OCALLMETH:
		if n.NoInline() { // 如果函数调用被标记不会被内联就直接返回这个函数节点
			return n
		}
	}
	...
	case OCALLFUNC: // Left(List/Rlist) (function call f(args))
		...
		if n.Left.Func != nil && n.Left.Func.Inl != nil && !isIntrinsicCall(n) { // normal case		当前节点是一个被内联的函数调用而且该函数的代码不是通过编译器生成的
			...
			if d := n.Left.Name.Defn; d.Op == OAS && d.Right.Op == OCLOSURE { // 某个变量被赋值为一个闭包函数
				if f := inlinableClosure(d.Right); f != nil {
					...
					if d.Left.Name.Addrtaken() { // 表示取了某个闭包函数的地址
						if Debug['m'] > 1 {
...
func tinlvar(t *types.Field, inlvars map[*Node]*Node) *Node {
	if n := asNode(t.Nname); n != nil && !n.isBlank() {
		inlvar := inlvars[n] // 获取receiver被内联的变量并返回
		if inlvar == nil {
...
func mkinlcall(n, fn *Node, maxCost int32, inlMap map[*Node]bool) *Node { // n是被内联的函数节点,fn是n.Left
	if fn.Func.Inl == nil {
		if logopt.Enabled() {
			logopt.LogOpt(n.Pos, "cannotInlineCall", "inline", Curfn.funcname(),
	...
	if fn.Func.Inl.Cost > maxCost { // 函数内联的代价过大,停止内联
	...
	inlvars := make(map[*Node]*Node) // 原变量与内联后的闭包变量(被转为局部变量)的映射
	...
	if fn.Name.Defn != nil {
		if c := fn.Name.Defn.Func.Closure; c != nil { // c为闭包函数的外部函数,fn为闭包函数
			for _, v := range c.Func.Closure.Func.Cvars.Slice() { // 遍历闭包函数捕获的变量
				if v.Op == OXXX {
					continue
				}

				o := v.Name.Param.Outer // outer variable
				...
				if v.Name.Byval() { // 变量v为值传递
					iv := typecheck(inlvar(v), ctxExpr)               // 根据v创建一个局部变量并添加到当前的函数的局部变量节点声明列表中
					ninit.Append(nod(ODCL, iv, nil))                  // 调用函数前先将闭包变量声明
					ninit.Append(typecheck(nod(OAS, iv, o), ctxStmt)) // 闭包变量声明后的赋值
					inlvars[v] = iv
				} else {
					addr := newname(lookup("&" + v.Sym.Name))
					addr.Type = types.NewPtr(v.Type)                                   // v的指针类型
					ia := typecheck(inlvar(addr), ctxExpr)                             // 依据v的指针类型创建一个新的局部变量
					ninit.Append(nod(ODCL, ia, nil))                                   // 对该局部变量进行声明
					ninit.Append(typecheck(nod(OAS, ia, nod(OADDR, o, nil)), ctxStmt)) // ia = &o
					inlvars[addr] = ia                                                 // 记录addr被内联后的变量为ia

					// When capturing by reference, all occurrence of the captured var
					// must be substituted with dereference of the temporary address
					inlvars[v] = typecheck(nod(ODEREF, ia, nil), ctxExpr) // 记录v被内联后的变量
				}
			}
		}
	}
	...
	for i, t := range fn.Type.Results().Fields().Slice() {
		var m *Node
		mpos := t.Pos
		if n := asNode(t.Nname); n != nil && !n.isBlank() { // 存在实名的返回变量节点
			m = inlvar(n) // 将返回参数内联
		...
		if genDwarfInline > 0 {
			...
			if !strings.HasPrefix(m.Sym.Name, "~R") { // 不处理blank返回参数
		...
		ninit.Append(nod(ODCL, m, nil)) // 声明内联变量m
		retvars = append(retvars, m)    // 将内联后的变量添加到retvars中
	}

	// Assign arguments to the parameters' temp names.
	as := nod(OAS2, nil, nil)
	as.Rlist.Set(n.List.Slice()) // n的实参用于赋值,List中也包含recv,相当于赋值,令形参列表等于实参列表

	// For non-dotted calls to variadic functions, we assign the
	// variadic parameter's temp name separately.
	var vas *Node

	if fn.IsMethod() { // 存在receiver
		rcv := fn.Type.Recv() // 获取函数的receiver

		if n.Left.Op == ODOTMETH { // Left.Sym (Left is non-interface, Right is method name)
			// For x.M(...), assign x directly to the
			// receiver parameter.
			if n.Left.Left == nil {
				Fatalf("method call without receiver: %+v", n)
			}
			ras := nod(OAS, tinlvar(rcv, inlvars), n.Left.Left) // 将实参赋值给内联后的节点
			...
			as.List.Append(tinlvar(rcv, inlvars)) // 为receiver内联后的节点进行赋值
		}
	}

	for _, param := range fn.Type.Params().Fields().Slice() { // 遍历处理函数形参
		// For ordinary parameters or variadic parameters in
		// dotted calls, just add the variable to the
		// assignment list, and we're done.
		if !param.IsDDD() || n.IsDDD() { // 当前的形参没有...或者函数最后一个实参有...
			as.List.Append(tinlvar(param, inlvars)) // 内联函数的形参直接替换为实参
			continue
		}

		// Otherwise, we need to collect the remaining values
		// to pass as a slice.
		// 以下表示当前的形参中有...但是实参中没有...,需要将实参整理为一个切片的情况
		numvals := n.List.Len() // 获取当前函数的参数的数量

		x := as.List.Len()            // 记录...参数开始的下标,从这里开始将其余的参数整理为一个切片
		for as.List.Len() < numvals { // 将所有实参加入到as.List
			as.List.Append(argvar(param.Type, as.List.Len())) // 将形参添加到List中
		}
		varargs := as.List.Slice()[x:] // 取出所有将被组合为slice的参数构成varargs切片

		vas = nod(OAS, tinlvar(param, inlvars), nil)
		if len(varargs) == 0 { // slice中其实是一个空切片
			vas.Right = nodnil() // 形参赋值为nil
			vas.Right.Type = param.Type
		} else {
			vas.Right = nod(OCOMPLIT, nil, typenod(param.Type)) // 形参赋值为形参类型的复合字面量		Right{List} (composite literal, not yet lowered to specific form)
	...
	for _, n := range retvars { // 如果存在定义了变量名的返回参数,设置为nil
		ras := nod(OAS, n, nil) // 将临时返回变量初始化为nil
		ras = typecheck(ras, ctxStmt)
		ninit.Append(ras)
	}

	retlabel := autolabel(".i") // 在当前函数中自动生成一个label
	...
	newIndex := Ctxt.InlTree.Add(parent, n.Pos, fn.Sym.Linksym()) // newIndex为当前被内联的函数在全局内联树中的index
...
func inlvar(var_ *Node) *Node {
	...
	n := newname(var_.Sym) // 根据符号创建一个新的局部变量
	n.Type = var_.Type
	n.SetClass(PAUTO)
	n.Name.SetUsed(true)
	...
	Curfn.Func.Dcl = append(Curfn.Func.Dcl, n) // 相当于当前函数添加了一个局部变量
	return n
}
...
func argvar(t *types.Type, i int) *Node {
	...
	n.Type = t.Elem() // 因为实际参数的类型是切片,所以这里获取切片元素的类型
	n.SetClass(PAUTO)
	n.Name.SetUsed(true)
	n.Name.Curfn = Curfn // the calling function, not the called one
	Curfn.Func.Dcl = append(Curfn.Func.Dcl, n)
...
// 排除ll中声明的同时被内联的局部变量,剩下的变量整理为一个切片并返回
func pruneUnusedAutos(ll []*Node, vis *hairyVisitor) []*Node {
	s := make([]*Node, 0, len(ll))
	for _, n := range ll {
		if n.Class() == PAUTO { // 如果是局部变量
			if _, found := vis.usedLocals[n]; !found {
				continue
			}
		}
		s = append(s, n) // 排除ll中声明的同时被内联的局部变量
	}
	return s
}
