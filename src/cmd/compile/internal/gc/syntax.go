...
type Node struct {
	...
	Left  *Node //如果该Node是一个函数Node的话，该成员为recv
	Right *Node
	Ninit Nodes // 在执行当前节点所定义的操作之前需要进行的操作
	Nbody Nodes // 函数体中的stamts组成的Nodes
	List  Nodes //如果该Node是一个函数Node的话，该成员为形参
	Rlist Nodes //如果该Node是一个函数Node的话，该成员为返回参数列表

	// most nodes
	Type *types.Type
	Orig *Node // original form, for printing, and tracking copies of ONAMEs	记录初始节点
	...
	Xoffset int64 // 局部变量的话该值为0

	Pos src.XPos

	flags bitset32

	Esc uint16 // EscXXX   用于记录当前的逃逸分析到了什么地步

	Op  Op // 这里的Op实际上是说明了当前Node的操作或者说类型(函数或者外部的变量声明等)
	aux uint8
...
const (
	...
	_, nodeNonNil    // guaranteed to be non-nil	保证这个指针不会为nil
...
type Name struct {
	...
	Defn      *Node      // initializing assignment	表示变量在何处被声明的  假如 a,b := range xxx, 那么a与b的Name.Defn就指向这行代码包装后的Node节点  Label的Defn指向下一行有效代码
	Curfn     *Node      // function for local variables
	Param     *Param     // additional fields for ONAME, OTYPE	记录额外的信息 会用来记录变量声明时的类型
	Decldepth int32      // declaration loop depth, increased for every loop or label
	Vargen    int32      // unique name for ONAME within a function.  Function outputs are numbered starting at one.	先处理出参列表,从1开始计数,之后是receiver,出参列表
	flags     bitset16
}
...
const (
	...
	nameIsClosureVar          // PAUTOHEAP closure pseudo-variable; original at n.Name.Defn		为true时表示是闭包中引用了外部的一个变量
	nameIsOutputParamHeapAddr // pointer to a result parameter's heap copy
	nameAssigned              // is the variable ever assigned to	是否对该变量进行了赋值操作
...
type Param struct {
	...
	Innermost *Node // 当前已知的作用域最里的闭包中的相同变量	用于最初定义的原始变量,记录在多层闭包函数中捕获该变量的最深层的那个闭包函数所在的变量
	Outer     *Node // 当前闭包外的闭包中的相同变量
...
type Func struct {
	...
	Exit      Nodes //　通过堆分配的返回参数变量,是一个个的OAS赋值语句
	Cvars     Nodes // closure params
	/*
		当前函数中声明的所有变量,receiver是第一项(如果有的话),之后是入参，再然后是出参,在之后是声明的局部变量
		如果返回值没有主动命名，就会从入参数量(设其为gen)开始(这里的入参不算receiver),生成~r + gen符号作为返回变量的名字
	*/
	...
	Closure    *Node     // OCLOSURE <-> ODCLFUNC	函数与函数间闭包的互相引用
	Nname      *Node     // 函数名符号对应的节点
	lsym       *obj.LSym // 函数名符号对应的LSym结构体

	Inl *Inline // 该成员不为nil表示可以被内联

	Label int32 // largest auto-generated label in this function

	Endlineno src.XPos // 函数的最后一行
...
type Inline struct {
	Cost int32 // heuristic cost of inlining this function	 内连该函数的花费

	// Copies of Func.Dcl and Nbody for use during inlining.
	Dcl  []*Node // 排除局部被内联的变量之后,函数声明的变量
	Body []*Node // 相当于内联前函数节点中的Nbody,存储函数体节点
}
...
const (
	...
	funcIsHiddenClosure                      // set表示当前函数为函数中的函数，所谓闭包
...
const (
	...
	OAS2DOTTYPE   // List = Right (x, ok = I.(int))		I是一个接口
	...
	ODOTTYPE       // Left.Right or Left.Type (.Right during parsing, .Type once resolved); after walk, .Right contains address of interface type descriptor and .Right contains address of concrete type descriptor
	...
	OIOTA        // iota
	OREAL        // real(Left)	返回复数的实部
	OIMAG        // imag(Left)	返回复数的虚部
	OCOMPLEX     // complex(Left, Right) or complex(List[0]) where List[0] is a 2-result function call	     complex(1, 2) // 1+2i
	...
	OVARLIVE    // variable is alive		Left必须是局部变量或者入参或实参
...
func (n *Nodes) copy() Nodes {
	...
	retSlice := make([]*Node, 0, len(*old.slice))
	for _, node := range *old.slice {
		retSlice = append(retSlice, node.copy())
	}
	return Nodes{&retSlice}
}
