...
type iimporterAndOffset struct {
	p   *iimporter
	off uint64	// 记录了某个符号位于p.declData中的偏移
}
...
func expandInline(fn *Node) {
	if fn.Func.Inl.Body != nil {	// 被内联的函数没有函数体的话就返回
		return
	}
...
func importReaderFor(n *Node, importers map[*types.Sym]iimporterAndOffset) *importReader {
	...
	return x.p.newReader(x.off, n.Sym.Pkg)	// 从x的off处开始写内联函数
}
...
func iimport(pkg *types.Pkg, in *bio.Reader) (fingerprint goobj2.FingerprintType) {
	...
	in.MustSeek(int64(sLen+dLen), os.SEEK_CUR)	//此处fmt.a的位置在0x2c4a处
...
type iimporter struct {
	ipkg *types.Pkg		// 表示该结构体代表的所导入的Pkg

	pkgCache     map[uint64]*types.Pkg
	posBaseCache map[uint64]*src.PosBase	// 表示你stringdata中的偏移与PosBase的映射，用于cache
	typCache     map[uint64]*types.Type

	stringData string	// 二进制文件中的字符串声明
	declData   string	// 猜测是一个二进制文件中的可执行的内容
}
...
func (p *iimporter) stringAt(off uint64) string {
	...
	slen, n := binary.Uvarint(x[:n])	// n表示从x[0]以Uvarint算法读取的数值所使用的字节数，slen表示读取的值，为string的长度
	if n <= 0 {
...
func (p *iimporter) posBaseAt(off uint64) *src.PosBase {
	...
	file := p.stringAt(off)	// 获取符号所在的文件位置
	posBase := src.NewFileBase(file, file)
	p.posBaseCache[off] = posBase
	return posBase
}
// off为stringData中的偏移，用于找到包字符串
...
type importReader struct {
	strings.Reader
	p *iimporter	// 指向包含该importReader结构体的iimport对象
