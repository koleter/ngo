...
// 相当于o对于r向上取整，这里的r值有限制
func Rnd(o int64, r int64) int64 {
	if r < 1 || r > 8 || r&(r-1) != 0 { // r需要在1到8之间且为2的幂次方
		Fatalf("rnd %d", r)
	}
	return (o + r - 1) &^ (r - 1)
}
...
func expandiface(t *types.Type) {
	...
	var methods []*types.Field // 保存interface中所声明的函数字段
	// m表示一个函数字段,explicit表示是否是显示声明的
	...
	for _, m := range t.Methods().Slice() {
		if m.Sym == nil { // interface类型中声明的函数,Sym相当于函数名
			continue
		}
	...
	for _, m := range t.Methods().Slice() {
		if m.Sym != nil { // 没有函数名,表示为嵌入式的字段类型
			continue
		}

		if !m.Type.IsInterface() { // interface中只能嵌入interface
		...
		// method set.
		for _, t1 := range m.Type.Fields().Slice() { // 遍历嵌入类型的interface所拥有的所有方法添加到当前的interface中(如果没有重名)
			f := types.NewField()
...
func widstruct(errtype *types.Type, t *types.Type, o int64, flag int) int64 {
	...
	for _, f := range t.Fields().Slice() {
		...
		dowidth(f.Type)                     // 计算字段的宽度
		if int32(f.Type.Align) > maxalign { //用最大的那个字段进行对齐
...
func resumecheckwidth() {
	if defercalc == 1 {
		for len(deferredTypeStack) > 0 {
			t := deferredTypeStack[len(deferredTypeStack)-1] // 弹出最后一项
