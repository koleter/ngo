...
var imported_unsafe bool // 是否引入了unsafe包

var (
	buildid      string
	spectre      string
	spectreIndex bool // 是否开启幽灵索引模式
)
...
func Main(archInit func(*Arch)) {
	...
	if ssaDump != "" {
		if strings.HasSuffix(ssaDump, "+") { // GOSSAFUNC环境变量指定的函数名由+结尾
			ssaDump = ssaDump[:len(ssaDump)-1]
			ssaDumpStdout = true
		}
		spl := strings.Split(ssaDump, ":")
		if len(spl) > 1 { // ssaDump中存在:字符
			ssaDump = spl[0]
			ssaDumpCFG = spl[1]
		}
	}
	...
	lines := parseFiles(flag.Args()) // lines记录编译的所有文件的总行数
	timings.Stop()
	...
	timings.Start("fe", "aop")
	for _, n := range xtop {
		if op := n.Op; op == ODCLFUNC { // 46, 34
			Curfn = n // 记录当前所处的函数
			doAopHandler(n)
		}
	}
	...
	var fcount int64 // 记录函数及闭包的总数量
	for i := 0; i < len(xtop); i++ {
		n := xtop[i]
		if op := n.Op; op == ODCLFUNC || op == OCLOSURE { // 46, 34
			Curfn = n // 记录当前所处的函数
			decldepth = 1
			saveerrors()
			...
			deadcode(Curfn) // 对函数中明显的无用代码进行删除
			fcount++
		}
	}
	...
	for _, n := range xtop {
		if n.Op == ODCLFUNC && n.Func.Closure != nil { // n是一个包含闭包的函数
			Curfn = n
			capturevars(n) // 整理被捕获的变量并分析通过值传递还是引用传递
		}
	}
	...
	if Debug['l'] != 0 {
		...
		visitBottomUp(xtop, func(list []*Node, recursive bool) {
			numfns := numNonClosures(list) // 获取list中不包含闭包的函数数量,根据visitBottomUp中的分析,这个list列表中应该是没有闭包函数的
			for _, n := range list {       // 遍历list中的函数声明节点并分别进行内联优化
				if !recursive || numfns > 1 { // 不是函数递归列表或者函数递归列表中不包含闭包的函数数量大于1
					// We allow inlining if there is no
					// recursion, or the recursion cycle is
					// across more than one function.
					caninl(n) // 判断函数节点n是否满足被内联的条件
				} else {
					if Debug['m'] > 1 {
	...
	for _, n := range xtop {
		if n.Op == ODCLFUNC && n.Func.Closure != nil { // 拥有闭包的函数声明
			Curfn = n
			transformclosure(n)
		}
	}
...
func importfile(f *Val) *types.Pkg {
	...
	file, found := findpkg(path_) // 找到import的package对应的文件路径
	if !found {
	...
	case 'B':
		...
		fingerprint = iimport(importpkg, imp) // 此时fmt.a文件指针已经到了0x2d7a处
...
func clearImports() {
	...
	var unused []importedPkg // 记录导入了却没有使用过的包
	...
	for _, s := range localpkg.Syms {
		...
		if n.Op == OPACK { // 符号是通过一个包名找到的
			// throw away top-level package name left over
			// from previous file.
			// leave s->block set to cause redeclaration
			// errors if a conflicting top-level name is
			// introduced by a different file.
			if !n.Name.Used() && nsyntaxerrors == 0 { // 导入的某个包没有使用
		...
		if IsAlias(s) { // s是一个别名
			...
			if n.Name != nil && n.Name.Pack != nil && !n.Name.Pack.Name.Used() && nsyntaxerrors == 0 { // 找到未使用变量所在包的实际定义时的名字
...
func recordPackageName() {
	s := Ctxt.Lookup(dwarf.CUInfoPrefix + "packagename." + myimportpath) // 根据字符串获取LSym
	s.Type = objabi.SDWARFINFO
	// Sometimes (for example when building tests) we can link
	// together two package main archives. So allow dups.
	s.Set(obj.AttrDuplicateOK, true)
...
func langSupported(major, minor int, pkg *types.Pkg) bool {
	...
	if langWant.major == 0 && langWant.minor == 0 { // 未通过-lang设置希望的语言版本
		return true
	}
...
func doAopHandler(n *Node) {
	autotmpgen = 0
	var genfunc *Node
	lineno = n.Pos
	// 代表AOP中作为CutPoint的参数所代表的节点
	var cutpoint *Node
	var afterNode, params []*Node
	var recrNum, paramNum, paramOutNum int
	var setKeyValue func(sym string, value *Node)
	setKeyValue = func(sym string, value *Node) {
		// cutpoint.$sym$ = $value$
		node := nodSym(OXDOT, cutpoint, lookup(sym))
		node = nod(OAS, node, value)
		n.Nbody.Append(node)
	}
	var interfaceSliceType *types.Type
	if n.Func.Nname.Name.Param.Ntype.Left != nil {
		recrNum = 1
	}
	for _, pair := range AopMap {
		// 用于匹配的函数名
		funcName := n.Func.Nname.Sym.Name

		//GenSliceLiteral = fmt.Sprintf("%-S.%s", t.Type, GenSliceLiteral)

		if !pair.reg.MatchString(funcName) {
			continue
		}
		if genfunc == nil {
			// 创建一个与n相同的函数
			genfunc = nod(ODCLFUNC, nil, nil)
			// 函数对应的ONAME节点
			genfunc.Func.Nname = newfuncnamel(lineno, localpkg.Lookup(fmt.Sprintf("%s%d", "~tmpfunc", funcgen)))
			declareAutoNode(genfunc.Func.Nname, PFUNC)

			funcgen++
			genfunc.Func.Nname.Name.Defn = genfunc
			// 赋予函数签名
			genfunc.Func.Nname.Name.Param.Ntype = n.Func.Nname.Name.Param.Ntype.copy()
			// 以下三行的copy函数用于确保参数字段类型*types.Field.Nname对应到对应的ONAME节点上
			genfunc.Func.Nname.Name.Param.Ntype.Left = n.Func.Nname.Name.Param.Ntype.Left.copy()
			genfunc.Func.Nname.Name.Param.Ntype.List = n.Func.Nname.Name.Param.Ntype.List.copy()
			genfunc.Func.Nname.Name.Param.Ntype.Rlist = n.Func.Nname.Name.Param.Ntype.Rlist.copy()

			genfunc.Nbody.Set(n.Nbody.Slice())
			//genfunc.Func.Dcl = n.Func.Dcl
			// curfn.Type.Results().FieldSlice()[i].Nname
			for _, node := range n.Func.Dcl {
				node.Name.Curfn = genfunc
				genfunc.Func.Dcl = append(genfunc.Func.Dcl, node)
			}

			// 添加函数声明节点到xtop中,类型检查会先处理函数声明
			xtop = append(xtop, genfunc)

			// 清空函数n的函数体以及变量
			n.Nbody.Set(nil)
			paramOutNum = len(n.Func.Nname.Name.Param.Ntype.Rlist.Slice())
			paramNum = len(n.Func.Nname.Name.Param.Ntype.List.Slice())
			limit := recrNum + paramNum + paramOutNum
			for i := 0; i < limit; i++ {
				tmpNode := genfunc.Func.Dcl[i].copy()
				tmpNode.Name = genfunc.Func.Dcl[i].Name.copy()
				tmpNode.Name.Curfn = n
				n.Func.Dcl[i] = tmpNode
			}

			n.Func.Dcl = n.Func.Dcl[:limit:limit]
			params = n.Func.Dcl[:recrNum+paramNum]

			ntype := n.Func.Nname.Name.Param.Ntype
			if recrNum == 1 {
				ntype.Left.Right = n.Func.Dcl[0]
			}
			for i := 0; i < paramNum; i++ {
				ntype.List.Slice()[i].Right = n.Func.Dcl[recrNum+i]
			}
			for i := 0; i < paramOutNum; i++ {
				ntype.Rlist.Slice()[i].Right = n.Func.Dcl[recrNum+paramNum+i]
			}

			// 下面开始构造aop.CutPoint结构体
			// 先打包函数的参数到args中
			interfaceSliceType = types.NewSlice(types.Types[TINTER])

			args := temp(interfaceSliceType)
			// 将params包装成一个interface切片字面量,创建临时变量作为一个列表
			inParams := make([]*Node, recrNum+paramNum)
			for i := 0; i < recrNum+paramNum; i++ {
				tmp := newname(lookupN(".aoptmp", autotmpgen))
				autotmpgen++
				declareAutoNode(tmp, PAUTO)
				oas := GenOAS(tmp, params[i])
				n.Nbody.Append(oas)
				inParams[i] = tmp
			}
			complit := GenSliceLiteral(OTINTER, inParams)
			node := nod(OAS, args, complit)
			node.SetColas(true)
			n.Nbody.Append(node)

			complit = nod(OCOMPLIT, nil, oldname(restrictlookup("CutPoint", types.NewPkg("aop", ""))))
			cutpoint = newname(lookupN(".aoptmp", autotmpgen))
			node = nod(OAS, cutpoint, nod(OADDR, complit, nil))
			node.Ninit.Append(nod(ODCL, cutpoint, nil))
			declareAutoNode(cutpoint, PAUTO)
			cutpoint.Name.Defn = node
			autotmpgen++

			node.SetColas(true)
			n.Nbody.Append(node)
			// 设置CutPoint的key-value对

			// cutpoint.Args = args
			setKeyValue("Args", args)
			// cutpoint.FuncName = <FuncName>
			setKeyValue("FuncName", nodlit(Val{funcName}))
		}

		for _, call := range pair.beforenodes {
			n.Nbody.Append(GenCallNode(call, cutpoint))
		}

		for _, call := range pair.afternodes {
			afterNode = append(afterNode, GenCallNode(call, cutpoint))
		}
	}
	// 如果不需要aop直接返回
	if genfunc == nil {
	...
	callGenFuncNode := GenCallNode(genfunc.Func.Nname, params...)
	if paramOutNum == 0 {
		n.Nbody.Append(callGenFuncNode)
	} else {
		// 进行函数调用并将返回值赋值
		node := nod(OAS2, nil, nil)
		for i := 0; i < paramOutNum; i++ {
			tmp := newname(lookupN(".aoptmp", autotmpgen))
			autotmpgen++
			declareAutoNode(tmp, PAUTO)
			node.List.Append(tmp)
			tmp.Name.Defn = node
		}
		node.Rlist.Set([]*Node{callGenFuncNode})
		n.Nbody.Append(node)
		results := make([]*Node, paramOutNum)
		for i := 0; i < paramOutNum; i++ {
			tmp := newname(lookupN(".aoptmp", autotmpgen))
			autotmpgen++
			declareAutoNode(tmp, PAUTO)
			oas := GenOAS(tmp, node.List.Slice()[i])
			n.Nbody.Append(oas)

			results[i] = tmp
		}
		// 将返回值构造成一个interface列表并赋给CutPoint.Results
		result := GenSliceLiteral(OTINTER, results)
		setKeyValue("Results", result)
	}
	n.Nbody.Append(afterNode...)
	returnNode := nod(ORETURN, nil, nil)
	if paramOutNum != 0 {
		// 有返回值,返回cutpoint.Results[i]强转后的结果列表
		for i := 0; i < paramOutNum; i++ {
			// 函数出参的类型节点
			t := n.Func.Nname.Name.Param.Ntype.Rlist.Slice()[i].Left
			// cutpoint.Results,一个interface切片
			resultSlice := nodSym(OXDOT, cutpoint, lookup("Results"))
			// 取切片下标元素   cutpoint.Results[i]
			ele := nod(OINDEX, resultSlice, nodlit(basicLit(&syntax.BasicLit{Value: fmt.Sprintf("%d", i)})))
			// 类型强转
			stmt := nod(ODOTTYPE, ele, t)
			returnNode.List.Append(stmt)
		}
	}
	n.Nbody.Append(returnNode)
}

// declare records that Node n declares symbol n.Sym in the specified
// declaration context.
func declareAutoNode(n *Node, ctxt Class) {
	if n.isBlank() { // _
		return
	}
