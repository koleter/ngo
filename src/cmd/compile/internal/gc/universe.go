...
func lexinit() {
	for _, s := range &basicTypes {		
		...
		s2 := builtinpkg.Lookup(s.name)	//寻找或者创建Sym
		t := types.Types[etype]	//从Types数组中取出Type
		if t == nil {
			t = types.New(etype)
			t.Sym = s2
			if etype != TANY && etype != TSTRING {
				dowidth(t)	//计算t的width以及align
			}
			types.Types[etype] = t
		}
...
func finishUniverse() {
	...
	for _, s := range builtinpkg.Syms {
		if s.Def == nil {	// builtinpkg中的符号如果没有定义就跳过
			continue
		}
		s1 := lookup(s.Name)	// 在当前包中根据字符串获取定义的符号
		if s1.Def != nil {	// 已经定义过了的符号不报错而是跳过
			continue
		}

		s1.Def = s.Def	// 将本包中的符号与builtinpkg.Syms中的符号进行关联
		s1.Block = s.Block
	}
