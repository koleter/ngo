...
func convlit1(n *Node, t *types.Type, explicit bool, context func() string) *Node {
	if explicit && t == nil {	// 显示类型转换类型必须给出
	...
	if !n.Type.IsUntyped() {	// n有明确的类型,不必转化
		// Already typed; nothing to do.
		return n
	}

	if n.Op == OLITERAL {	// n是字面量
		// Can't always set n.Type directly on OLITERAL nodes.
		// See discussion on CL 20813.		1 = 2会出问题
		n = n.rawcopy()		// 创建一个n的浅拷贝
	}
	...
	if n.Type.Etype == TNIL {
		...
		if !t.HasNil() {	// 如果t的类型不可以使用nil进行赋值
...
func convertVal(v Val, t *types.Type, explicit bool) Val {
	switch ct := v.Ctype(); ct {	// 判断v这个常量的类型
	case CTBOOL:
		if t.IsBoolean() {
			return v
		}
...
func overflow(v Val, t *types.Type) bool {
	...
	if t == nil || t.Etype == TIDEAL {	// TIDEAL: 29
		return false
	}
...
func evconst(n *Node) {
	...
	switch op := n.Op; op {
	case OPLUS, ONEG, OBITNOT, ONOT:	// 一元运算
		if nl.Op == OLITERAL {
...
func match(x, y Val) (Val, Val) {
	switch {
	case x.Ctype() == CTCPLX || y.Ctype() == CTCPLX:	// 有一个以上是组合类型
		return tocplx(x), tocplx(y)
	case x.Ctype() == CTFLT || y.Ctype() == CTFLT:	// 有一个以上是浮点类型
...
// op是节点n的操作符,x是节点n的左子节点的Val, t是节点n的type
...
func setconst(n *Node, v Val) {
	...
	if n.Orig == n {	// n就是初始的节点，不是从某个其他节点拷贝而来的
		n.Orig = n.sepcopy()	// 对n进行浅拷贝
	}
...
func defaultlit2(l *Node, r *Node, force bool) (*Node, *Node) {
	...
	if !l.Type.IsUntyped() {
		r = convlit(r, l.Type)	// 右节点转为左节点的类型
		return l, r
	}

	if !r.Type.IsUntyped() {
		l = convlit(l, r.Type)	// 左节点转为右节点的类型
		return l, r
	}

	if !force {		// 左右节点都不是Untyped类型,不强制转换,直接返回
		return l, r
	}
...
func indexconst(n *Node) int64 {
	...
	vi, ok := v.U.(*Mpint)
	if !ok || vi.CmpInt64(0) < 0 {	// 不是*Mpint类型且小于0
		return -1
	}
	if vi.Cmp(maxintval[TINT]) > 0 {	// 值溢出返回-2
		return -2
	}
