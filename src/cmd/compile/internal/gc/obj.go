...
func dumpobj1(outfile string, mode int) {
	...
	if mode&modeCompilerObj != 0 {
		...
		finishArchiveEntry(bout, start, "__.PKGDEF") // 到这里已经输出了编译信息
	}
	if mode&modeLinkerObj != 0 {
		start := startArchiveEntry(bout) // 填充60个字节
		dumpLinkerObj(bout)
...
func startArchiveEntry(bout *bio.Writer) int64 {
	...
	bout.Write(arhdr[:]) // 先写入60个字节用于开辟存储空间
	return bout.Offset()
}
...
func addptabs() {
	...
	for _, exportn := range exportlist {
		...
		if !types.IsExported(s.Name) { // 判断不能导出
			continue
		}
		if s.Pkg.Name != "main" { // main包
			continue
		}
...
func dsymptr(s *obj.LSym, off int, x *obj.LSym, xoff int) int {
	off = int(Rnd(int64(off), int64(Widthptr))) // off根据Widthptr对齐
