...
func cmpstackvarlt(a, b *Node) bool {
	if (a.Class() == PAUTO) != (b.Class() == PAUTO) {
		return b.Class() == PAUTO // 非局部变量在前面
	}

	if a.Class() != PAUTO { // 两个变量都不是局部变量
		return a.Xoffset < b.Xoffset // 偏移小的非局部变量在前面
	}

	if a.Name.Used() != b.Name.Used() {
		return a.Name.Used() // 被使用的局部变量在未被使用的局部变量前面
	}
...
func (s *ssafn) AllocFrame(f *ssa.Func) {
	...
	// 标记所有values中是否出现了需要临时内存空间的value
	scratchUsed := false
	...
	lastHasPtr := false // 标记上一个局部变量的类型是否包含指针
	for i, n := range fn.Dcl {
		if n.Op != ONAME || n.Class() != PAUTO { // 找到局部变量
			continue
		}
		if !n.Name.Used() { // 当前的局部变量未被使用
			fn.Dcl = fn.Dcl[:i] // 从当前未被使用的局部变量截断,因为上面已经排好序了,后面的都是没有使用的局部变量
			break
		}
		...
		if w == 0 && lastHasPtr { // 类型宽度为0且上一个变量类型中包含指针
		...
		// 在以下架构中每个变量的存储必须按照指针长度对齐
		if thearch.LinkArch.InFamily(sys.MIPS, sys.MIPS64, sys.ARM, sys.ARM64, sys.PPC64, sys.S390X) {
			s.stksize = Rnd(s.stksize, int64(Widthptr))
		}
		n.Xoffset = -s.stksize // 设置变量的偏移,rbp-s.stksize
	}

	s.stksize = Rnd(s.stksize, int64(Widthreg))       // 栈帧大小按照指针长度对齐
	s.stkptrsize = Rnd(s.stkptrsize, int64(Widthreg)) // 栈帧中包含指针的那部分也按照指针长度对齐
}
...
func compile(fn *Node) {
	...
	walk(fn) // 遍历抽象语法树,将statement的根节点以下的所有子节点的init节点添加到这个根节点中
	if nerrors != 0 {
		return
	}
	if instrumenting {
	...
	for _, n := range fn.Func.Dcl { // 遍历函数声明的局部变量
		switch n.Class() {
	...
	if compilenow(fn) {
		...
		compilequeue = append(compilequeue, fn) // 加入编译队列等待compileFunctions消费
	}
}
...
func compileSSA(fn *Node, worker int) {
	f := buildssa(fn, worker) // 抽象语法树转为ssa并进行52个步骤的优化
