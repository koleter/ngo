...
func iexport(out *bufio.Writer) {
	...
	hdr.uint64(uint64(p.strings.Len()))	// string域的字节数
	hdr.uint64(dataLen)	// declData的字节数
...
func (w *exportWriter) writeIndex(index map[*Node]uint64, mainIndex bool) {
	...
	w.uint64(uint64(len(pkgs)))	// 写出引用的包的数量
	for _, pkg := range pkgs {
		w.string(pkg.Path)	// 写出包路径在string域的偏移
		if mainIndex {
			w.string(pkg.Name)	// 写出包名在string域的偏移
			w.uint64(uint64(pkg.Height))	// 写出包的Height
		}

		objs := pkgObjs[pkg]
		w.uint64(uint64(len(objs)))	// 写出这个包包含的可导出的全局声明节点数量
		for _, n := range objs {
			w.string(n.Sym.Name)	// 写出节点的符号名
			w.uint64(index[n])	// 写出该符号在数据域中的偏移
		}
	}
}
...
type iexporter struct {
	...
	declTodo nodeQueue	// 临时存储全局声明的一个队列

	strings     intWriter
	stringIndex map[string]uint64	// 记录一个string是string域的第几个字符串,这个字符串先记录了字符串长度,之后根据长度读取该字符串

	data0       intWriter		// 数据域
	declIndex   map[*Node]uint64	// 某个全局声明的节点在数据域中的偏移
	inlineIndex map[*Node]uint64	// inline节点对应的偏移
	typIndex    map[*types.Type]uint64	// 某种类型对应predecl数组的index
}
...
func (p *iexporter) stringOff(s string) uint64 {
	...
	if !ok {
		...
		p.strings.uint64(uint64(len(s)))	// 先写入字符串的长度
		p.strings.WriteString(s)	// 再写入字符串
	}
	return off
}
...
type exportWriter struct {
	...
	data       intWriter	// 数据域
	currPkg    *types.Pkg	// 当前写出的包
	prevFile   string	// 上一次编译的文件名
	prevLine   int64	// 上一次编译的节点行号
	prevColumn int64	// 上一次编译的节点列号
}
...
func (w *exportWriter) pos(pos src.XPos) {
	...
	file := p.Base().AbsFilename()	// pos所记录的文件的绝对路径
	line := int64(p.RelLine())	// 实际的行号
	column := int64(p.RelCol())	// 实际的列号
	...
	if file != w.prevFile {	// 文件变了就将deltaLine第一个bit位置1
		deltaLine |= 1
	}
	if deltaLine != 0 {	// 文件或者行号有变化就将deltaColumn第一个bit位置1
		deltaColumn |= 1
	}

	w.int64(deltaColumn)	// 将deltaColumn写出到数据域
	if deltaColumn&1 != 0 {		// 文件或者行号改变了
		w.int64(deltaLine)	// 将deltaLine写出到数据域
		if deltaLine&1 != 0 {	// 文件改变了
			w.string(file)	// 将变化后的文件名在string域中的index写入数据域
		}
	}
...
func (w *exportWriter) doTyp(t *types.Type) {
	...
	case TSTRUCT:
		w.startType(structType)	// 写入一个数字到data域以区分类型
		w.setPkg(t.Pkg(), true)
...
func (w *exportWriter) param(f *types.Field) {
	w.pos(f.Pos)
	w.localIdent(origSym(f.Sym), 0)	// 写出f.Sym的原始Sym代表的字符串
	w.typ(f.Type)
}
