	...
	for _, ncase := range sel.List.Slice() {
		...
		if ncase.List.Len() == 0 {
			...
			case OAS:	// Left = Right or (if Colas=true) Left := Right
				...
				n.Op = OSELRECV	 // Left = <-Right.Left: (appears as .Left of OCASE; Right.Op == ORECV)
			...
			case OAS2RECV:	// List = Right (x, ok = <-c)
				...
				n.Op = OSELRECV2	// List = <-Right.Left: (appears as .Left of OCASE; count(List) == 2, Right.Op == ORECV)
				n.Left = n.List.First()
				n.List.Set1(n.List.Second())

				// convert <-c into OSELRECV(N, <-c)
			case ORECV:
				n = nodl(n.Pos, OSELRECV, nil, n)	// Left = <-Right.Left: (appears as .Left of OCASE; Right.Op == ORECV)
...
func walkselectcases(cases *Nodes) []*Node {
	...
	if n == 1 {	// 只有一个case
		...
		if cas.Left != nil { // not default:
			n := cas.Left	// 获取case之后的表达式
			l = append(l, n.Ninit.Slice()...)
			n.Ninit.Set(nil)
			var ch *Node	// 记录channel节点
			switch n.Op {
			default:
			...
			case OSELRECV, OSELRECV2:	 // Left = <-Right.Left: (appears as .Left of OCASE; Right.Op == ORECV), // List = <-Right.Left: (appears as .Left of OCASE; count(List) == 2, Right.Op == ORECV)
				...
				if n.Op == OSELRECV || n.List.Len() == 0 {
					if n.Left == nil {	// <-Right.Left:
						n = n.Right
					} else {
						n.Op = OAS	// Left = <-Right.Left:
					}
					break
				}
				...
				n.Op = OAS2	// List = Rlist (x, y, z = a, b, c)
	...
	for _, cas := range cases.Slice() {
		setlineno(cas)
		n := cas.Left	// 获取case之后的expr
		if n == nil {
			continue
		}
		switch n.Op {
		case OSEND:	// Left <- Right
			n.Right = nod(OADDR, n.Right, nil)	// 其实放到管道中的只是一个变量的地址
	...
	if n == 2 && (cases.First().Left == nil || cases.Second().Left == nil) {
		...
		case OSELRECV:	// Left = <-Right.Left: (appears as .Left of OCASE; Right.Op == ORECV)
		...
		case OSELRECV2:	// List = <-Right.Left: (appears as .Left of OCASE; count(List) == 2, Right.Op == ORECV)
	...
	order := temp(types.NewArray(types.Types[TUINT16], 2*int64(n)))	// 2倍的case数量长度的uint16数组
	r = nod(OAS, order, nil)
	r = typecheck(r, ctxStmt)
	init = append(init, r)
