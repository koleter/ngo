...
const (
	wordBits  = 32
	wordMask  = wordBits - 1
	wordShift = 5 // 考虑到2的5次方为32,这个应该是用于位运算除32的
)

// A bvec is a bit vector.
type bvec struct {
	n int32    // number of bits in vector,a bit is a pointer
	b []uint32 // words holding bits,uint32作为bitset
}

func bvalloc(n int32) bvec {
	nword := (n + wordBits - 1) / wordBits // n除wordBits并向上取整,计算出用多少个WORD存储
	return bvec{n, make([]uint32, nword)}
}

type bulkBvec struct {
	words []uint32 // 该数组的长度为nword*(块的总数乘以7)
	nbit  int32
	nword int32 // nbit对wordBits(32)进行向上取整的值
}

func bvbulkalloc(nbit int32, count int32) bulkBvec {
	nword := (nbit + wordBits - 1) / wordBits // 将nbit对wordBits进行向上取整
...
// 将nword长度的bulkBvec的words成员截取并构造成bvec并返回
...
func (m *bvecSet) add(bv bvec) int {
	...
	for {
		...
		if j < 0 {
			...
			return len(m.uniq) - 1 // 返回插入的bit vector在m.uniq中的索引
		}
		jlive := m.uniq[j] // 找到之前存在的bit向量
		if bv.Eq(jlive) {  // 要插入的向量与现有的向量相同
			// Existing bvec.
			return j
		}
