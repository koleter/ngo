...
type itabEntry struct {
	t, itype *types.Type // t为实现了itype接口的类型
...
var (
	...
	signatset   = make(map[*types.Type]struct{}) // key为类型
	signatslice []*types.Type                    // 存储签署的类型
...
type Sig struct {
	name  *types.Sym  // 声明的函数的Sym
	isym  *types.Sym  // 类型使用接口表示的符号类型
	tsym  *types.Sym  // 与特定接收者类型关联的方法名称的方法符号,用于区分出现在不同方法集中的相同方法,例如,T.M和（* T）.M具有不同的方法符号
	type_ *types.Type // 通过receiver调用的函数类型,也是接口可用的类型
	mtype *types.Type // 不通过receiver调用的函数类型
}
...
func bmap(t *types.Type) *types.Type {
	...
	otyp := types.NewPtr(bucket) // 创建结构体指针,指向bucket
	if !elemtype.HasPointers() && !keytype.HasPointers() {
		otyp = types.Types[TUINTPTR] // key与value都没有指针otyp的类型改为uintptr
	}
	overflow := makefield("overflow", otyp) // 创建overflow指针
...
func deferstruct(stksize int64) *types.Type {
	...
	argtype := types.NewArray(types.Types[TUINT8], stksize) // stksize长度的字节数组
	...
	s := types.New(TSTRUCT)
	s.SetNoalg(true) // 不生成hash以及equals函数
	s.SetFields(fields)
...
func methodfunc(f *types.Type, receiver *types.Type) *types.Type {
	inLen := f.Params().Fields().Len() // 获取函数f的入参数量
	if receiver != nil {
		inLen++
	}
	in := make([]*Node, 0, inLen) // 整理入参列表
	...
	out := make([]*Node, 0, outLen) // 整理返回值列表
...
func methods(t *types.Type) []*Sig {
	...
	for _, f := range mt.AllMethods().Slice() {
		...
		this := f.Type.Recv().Type // 获取声明的函数的receiver

		if !sig.isym.Siggen() {
			sig.isym.SetSiggen(true)
			if !types.Identical(this, it) { // 函数的receiver类型与it不同
...
func imethods(t *types.Type) []*Sig {
	var methods []*Sig
	for _, f := range t.Fields().Slice() { // 接口的所有声明的字段,或者说是函数
		...
		if n := len(methods); n > 0 {
			last := methods[n-1] // methods中的最后一项
		...
		if !isym.Siggen() {
			isym.SetSiggen(true) // 标记isym是生成的Sym
			genwrapper(t, f, isym)
		}
	}
...
func dimportpath(p *types.Pkg) {
	if p.Pathsym != nil { // 已经导入了该package
		return
	}
...
func dnameData(s *obj.LSym, ot int, name, tag string, pkg *types.Pkg, exported bool) int {
	...
	var bits byte // 第一个bit表示是否可导出，第二bit表示是否存在tag，第三bit表示pkg不为nil
	l := 1 + 2 + len(name)
	if exported {
		bits |= 1 << 0
	}
	...
	b := make([]byte, l)
	b[0] = bits
	b[1] = uint8(len(name) >> 8) // name的长度用2个字节小端序存储
	b[2] = uint8(len(name))
	copy(b[3:], name)
	if len(tag) > 0 {
		tb := b[3+len(name):]
		tb[0] = uint8(len(tag) >> 8) // tag的长度用2个字节小端序存储
		tb[1] = uint8(len(tag))
		copy(tb[2:], tag)
	}
...
func dextratype(lsym *obj.LSym, ot int, t *types.Type, dataAdd int) int {
	...
	noff := int(Rnd(int64(ot), int64(Widthptr))) // 内存对齐
	if noff != ot {
		Fatalf("unexpected alignment in dextratype for %v", t)
	}

	for _, a := range m {
		dtypesym(a.type_) // 处理method
	}

	ot = dgopkgpathOff(lsym, ot, typePkg(t))

	dataAdd += uncommonSize(t)
	mcount := len(m) // 声明的可调用函数数量
	if mcount != int(uint16(mcount)) {
		Fatalf("too many methods on %v: %d", t, mcount)
	}
	xcount := sort.Search(mcount, func(i int) bool { return !types.IsExported(m[i].name.Name) }) // 找到函数集m中index最小的不可导出的函数下标
	if dataAdd != int(uint32(dataAdd)) {
		Fatalf("methods are too far away on %v: %d", t, dataAdd)
	}

	ot = duint16(lsym, ot, uint16(mcount))  // 写入函数数量
	ot = duint16(lsym, ot, uint16(xcount))  // 写入函数集中index最小的不可导出的函数下标
	ot = duint32(lsym, ot, uint32(dataAdd)) // 写入uncommonSize
	ot = duint32(lsym, ot, 0)               // 写入0
	return ot
}
...
func typeptrdata(t *types.Type) int64 {
	if !t.HasPointers() { // 类型t中不包含堆指针就返回0
		return 0
	}
	...
	case TSTRING: // string结构体中只有str一个指针,所以是Widthptr
		// struct { byte *str; intgo len; }
		return int64(Widthptr)

	case TINTER: // 两个指针类型组成的结构体
	...
	case TSTRUCT:
		...
		for _, t1 := range t.Fields().Slice() { // 遍历结构体的所有字段
			if t1.Type.HasPointers() { // 如果t1字段中包含堆指针
				lastPtrField = t1
			}
		}
...
const (
	tflagUncommon      = 1 << 0
	tflagExtraStar     = 1 << 1 // 写出非指针类型时连同该类型的指针类型一同写出
	tflagNamed         = 1 << 2
...
func dcommontype(lsym *obj.LSym, t *types.Type) int {
	dowidth(t)
	eqfunc := geneq(t) // 用于t类型的比较函数

	sptrWeak := true
	var sptr *obj.LSym
	if !t.IsPtr() || t.IsPtrElem() {
		tptr := types.NewPtr(t) // 指向t的指针
	...
	//	type rtype struct {
	//		size          uintptr
	//		ptrdata       uintptr	ptrdata表示该类型最后一个堆指针前面所有成员所占字节数
	//		hash          uint32
	//		tflag         tflag
	//		align         uint8
	//		fieldAlign    uint8
	...
	//	}
	ot := 0
	ot = duintptr(lsym, ot, uint64(t.Width)) // 将t.Width以一个指针的长度写入lsym
	ot = duintptr(lsym, ot, uint64(ptrdata)) // 将ptrdata以一个指针的长度写入lsym
	ot = duint32(lsym, ot, typehash(t))      // 将t的typehash以4字节的长度写入lsym
...
func typename(t *types.Type) *Node {
	s := typenamesym(t) // 获取t代表的符号
	if s.Def == nil {   // 类型未定义
		n := newnamel(src.NoXPos, s) // 通过s这个Sym创建一个ONAME节点
		n.Type = types.Types[TUINT8]
		n.SetClass(PEXTERN) // 设为全局变量
		n.SetTypecheck(1)
		s.Def = asTypesNode(n)
	}

	n := nod(OADDR, asNode(s.Def), nil) // 相当于获取类型对应的ONAME节点的地址
...
func itabname(t, itype *types.Type) *Node {
	...
	if s.Def == nil {
		n := newname(s) // 根据符号s新建一个ONAME节点
		n.Type = types.Types[TUINT8]
		n.SetClass(PEXTERN) // 设置为全局变量
		n.SetTypecheck(1)
		s.Def = asTypesNode(n)
...
func dtypesym(t *types.Type) *obj.LSym {
	...
	s := typesym(t) // 根据类型获取符号
	lsym := s.Linksym()
	if s.Siggen() { // type symbol has been generated
		return lsym
	}
	s.SetSiggen(true)
	...
	if myimportpath != "runtime" || (tbase != types.Types[tbase.Etype] && tbase != types.Bytetype && tbase != types.Runetype && tbase != types.Errortype) { // int, float, etc
		...
		if tbase.Sym != nil && tbase.Sym.Pkg != localpkg { // 在其他包中声明了就使用其他包中声明的内容
			return lsym
		}
	...
	case TARRAY:
		...
		s2 := dtypesym(t2)
		ot = dcommontype(lsym, t)                    // typ
		ot = dsymptr(lsym, ot, s1, 0)                //	elem
		ot = dsymptr(lsym, ot, s2, 0)                //	slice
		ot = duintptr(lsym, ot, uint64(t.NumElem())) //	len
		ot = dextratype(lsym, ot, t, 0)
	...
	case TSLICE:
		...
		//	elem *_type
		//}
		s1 := dtypesym(t.Elem())      // 存储的元素的类型的链接符号
		ot = dcommontype(lsym, t)     //	typ  _type
		ot = dsymptr(lsym, ot, s1, 0) //	elem *_type
		ot = dextratype(lsym, ot, t, 0)
	...
	case TCHAN:
		...
		ot = dcommontype(lsym, t)                    //	typ  _type
		ot = dsymptr(lsym, ot, s1, 0)                //	elem *_type
		ot = duintptr(lsym, ot, uint64(t.ChanDir())) //	dir  uintptr
		ot = dextratype(lsym, ot, t, 0)
	...
	case TFUNC:
		...
		//	outCount uint16
		//}
		for _, t1 := range t.Recvs().Fields().Slice() { // 写入receiver的类型符号
			dtypesym(t1.Type)
		}
		isddd := false
		...
		ot = dcommontype(lsym, t)               //	typ      _type
		inCount := t.NumRecvs() + t.NumParams() // 入参数量(算上receiver)
		outCount := t.NumResults()              // 出参数量
		if isddd {
			outCount |= 1 << 15
		}
		ot = duint16(lsym, ot, uint16(inCount))  //	inCount  uint16
		ot = duint16(lsym, ot, uint16(outCount)) //	outCount uint16
		if Widthptr == 8 {
	...
	case TINTER:
		...
		ot = dgopkgpath(lsym, ot, tpkg) //	pkgpath name

		ot = dsymptr(lsym, ot, lsym, ot+3*Widthptr+uncommonSize(t))
		ot = duintptr(lsym, ot, uint64(n))
		ot = duintptr(lsym, ot, uint64(n))
		dataAdd := imethodSize() * n
		ot = dextratype(lsym, ot, t, dataAdd) // 写入imethod的字节数
		...
		for _, a := range m {
			...
			//	ityp typeOff
			//}
			exported := types.IsExported(a.name.Name) // 接口所声明的函数是否可导出
			var pkg *types.Pkg
	...
	case TSTRUCT:
		...
		for _, f := range fields {
			if !types.IsExported(f.Sym.Name) { // 结构体的字段是不可导出的
				spkg = f.Sym.Pkg
				break
			}
		}
...
func genfun(t, it *types.Type) []*obj.LSym {
	...
	for _, m := range methods {
		if m.name == sigs[0].name { // 将函数按照interface的字段顺序进行排序
...
func dumpsignats() {
	...
	for len(signatslice) > 0 { // 存在已被签署的类型
		signats = signats[:0]
		// Transfer entries to a slice and sort, for reproducible builds.
		for _, t := range signatslice { // 将signatset中的元素组合为typeAndStr结构体并保存到signats
			signats = append(signats, typeAndStr{t: t, short: typesymname(t), regular: t.String()})
			delete(signatset, t)
		}
		signatslice = signatslice[:0] // 清空signatslice
		sort.Sort(typesByString(signats))
		for _, ts := range signats {
			t := ts.t
			dtypesym(t)
			if t.Sym != nil {
				dtypesym(types.NewPtr(t)) // 生成该t类型的指针类型
			}
		}
	}
}
...
func dumptabs() {
	...
	for _, i := range itabs {
		...
		// }
		o := dsymptr(i.lsym, 0, dtypesym(i.itype), 0) //   inter  *interfacetype  记录接口类型
		o = dsymptr(i.lsym, o, dtypesym(i.t), 0)      //   _type  *_type
		o = duint32(i.lsym, o, typehash(i.t))         // copy of type hash
		o += 4                                        // skip unused field
		for _, fn := range genfun(i.t, i.itype) {     // 对于实现了itype类型的所有t的对应方法
...
func dumpimportstrings() {
	...
	for _, p := range types.ImportedPkgList() { // 遍历所有的直接引入的包
		dimportpath(p)
	}
}
...
func dumpbasictypes() {
	...
	if myimportpath == "runtime" {
		for i := types.EType(1); i <= TBOOL; i++ { // dump所有的基本类型
...
func dgcptrmask(t *types.Type) *obj.LSym {
	ptrmask := make([]byte, (typeptrdata(t)/int64(Widthptr)+7)/8) // 依据类型t的ptrdata计算掩码(ptrdata所占用的指针数量除8向上取整)
...
func fillptrmask(t *types.Type, ptrmask []byte) {
	for i := range ptrmask { // 将ptrmask中的所有元素初始化为0
		ptrmask[i] = 0
	}
	if !t.HasPointers() { // 如果t中不包含堆指针就返回
		return
	}

	vec := bvalloc(8 * int32(len(ptrmask))) // ptrmask是byte类型,8bit,所以这里乘8
	onebitwalktype1(t, 0, vec)              // 对最后一个堆指针之前的所有成员设置其偏移对应的指针数所在的bit位到vec.b中

	nptr := typeptrdata(t) / int64(Widthptr) // 类型t的ptrdata占用多少个指针空间
...
func dgcprog(t *types.Type) (*obj.LSym, int64) {
	...
	p.init(lsym)
	p.emit(t, 0)
	offset := p.w.BitIndex() * int64(Widthptr) // 写入数据的总偏移
	p.end()
...
func (p *GCProg) emit(t *types.Type, offset int64) {
	dowidth(t)
	if !t.HasPointers() { // 不包含指向堆中的指针就返回
		return
	}
	if t.Width == int64(Widthptr) { // t类型只占用一个指针大小或者通过一个指针进行存储
