...
func eqstring(s, t *Node) (eqlen, eqmem *Node) {
	...
	call := nod(OCALL, fn, nil)	// 创建一个调用fn的节点
	call.List.Append(sptr, tptr, slen.copy())	// 设置函数的参数
	call = typecheck(call, ctxExpr|ctxMultiOK)

	cmp := nod(OEQ, slen, tlen)	// 比较两个字符串的长度是否相同
...
func eqinterface(s, t *Node) (eqtab, eqdata *Node) {
	...
	call.List.Append(stab, sdata, tdata)	// 设置实参
	call = typecheck(call, ctxExpr|ctxMultiOK)
