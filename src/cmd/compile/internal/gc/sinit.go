...
func (s *InitSchedule) staticcopy(l *Node, r *Node) bool {
	...
	case OPTRLIT:	// &Left (left is composite literal)
		switch r.Left.Op {
...
const (
	inInitFunction initContext = iota	// OSTRUCTLIT
	inNonInitFunction		// OARRAYLIT
)
...
func getdyn(n *Node, top bool) initGenType {
	...
	case OSLICELIT:	// Type{List} (composite literal, Type is slice) Right.Int64() = slice length.
		if !top {
			return initDynamic
		}
	...
	for _, n1 := range n.List.Slice() {	// 遍历所有元素
		switch n1.Op {
		case OKEY:
			n1 = n1.Right
		case OSTRUCTKEY:
...
func fixedlit(ctxt initContext, kind initKind, n *Node, var_ *Node, init *Nodes) {
	...
	case OARRAYLIT, OSLICELIT:
		...
		splitnode = func(r *Node) (*Node, *Node) {
			...
			k++
			return a, r	// 取下标元素的操作，获取到的元素
		}
	case OSTRUCTLIT:
		splitnode = func(r *Node) (*Node, *Node) {
			if r.Op != OSTRUCTKEY {	// Sym:Left (key:value in struct literal, after type checking)
				Fatalf("fixedlit: rhs not OSTRUCTKEY: %v", r)
			}
			if r.Sym.IsBlank() {	// _: Left这样的键值对
	...
	for _, r := range n.List.Slice() {	// 遍历所有的元素
		a, value := splitnode(r)
		...
		islit := isLiteral(value)	// 判断value是否为字面量
...
func slicelit(ctxt initContext, n *Node, var_ *Node, init *Nodes) {
	...
	if ctxt == inNonInitFunction {	// 不在初始化函数中
	...
	mode := getdyn(n, true)	// 判断n需要静态初始化还是动态初始化或者同时都有
	if mode&initConst != 0 && !isSmallSliceLit(n) {	// 静态初始化且切片较大
		vstat = staticname(t)
	...
	vauto := temp(types.NewPtr(t))	// 创建t类型的指针数组临时变量
	...
	var index int64
	for _, value := range n.List.Slice() {	// 遍历切片字面量中的元素
		if value.Op == OKEY {
			index = indexconst(value.Left)	// 获取数组索引
			if index < 0 {
				Fatalf("slicelit: invalid index %v", value.Left)
			}
			value = value.Right	// 获取值
		}
		a := nod(OINDEX, vauto, nodintconst(index))	// 取数组下标中的值
		a.SetBounded(true)
		index++
...
func anylit(n *Node, var_ *Node, init *Nodes) {
	...
	case ONAME:
		a := nod(OAS, var_, n)	// 赋值 var_ = n
		a = typecheck(a, ctxStmt)
		init.Append(a)

	case OPTRLIT:	// &Left (left is composite literal)		var_ = &n
		if !t.IsPtr() {
		...
		if n.Right != nil {
			...
			init.Append(nod(OAS, n.Right, nil)) // zero backing store, just in case (#18410)	初始化字面量指针为nil
			r = nod(OADDR, n.Right, nil)	// 对n.Right取地址
			r = typecheck(r, ctxExpr)
		} else {
			r = nod(ONEW, nil, nil)	// new(Left); corresponds to calls to new in source code	通过new创建一个t类型的指针
			r.SetTypecheck(1)
			r.Type = t
			r.Esc = n.Esc
		}
		...
		var_ = nod(ODEREF, var_, nil)	// 对var_解引用
		var_ = typecheck(var_, ctxExpr|ctxAssign)
		anylit(n.Left, var_, init)	// 将双方指针解引用后继续递归
	...
	case OSTRUCTLIT, OARRAYLIT:
		...
		if n.Op == OARRAYLIT {	// 如果n是数组字面量
			components = t.NumElem()	// 容量为数组的长度
		} else {
			components = int64(t.NumFields())	// n是结构体字面量的话那么容量就是字段数量
		}
		// initialization of an array or struct with unspecified components (missing fields or arrays)
		if var_.isSimpleName() || int64(n.List.Len()) < components {
			a := nod(OAS, var_, nil)	// 初始化数组或者结构体为nil
			a = typecheck(a, ctxStmt)
			a = walkexpr(a, init)
			init.Append(a)
		}
...
func oaslit(n *Node, init *Nodes) bool {
	...
	if !n.Left.isSimpleName() {	// 如果n.Left是全局变量或者移动到了堆中的局部变量
		// not a special composite literal assignment
		return false
	}
	...
	// 到了这里的话,n.Left应该是局部存储在栈中的变量且n.Left与n.Right的类型相同
	switch n.Right.Op {
	default:
		// not a special composite literal assignment
		return false
