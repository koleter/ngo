...
func ImportFromFiles(gofiles []string) {
	...
	tags := imports.Tags()                                        //a map something is true
	imports, testImports, err := imports.ScanFiles(gofiles, tags) // 获取gofiles导入的所有包
	if err != nil {
...
type loadPkg struct {
	...
	stack       *loadPkg       // package importing this one in minimal import stack for this pkg		初始指向自身,后期指向导入自己的loadPkg,root包为nil
...
func (ld *loader) load(roots func() []string) {
	...
	for {
		...
		for _, pkg := range ld.pkgs { // 遍历引用的所有的包
...
func (ld *loader) doPkg(item interface{}) {
	...
	if pkg.testOf != nil {
		...
		pkg.mod, pkg.dir, pkg.err = Import(pkg.path) // 根据导入的包名找到这个包所在路径
		if pkg.dir == "" {
			return
		}
		var testImports []string
		var err error
		imports, testImports, err = scanDir(pkg.dir, ld.tags) // 获取这个包导入的其他包以及有测试的包
		if err != nil {
			pkg.err = err
			return
		}
...
func (ld *loader) buildStacks() {
	...
	for i := 0; i < len(ld.pkgs); i++ { // not range: appending to ld.pkgs in loop		// 遍历并将所有用到的包保存到ld.pkgs中
		pkg := ld.pkgs[i]
		for _, next := range pkg.imports {
			if next.stack == nil {
				next.stack = pkg // 记录pkg导入了next
				ld.pkgs = append(ld.pkgs, next)
			}
		}
		if next := pkg.test; next != nil && next.stack == nil { // 如果当前包有测试用例,导入该包
			next.stack = pkg
			ld.pkgs = append(ld.pkgs, next)
		}
	}
