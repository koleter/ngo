...
type PackagePublic struct {
	...
	Target        string                `json:",omitempty"` // installed target for this package (may be executable)	编译后生成的目标程序
...
func setToolFlags(pkgs ...*Package) {
	for _, p := range PackageList(pkgs) { // 遍历root包引用的所有包
...
func PackagesForBuild(args []string) []*Package {
	...
	for _, pkg := range PackageList(pkgs) { // 校验是否有重复导入的包
...
func GoFilesPackage(gofiles []string) *Package {
	...
	for _, f := range gofiles { // 检查,所有的文件必须是.go结尾
	...
	var dirent []os.FileInfo // 所有文件列表对应的FileInfo列表
	var dir string           // 所有go文件的父目录
	...
	setToolFlags(pkg) // 为每个包赋予编译的flags
