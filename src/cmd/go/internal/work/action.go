...
type Builder struct {
	...
	exec      sync.Mutex
	readySema chan bool
	ready     actionQueue //可以进行编译的action
...
type Action struct {
	...
	Func       func(*Builder, *Action) error // the action itself (nil = no-op)		Action执行的函数
...
func (b *Builder) CompileAction(mode, depMode BuildMode, p *load.Package) *Action {
	...
	a := b.cacheAction("build", p, func() *Action {
		...
		if p.Error == nil || !p.Error.IsImportCycle {
			for _, p1 := range p.Internal.Imports { //处理直接引用的依赖包
...
func (b *Builder) LinkAction(mode, depMode BuildMode, p *load.Package) *Action {
	...
	a := b.cacheAction("link", p, func() *Action {
		...
		a.Deps = []*Action{a1}   // 依赖只有一个,也就是说最终要链接的只有一个main包
		a.Objdir = a1.Objdir
		...
		a.built = a.Target                 //实际创建的可执行文件名(绝对路径)
		b.addTransitiveLinkDeps(a, a1, "") //将a1中的deps添加到a
