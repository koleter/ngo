...
func (b *Builder) Do(root *Action) {
	...
	// Initialize per-action execution state.	初始化每个action的执行状态
	for _, a := range all {
		for _, a1 := range a.Deps {
	...
	handle := func(a *Action) { //针对单个action的处理
		...
		if a.Func != nil && (!a.Failed || a.IgnoreFail) {
			err = a.Func(b, a) //执行action绑定的操作	build函数
		}
		if a.json != nil {
...
func (b *Builder) build(a *Action) (err error) {
	...
	if !p.BinaryOnly { //可以重编译的package
		...
		// (e.g., go list -compiled -find).	 源文件缓存的处理
	...
	// make target directory	创建package编译的目标文件的目录
	...
	// Collect symbol ABI requirements from assembly.	从.s文件中获取ABI
	...
	ofile, out, err := BuildToolchain.gc(b, a, objpkg, icfg.Bytes(), symabis, len(sfiles) > 0, gofiles) // 具体go的编译
	if len(out) > 0 {
