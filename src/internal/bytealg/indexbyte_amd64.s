...
TEXT	·IndexByteString(SB), NOSPLIT, $0-32
	MOVQ s_base+0(FP), SI	//第一个参数，字符串指针
	MOVQ s_len+8(FP), BX	//第二个参数，length
	MOVB c+16(FP), AL		//第三个参数保存到AL寄存器，在字符串中查找的指定byte
	LEAQ ret+24(FP), R8		//返回值地址保存到r8
	JMP  indexbytebody<>(SB)
...
TEXT	indexbytebody<>(SB), NOSPLIT, $0
	...
	CMPQ BX, $16	//待查询字符串长度小于16走small的逻辑
	JLT small

	MOVQ SI, DI	//字符串指针赋给SI

	CMPQ BX, $32	//待查询字符串长度大于32走avx2的逻辑
	JA avx2
sse:
